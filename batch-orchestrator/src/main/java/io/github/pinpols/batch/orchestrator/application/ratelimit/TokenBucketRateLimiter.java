 ...
 * maxPerMinute}/min。
 *
 * <p><b>时钟回拨</b>：不再需要旧的自研 CAS 回拨保护。旧保护是为“固定窗口按客户端 wall-clock 对齐 key”而设—— 回拨会让老窗口 key
 * 复活叠加计数。令牌桶不做窗口对齐：桶状态存在 Redis，令牌按 refill 速率在 Redis 服务端 时间单调累积，与任一副本的本地 wall-clock
 * 无关，故单机时钟回拨不会击穿配额。
 *
...
public class TokenBucketRateLimiter {
  ...
  public boolean tryConsume(String tenantId, String action, long maxPerMinute) {
    ...
    try {
      return proxyManager.getProxy(key, bucketConfiguration(maxPerMinute)).tryConsume(1);
    } catch (RedisException ex) {
      // fail-open：Redis 不可达时放行，与旧固定窗口实现同方向。宁可短暂不限流，也不因 Redis 抖动整体 5xx。
      log.warn(
  ...
  private Supplier<BucketConfiguration> bucketConfiguration(long maxPerMinute) {
    return () ->
