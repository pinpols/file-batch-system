 ...
 *
 * <ul>
 *   <li><b>策略链权重</b>: 实现必须用 {@code @Order} 指定优先级,序号越小越先尝试; 数据量 / 时长 / Worker 容量等通用估算应放在显式覆盖之后, 避免
 *       override 形同虚设。
 *   <li><b>不可扩展边界</b>: 平台分片语义属于 Pipeline / Job 调度核心, <b>不允许在无 ADR 批准的情况下新增实现</b>; 新增前必须先确认现有 4 个实现
 *       (显式 / 数据量 / 时长 / Worker) 无法满足业务。
