 ...
 * 这种方式产生的也是 Spring 管理 bean,理论上 {@code @Scheduled} 心跳能生效;但为消除歧义, 已新增 {@link #selfCheckOnStartup()}
 * 启动期自检,首次 heartbeat 失败会立刻 WARN 暴露。 真正改 @Component 需要把构造参数 (memberId / memberTtl) 重构为 @Value /
 * properties bean 注入, 牵动分片机制初始化时序,留待人工审阅。
 */
@Slf4j
