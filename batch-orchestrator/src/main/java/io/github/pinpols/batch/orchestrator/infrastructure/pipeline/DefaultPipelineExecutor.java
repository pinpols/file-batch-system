...
import io.github.pinpols.batch.orchestrator.application.route.WorkerRouter;
import io.github.pinpols.batch.orchestrator.domain.pipeline.ExecutionContext;
import io.github.pinpols.batch.orchestrator.domain.pipeline.PipelineDefinition;
import io.github.pinpols.batch.orchestrator.domain.pipeline.PipelineExecutionResult;
import io.github.pinpols.batch.orchestrator.domain.pipeline.PipelineExecutor;
...
import io.github.pinpols.batch.orchestrator.domain.pipeline.StepRegistry;
import io.github.pinpols.batch.orchestrator.domain.pipeline.StepResult;
 ...
 * 默认流水线执行器。
 *
 * <p>按 {@code stepOrder} 升序依次执行 {@link PipelineDefinition} 中已启用的步骤， 每步通过 {@link StepRegistry} 查找对应的
 * {@link Step} 实现并传入 {@link ExecutionContext} 驱动执行。 Worker 路由优先使用 {@code
 * context.defaultWorkerRoute}；若步骤定义覆盖了 {@code workerType}， 则克隆路由对象并替换类型与能力标签，避免修改共享引用。执行结果汇总为
