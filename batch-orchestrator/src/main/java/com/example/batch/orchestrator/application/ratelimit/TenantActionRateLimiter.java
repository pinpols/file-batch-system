...
/**
 * 租户级动作限流门面：{@link RateLimitAction} 决定读哪个配额配置，然后委托 {@link TokenBucketRateLimiter}
 * 做 Redis 固定窗口计数。
 *
 * <p>放行短路：
 *
 ...
 *   <li>{@code tenantId} 为空：无法归属到租户则不限流——防御式处理内部调用缺失 tenantId 的场景，
 *       限流只对已识别的租户生效。
 * </ul>
 *
 * <p>未登记的 action 视为 max=0 即直接拒绝（走底层 {@code maxPerMinute <= 0} 的放行逻辑实际上会通过——
 * 见 {@link TokenBucketRateLimiter#tryConsume}），注册新 action 时务必同步在此分支补 quota 源。
 */
@Component
