 ...
 *
 * <p><b>C-2.4 c · 时钟回拨保护</b>：若检测到 {@code nowEpochMillis} 比上次调用早超过
 * {@link #CLOCK_ROLLBACK_TOLERANCE_MILLIS}，记 WARN 并直接拒绝本次请求——回拨可能让
 * 已过期的老窗口 key 被重新激活叠加计数，导致限流失效。宁可短暂丢请求，也不让配额被击穿。
 */
@Slf4j
@Component
public class TokenBucketRateLimiter {
  ...
  /**
   * 用 CAS 单调推进 lastSeenMillis；若传入 now 比 last 早超过容忍阈值即视为回拨。
   * CAS 更新不成功时不重试——说明另一线程已经把时间推到更晚，我们直接以当次 now 判断即可。
   */
