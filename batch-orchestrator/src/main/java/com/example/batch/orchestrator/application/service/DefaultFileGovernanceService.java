...
public class DefaultFileGovernanceService implements FileGovernanceService {
    ...
    public String presignFileDownload(FileGovernanceCommand command) {
        ...
        Map<String, Object> security =
                fileGovernanceRepository.loadTemplateSecurityForFile(
                        command.tenantId(), command.fileId());
        if (requiresDownloadApproval(security) && !StringUtils.hasText(command.approvalId())) {
            throw new BizException(
                    ResultCode.BUSINESS_ERROR,
                    "approvalId is required for download on this file template");
        }
        ...
        auditDetail.put("approvalId", command.approvalId());
        auditDetail.put(
                "downloadRequiresApproval", truthy(security.get("download_requires_approval")));
        auditDetail.put(
                "contentEncryptionEnabled", truthy(security.get("content_encryption_enabled")));
        auditDetail.put("encryptionKeyRef", security.get("encryption_key_ref"));
    ...
    public String operateArrivalGroup(ArrivalGroupGovernanceCommand command) {
        ...
        if ("EMPTY_RUN".equals(action) && !toBoolean(groupFiles.get(0).get("allow_empty_run"))) {
            throw new BizException(
                    ResultCode.STATE_CONFLICT, "arrival group does not allow empty run");
        }
        if ("SKIP_BATCH".equals(action)
                && !toBoolean(groupFiles.get(0).get("allow_skip_biz_date"))) {
            throw new BizException(
                    ResultCode.STATE_CONFLICT, "arrival group does not allow skip batch");
        }
    ...
    private JobTaskEntity resolveDispatchTask(String tenantId, Long jobInstanceId) {
        List<JobTaskEntity> tasks =
                jobTaskMapper.selectByQuery(
                        new JobTaskQuery(tenantId, jobInstanceId, null, null, null));
        return tasks.stream()
                .filter(task -> "DISPATCH".equalsIgnoreCase(task.getTaskType()))
                .sorted(
                        Comparator.comparing(
                                JobTaskEntity::getTaskSeq,
                                Comparator.nullsLast(Integer::compareTo)))
                .findFirst()
                .orElseThrow(
                        () -> new BizException(ResultCode.NOT_FOUND, "dispatch task not found"));
    }
    ...
    private void assertNoActiveRuntime(FileGovernanceCommand command) {
        long activePipelines =
                fileGovernanceRepository.countActivePipelineInstances(
                        command.tenantId(), command.fileId());
        if (activePipelines > 0) {
            throw new BizException(
                    ResultCode.STATE_CONFLICT, "file still has active pipeline instances");
        }
        long pendingDispatches =
                fileGovernanceRepository.countPendingDispatchRecords(
                        command.tenantId(), command.fileId());
        if (pendingDispatches > 0) {
            throw new BizException(
                    ResultCode.STATE_CONFLICT, "file still has pending dispatch records");
        }
    }
