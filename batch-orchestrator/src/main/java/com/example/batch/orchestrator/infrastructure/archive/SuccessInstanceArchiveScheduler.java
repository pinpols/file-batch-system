...
/**
 * P3-3 archive 系列：SUCCESS / PARTIAL_FAILED job_instance 级联归档调度器。默认每周日 04:30 跑
 * （与 file-archive、quota-reset、workflow-archive 错峰；周频已足够 30 天保留窗口）。
 *
 * <p>单 tick 内最多连刷 {@link #MAX_BATCHES_PER_TICK} 批，每批清 batchSize（默认 1000）个 instance；
 * 命中上限或抓不到候选自然停。
 */
@Slf4j
@Component
