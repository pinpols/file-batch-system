...
public interface WorkerRegistryMapper {
   ...
   *
   * <p>约定：{@code capability_tags} 要么为 NULL / 空串（表示无能力标签），要么是 JSONB 字符串数组（如
   * {@code ["ingest","delivery"]}）。不符合的形态（对象、标量、含非字符串元素的数组）会让
   * {@link com.example.batch.orchestrator.infrastructure.scheduler.DefaultWorkerSelector} 静默把该 worker
