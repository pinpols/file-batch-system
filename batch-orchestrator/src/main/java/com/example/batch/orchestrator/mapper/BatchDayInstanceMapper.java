 ...
 *
 * <ul>
 *   <li>{@link #insert(BatchDayInstanceRecord)} 用 {@code ON CONFLICT (tenant_id, calendar_code,
 *       biz_date) DO NOTHING}：并发首次创建只一行落库；caller 拿不到 id 也无所谓，下一轮 {@link
 *       #selectByTenantCalendarBizDate} 自然读到。
 *   <li>{@link #updateWithCas(BatchDayInstanceRecord)} 走 {@code WHERE id=? AND version=?} CAS：返回值 0
...
public interface BatchDayInstanceMapper {
   ...
   */
  int insert(BatchDayInstanceRecord record);
