...
/**
 * 分区生命周期管理服务。
 * 负责 JobPartition 的创建、认领（CLAIM）、租约续期、过期回收以及状态释放等全生命周期操作。
 * 默认的 {@code createPartitions} 以 {@code CREATED} 状态初始化分区；
 * {@code releaseForDispatch} 须以乐观并发方式校验 fromStatus，确保分区状态机的单向推进。
 */
