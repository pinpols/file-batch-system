 ...
 *
 * <ul>
 *   <li>{@link #insert(QuotaRuntimeStateRecord)} 用 {@code ON CONFLICT (tenant_id, quota_scope,
 ...
 *   <li>{@link #updateWithCas(QuotaRuntimeStateRecord)} 走 {@code WHERE id=? AND version=?} CAS：返回值
...
public interface QuotaRuntimeStateMapper {
   ...
   */
  int insert(QuotaRuntimeStateRecord record);
   ...
   */
  int updateWithCas(QuotaRuntimeStateRecord record);

  /** 按租户 + 维度 + owner（unique constraint 三元组）查询当前行。 */
  QuotaRuntimeStateRecord selectByTenantQuotaScopeOwner(
  ...
  List<QuotaRuntimeStateRecord> selectExpired(@Param("now") Instant now);
}
