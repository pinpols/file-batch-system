...
public class LaunchBatchDayService {
  ...
  void upsertBatchDayInstance(
    ...
    for (int attempt = 1; attempt <= UPSERT_MAX_ATTEMPTS; attempt++) {
      try {
        ...
        logUpsertRetry(
            "concurrent insert (uk_batch_day_instance); will retry as update", request, attempt);
      }
    }
    throw last;
  }

  private void logUpsertRetry(String reason, LaunchRequest request, int attempt) {
    log.info(
        "batch day upsert {}: tenantId={}, jobCode={}, bizDate={}, attempt={}/{}",
        reason,
  ...
  private void insertNewBatchDay(
      LaunchRequest request, BatchDayUpsertContext ctx, Instant batchDaySlaDeadlineAt) {
  ...
  private static String resolveCreateDayStatus(BatchDayUpsertContext ctx, boolean pastCutoff) {
    if (ctx.catchUpLaunch() || ctx.lateAccepted()) {
      return "IN_FLIGHT";
    }
    return pastCutoff ? "CUTOFF" : "OPEN";
  }

  private static String resolveCreateReasonCode(BatchDayUpsertContext ctx, boolean pastCutoff) {
    if (ctx.lateAccepted()) {
      return REASON_LATE_ACCEPTED;
    }
    if (ctx.catchUpLaunch()) {
      return "CATCH_UP_LAUNCHED";
    }
    return pastCutoff ? "CUTOFF_REACHED_ON_CREATE" : "BATCH_DAY_OPENED";
  }

  private void updateExistingBatchDay(
      LaunchRequest request,
      BatchDayUpsertContext ctx,
      BatchDayInstanceEntity existing,
      Instant batchDaySlaDeadlineAt) {
    BatchDayUpdatePlan plan = planBatchDayUpdate(ctx, existing, batchDaySlaDeadlineAt);
    if (!plan.changed()) {
      return;
    }
  ...
  private BatchDayUpdatePlan planBatchDayUpdate(
      BatchDayUpsertContext ctx, BatchDayInstanceEntity existing, Instant batchDaySlaDeadlineAt) {
    boolean pastCutoff = ctx.cutoffAt() != null && !ctx.now().isBefore(ctx.cutoffAt());
    boolean shouldMoveToCutoff = "OPEN".equalsIgnoreCase(existing.dayStatus()) && pastCutoff;
    boolean shouldReopen =
        shouldReopenBatchDay(existing.dayStatus()) || ctx.lateAccepted() || ctx.catchUpLaunch();
    BatchDayInstanceEntity updated = existing;
    boolean changed = false;
    String reasonCode = null;
    if (updated.slaDeadlineAt() == null && batchDaySlaDeadlineAt != null) {
      updated = updated.withSlaDeadlineAt(batchDaySlaDeadlineAt, ctx.now());
      changed = true;
    }
    if (updated.cutoffAt() == null && ctx.cutoffAt() != null) {
      updated = updated.withCutoffAt(ctx.cutoffAt(), ctx.now());
      changed = true;
    }
    if (shouldMoveToCutoff && !ctx.lateAccepted() && !ctx.catchUpLaunch()) {
      updated = updated.withDayStatus("CUTOFF", ctx.now());
      changed = true;
      reasonCode = "CUTOFF_REACHED";
    }
    if (ctx.lateAccepted()) {
      updated =
          updated.withLateCount(LaunchParamResolver.safeIncrement(updated.lateCount()), ctx.now());
      changed = true;
    ...
    if (shouldReopen) {
      updated = updated.withReopened(ctx.now());
      changed = true;
      reasonCode = resolveReopenReason(ctx);
    }
    return new BatchDayUpdatePlan(updated, changed, reasonCode);
  }

  private static String resolveReopenReason(BatchDayUpsertContext ctx) {
    if (ctx.lateAccepted()) {
      return "LATE_ACCEPTED_REOPEN";
    }
  ...
  private record BatchDayUpdatePlan(
      BatchDayInstanceEntity updated, boolean changed, String reasonCode) {}
