...
public class WaitingPartitionDispatchScheduler {
    ...
    public void dispatchWaitingPartitions() {
        ...
        Comparator<WaitingDispatchCandidate> comparator =
                Comparator.comparingLong(WaitingDispatchCandidate::fairnessScore)
                        .reversed()
                        .thenComparing(
                                Comparator.comparingInt(WaitingDispatchCandidate::priority)
                                        .reversed())
    ...
    private WaitingDispatchCandidate buildCandidate(JobPartitionEntity partition) {
        ...
        ResourceSchedulingDecision decision =
                resourceScheduler.schedule(
                        buildRequest(jobInstance, partition, task, jobDefinition));
        ...
        BatchMdc.withTenantAndTrace(
                ...
                () -> {
                    ...
                    try {
                        ...
                        if (JobInstanceStatus.WAITING
                                .code()
                                .equals(jobInstance.getInstanceStatus())) {
                            int updated =
                        ...
                        if (workflowRun != null
                                && WorkflowRunStatus.CREATED
                                        .code()
                                        .equals(workflowRun.getRunStatus())) {
    ...
    private record WaitingDispatchCandidate(
        ...
        private long partitionId() {
            return partition == null || partition.getId() == null
                    ? Long.MAX_VALUE
                    : partition.getId();
        }
    }
