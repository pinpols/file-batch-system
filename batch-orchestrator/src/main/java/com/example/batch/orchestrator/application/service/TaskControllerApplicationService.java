 ...
 * 防止在并发场景下错误地响应成功。上报时对请求字段进行兼容处理——同时支持新版
 * {@code errorCode/errorMessage} 和旧版 {@code code/message} 字段，缺失时降级为 {@code "UNKNOWN"}，
 * 保证向后兼容。续期失败时抛出业务异常，要求 Worker 重新认领或放弃任务。
 *
 * <p>该类作为 HTTP Controller 与 {@link TaskExecutionService} 之间的薄适配层，
 * 不包含任务状态机核心逻辑，复杂业务逻辑下沉至 {@code TaskExecutionService}。
 */
@Service
