 ...
 *   <li>{@code priority}：基 topic 后追加 {@code .<priorityBand>}（HIGH / NORMAL / LOW），高优 topic
 *       独立 consumer group 防止低优堆积阻塞高优
 * </ul>
 *
 * <p>消费侧需要订阅对应 topic patterns（如 {@code task.dispatch.import.*}）才能接到分流后的消息；
 * worker 配置同步调整。
 */
@Data
