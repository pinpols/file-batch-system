...
class FileErrorRecordIntegrationTest extends AbstractIntegrationTest {
    ...
    void shouldInsertMultipleErrorsForSameFile() {
        ...
        Long count = jdbcTemplate.queryForObject(
                ...
                Long.class, "t1", fileId);

        assertThat(count).isEqualTo(2L);

        List<Map<String, Object>> skipped = jdbcTemplate.queryForList(
                "select * from batch.file_error_record where tenant_id = ? and file_id = ? and is_skipped = true",
                "t1", fileId);
    ...
    void shouldQueryByErrorStageIndex() {
        ...
        Long parseErrors = jdbcTemplate.queryForObject(
                "select count(*) from batch.file_error_record where tenant_id = ? and file_id = ? and error_stage = 'PARSE'",
                Long.class, "t1", fileId);
        Long validateErrors = jdbcTemplate.queryForObject(
                "select count(*) from batch.file_error_record where tenant_id = ? and file_id = ? and error_stage = 'VALIDATE'",
                Long.class, "t1", fileId);
