...
class BatchDaySqlMigrationsIntegrationTest {
    ...
    void emptyDb_migration_createsBatchDayInstanceAndBusinessCalendarColumns() {
        ...
        try {
            ...
            assertThat(columnExists(jdbc, "batch", "business_calendar", "late_arrival_tolerance_min")).isTrue();
            assertThat(columnExists(jdbc, "batch", "business_calendar", "sla_offset_min")).isTrue();

            assertThat(uniqueConstraintExists(jdbc, "batch", "batch_day_instance", "uk_batch_day_instance")).isTrue();
            // V32：乐观锁 version 列
    ...
    void existingDb_upgradeFromBeforeV31_appliesV31AndIsIdempotent() {
        ...
        try {
            ...
            assertThat(columnExists(jdbc, "batch", "business_calendar", "late_arrival_tolerance_min")).isFalse();
    ...
    private static boolean columnExists(JdbcTemplate jdbc, String schema, String table, String column) {
        Long cnt = jdbc.queryForObject(
                """
                select count(*)
    ...
    private static boolean uniqueConstraintExists(JdbcTemplate jdbc, String schema, String table, String constraintName) {
        Long cnt = jdbc.queryForObject(
                """
                select count(*)
