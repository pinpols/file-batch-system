...
class WorkflowConditionEvaluatorTest {
    ...
    void shouldMatchAndCondition() {
        assertThat(evaluator.matches("status = 'SUCCESS' && code = 'JOB1'",
                "{\"status\":\"SUCCESS\",\"code\":\"JOB1\"}")).isTrue();
        assertThat(evaluator.matches("status = 'SUCCESS' && code = 'JOB1'",
                "{\"status\":\"SUCCESS\",\"code\":\"JOB2\"}")).isFalse();
    }

    @Test
    void shouldMatchOrCondition() {
        assertThat(evaluator.matches("status = 'SUCCESS' || status = 'PARTIAL_FAILED'",
                "{\"status\":\"PARTIAL_FAILED\"}")).isTrue();
        assertThat(evaluator.matches("status = 'SUCCESS' || status = 'PARTIAL_FAILED'",
                "{\"status\":\"FAILED\"}")).isFalse();
    }
    ...
    void shouldMatchInOperator() {
        assertThat(evaluator.matches("status in ['SUCCESS','PARTIAL_FAILED']",
    ...
    void shouldMatchNotInOperator() {
        assertThat(evaluator.matches("status not in ['FAILED','CANCELLED']",
                "{\"status\":\"SUCCESS\"}")).isTrue();
        assertThat(evaluator.matches("status not in ['FAILED','CANCELLED']",
                "{\"status\":\"FAILED\"}")).isFalse();
    }
    ...
    void shouldMatchContainsOnString() {
        assertThat(evaluator.matches("message contains 'error'",
                "{\"message\":\"file parse error occurred\"}")).isTrue();
        assertThat(evaluator.matches("message contains 'error'",
                "{\"message\":\"all good\"}")).isFalse();
    }
    ...
    void shouldMatchEndsWith() {
        assertThat(evaluator.matches("filename endsWith '.csv'", "{\"filename\":\"data.csv\"}")).isTrue();
        assertThat(evaluator.matches("filename endsWith '.csv'", "{\"filename\":\"data.json\"}")).isFalse();
    }
    ...
    void shouldResolveNestedPath() {
        assertThat(evaluator.matches("result.status = 'OK'",
                "{\"result\":{\"status\":\"OK\"}}")).isTrue();
        assertThat(evaluator.matches("result.status = 'OK'",
                "{\"result\":{\"status\":\"FAIL\"}}")).isFalse();
    }
    ...
    void shouldHandleParenthesesGrouping() {
        ...
        assertThat(evaluator.matches(expr, "{\"status\":\"PARTIAL_FAILED\",\"retryCount\":2}")).isTrue();
