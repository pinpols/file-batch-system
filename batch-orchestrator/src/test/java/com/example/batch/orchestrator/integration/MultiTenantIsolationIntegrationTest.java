...
/** 集成测试：多租户数据隔离。 验证租户 t1 的 job instance 和 outbox event 对租户 t2 的查询不可见， 且跨租户种子数据（t2/t3）各自独立加载和访问。 */
@SpringBootTest(
...
class MultiTenantIsolationIntegrationTest extends AbstractIntegrationTest {
  ...
  void t2JobDefinitionsFromSeedAreAccessibleUnderT2Only() {
    Long t2Count =
        jdbcTemplate.queryForObject(
            "select count(*) from batch.job_definition where tenant_id = 't2' and job_code like"
                + " 'T2_%'",
            Long.class);
    Long t1Count =
        jdbcTemplate.queryForObject(
  ...
  void t3JobDefinitionsFromSeedAreAccessibleUnderT3Only() {
    Long t3Count =
        jdbcTemplate.queryForObject(
            "select count(*) from batch.job_definition where tenant_id = 't3' and job_code like"
                + " 'T3_%'",
            Long.class);
    Long t1Count =
        jdbcTemplate.queryForObject(
  ...
  void quotaPoliciesAreScopedPerTenant() {
    Long t2Policies =
        jdbcTemplate.queryForObject(
            "select count(*) from batch.tenant_quota_policy where tenant_id = 't2' and policy_code"
                + " = 'DEFAULT'",
            Long.class);
    Long t3Policies =
        jdbcTemplate.queryForObject(
            "select count(*) from batch.tenant_quota_policy where tenant_id = 't3' and policy_code"
                + " = 'DEFAULT'",
            Long.class);
  ...
  void workerRegistriesAreScopedPerTenant() {
    ...
    List<String> t2WorkerCodes =
        t2Workers.stream().map(r -> (String) r.get("worker_code")).toList();
    List<String> t3WorkerCodes =
        t3Workers.stream().map(r -> (String) r.get("worker_code")).toList();
