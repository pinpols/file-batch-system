...
public final class ImportPreprocessPipeline {
    ...
    public static byte[] run(byte[] input, ImportPayload payload, Map<String, Object> template, boolean testingOpen) {
        try {
            if (input == null) {
                input = new byte[0];
            }
            ...
            for (Map<String, Object> step : steps) {
                ...
                switch (type.toUpperCase(Locale.ROOT)) {
                    ...
                    default -> throw new ImportPreprocessException(
                            "IMPORT_PREPROCESS_UNKNOWN_STEP",
                            "unknown preprocess step type: " + type
                    );
                }
            }
    ...
    private static List<Map<String, Object>> resolveSteps(Map<String, Object> template, boolean testingOpen) {
        ...
        if ("AES".equalsIgnoreCase(enc)) {
            ...
            throw new ImportPreprocessException(
                    ...
                    "encrypt_type " + enc + " is not supported in implicit mode; use preprocess_pipeline or NONE"
            );
        }
        return implicit;
    }
    ...
    private static List<Map<String, Object>> parsePipeline(Object raw) {
        ...
        if (raw instanceof String text && StringUtils.hasText(text)) {
            try {
                ...
                throw new ImportPreprocessException("IMPORT_PREPROCESS_PIPELINE_JSON", "invalid preprocess_pipeline json", ex);
            }
        }
        return List.of();
    }
    ...
    private static byte[] aesGcmDecrypt(byte[] input, Map<String, Object> step, ImportPayload payload) throws Exception {
        Map<String, Object> meta = payload != null ? payload.metadata() : Map.of();
        String keyB64 = firstNonBlank(stringProp(step, "aesKeyBase64"), metaString(meta, "decryptAesKeyBase64"));
        String ivB64 = firstNonBlank(stringProp(step, "aesIvBase64"), metaString(meta, "decryptAesIvBase64"));
        if (!StringUtils.hasText(keyB64) || !StringUtils.hasText(ivB64)) {
            throw new ImportPreprocessException(
                    "IMPORT_PREPROCESS_AES_KEY_MISSING",
                    "AES_GCM_DECRYPT requires aesKeyBase64 and aesIvBase64 on the step or metadata decryptAesKeyBase64 / decryptAesIvBase64"
            );
        }
    ...
    private static void verifyDigest(byte[] input, Map<String, Object> step, ImportPayload payload, Map<String, Object> template) throws Exception {
        String algorithm = firstNonBlank(stringProp(step, "algorithm"), digestAlgorithm(template, payload));
        ...
        if (!actual.equalsIgnoreCase(expected.trim().replace(" ", ""))) {
            throw new ImportPreprocessException(
                    "IMPORT_PREPROCESS_DIGEST_MISMATCH",
                    "digest mismatch for " + algorithm
            );
        }
    }

    private static void verifyImplicitChecksum(byte[] input, ImportPayload payload, Map<String, Object> template) throws Exception {
        ...
        if (!actual.equalsIgnoreCase(expected.trim().replace(" ", ""))) {
            throw new ImportPreprocessException(
                    "IMPORT_PREPROCESS_CHECKSUM_MISMATCH",
                    "checksum mismatch for " + algorithm
            );
        }
    }

    private static String digestAlgorithm(Map<String, Object> template, ImportPayload payload) {
        if (payload != null && StringUtils.hasText(payload.checksumType()) && !"NONE".equalsIgnoreCase(payload.checksumType())) {
    ...
    private static void verifyRsaSha256(byte[] input, Map<String, Object> step, ImportPayload payload) throws Exception {
        String pem = stringProp(step, "publicKeyPem");
        String signatureB64 = firstNonBlank(stringProp(step, "signatureBase64"), metaString(payload == null ? Map.of() : payload.metadata(), "signatureBase64"));
        if (!StringUtils.hasText(pem) || !StringUtils.hasText(signatureB64)) {
            throw new ImportPreprocessException(
                    "IMPORT_PREPROCESS_RSA_CONFIG_MISSING",
                    "VERIFY_RSA_SHA256 requires publicKeyPem and signatureBase64 (step or metadata.signatureBase64)"
            );
        }
        ...
        if (!signature.verify(signBytes)) {
            throw new ImportPreprocessException("IMPORT_PREPROCESS_RSA_VERIFY_FAILED", "RSA signature verification failed");
        }
    }
