...
class ConsoleBusinessCalendarExcelControllerTest {
  ...
  private final ConsoleBusinessCalendarExcelApplicationService excelService =
      mock(ConsoleBusinessCalendarExcelApplicationService.class);
  ...
  void setUp() {
    ...
    mockMvc =
        MockMvcBuilders.standaloneSetup(
                new ConsoleBusinessCalendarExcelController(excelService, responseFactory))
  ...
  void shouldExportTemplateUploadPreviewWorkbookAndApply() throws Exception {
    ...
    when(excelService.exportBusinessCalendars(any()))
        .thenReturn(
    ...
    when(excelService.apply(anyString(), any()))
        .thenReturn(new ConsoleBusinessCalendarExcelApplyResponse("token-1", "t1", 1, 1, 0, 1));

    mockMvc
        .perform(get("/api/console/config/business-calendars/excel/export").param("tenantId", "t1"))
        .andExpect(status().isOk())
        .andExpect(content().bytes(exportBytes));

    mockMvc
        .perform(get("/api/console/config/business-calendars/excel/template"))
        .andExpect(status().isOk())
        .andExpect(content().bytes(templateBytes));

    mockMvc
        .perform(
            multipart("/api/console/config/business-calendars/excel/upload")
                .file(
    ...
    mockMvc
        .perform(get("/api/console/config/business-calendars/excel/preview/token-1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.totalCalendarRows").value(1))
        .andExpect(jsonPath("$.data.totalHolidayRows").value(1));

    mockMvc
        .perform(get("/api/console/config/business-calendars/excel/preview/token-1/workbook"))
        .andExpect(status().isOk())
        .andExpect(content().bytes(workbookBytes));

    mockMvc
        .perform(
            post("/api/console/config/business-calendars/excel/apply/token-1")
