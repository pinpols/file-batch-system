...
class ConsolePipelineDefinitionExcelControllerTest {
  ...
  private final ConsolePipelineDefinitionExcelApplicationService excelService =
      mock(ConsolePipelineDefinitionExcelApplicationService.class);
  ...
  void setUp() {
    ...
    mockMvc =
        MockMvcBuilders.standaloneSetup(
                new ConsolePipelineDefinitionExcelController(excelService, responseFactory))
  ...
  void shouldExportTemplateUploadPreviewWorkbookAndApply() throws Exception {
    ...
    when(excelService.exportPipelineDefinitions(anyString(), any(), any(), any()))
        .thenReturn(
    ...
    when(excelService.upload(any()))
        .thenReturn(
            new ConsolePipelineDefinitionExcelUploadResponse("token-1", "pipeline.xlsx", 1, 2));
    when(excelService.preview(anyString()))
        .thenReturn(
            new ConsolePipelineDefinitionExcelPreviewResponse(
    ...
    when(excelService.apply(anyString(), any()))
        .thenReturn(new ConsolePipelineDefinitionExcelApplyResponse("token-1", "t1", 1, 1, 0, 2));
    ...
    mockMvc
        .perform(get("/api/console/config/pipeline-definitions/excel/template"))
        .andExpect(status().isOk())
        .andExpect(content().bytes(templateBytes));

    mockMvc
        .perform(
            multipart("/api/console/config/pipeline-definitions/excel/upload")
                .file(
    ...
    mockMvc
        .perform(get("/api/console/config/pipeline-definitions/excel/preview/token-1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.totalPipelineRows").value(1))
        .andExpect(jsonPath("$.data.totalStepRows").value(2));

    mockMvc
        .perform(get("/api/console/config/pipeline-definitions/excel/preview/token-1/workbook"))
        .andExpect(status().isOk())
        .andExpect(content().bytes(workbookBytes));

    mockMvc
        .perform(
            post("/api/console/config/pipeline-definitions/excel/apply/token-1")
