...
class ConsoleAlertRoutingExcelControllerTest {
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(new ConsoleAlertRoutingExcelController(excelService, responseFactory))
    ...
    void shouldUploadPreviewAndApplyAlertRoutingExcel() throws Exception {
        when(excelService.upload(any())).thenReturn(new ConsoleAlertRoutingExcelUploadResponse("token-1", "routing.xlsx", "alert_routing_config", 1));
        when(excelService.preview(anyString())).thenReturn(new ConsoleAlertRoutingExcelPreviewResponse(
                "token-1", "routing.xlsx", "alert_routing_config", 1, 1, 0, List.of(), List.of()));
        when(excelService.apply(anyString(), any())).thenReturn(new ConsoleAlertRoutingExcelApplyResponse("token-1", "t1", 1, 1, 0));

        mockMvc.perform(multipart("/api/console/config/alert-routings/excel/upload")
                        .file(new MockMultipartFile("file", "routing.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", new byte[] {1, 2, 3})))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.rowCount").value(1));

        mockMvc.perform(get("/api/console/config/alert-routings/excel/preview/token-1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.totalRows").value(1))
                .andExpect(jsonPath("$.data.validRows").value(1));

        mockMvc.perform(post("/api/console/config/alert-routings/excel/apply/token-1")
    ...
    void shouldDownloadTemplate() throws Exception {
        when(excelService.downloadTemplate()).thenReturn(ResponseEntity.ok()
                .body(new InputStreamResource(new ByteArrayInputStream(new byte[]{1, 2, 3}))));

        mockMvc.perform(get("/api/console/config/alert-routings/excel/template"))
                .andExpect(status().isOk());
    }
}
