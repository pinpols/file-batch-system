...
class ConsoleFileChannelControllerTest {
    ...
    private final ConsoleFileChannelApplicationService applicationService = org.mockito.Mockito.mock(ConsoleFileChannelApplicationService.class);
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(new ConsoleFileChannelController(applicationService, responseFactory))
    ...
    void shouldReturn200WhenGetChannelById() throws Exception {
        when(applicationService.get(anyLong(), anyString())).thenReturn(Map.of("id", 1L, "tenantId", "t1"));

        mockMvc.perform(get("/api/console/file-channels/1").param("tenantId", "t1"))
    ...
    void shouldReturn400WhenCreateRequestMissingRequired() throws Exception {
        mockMvc.perform(post("/api/console/file-channels")
    ...
    void shouldReturn200WhenToggleChannel() throws Exception {
        ...
        mockMvc.perform(post("/api/console/file-channels/1/toggle")
                        .param("tenantId", "t1")
                        .param("enabled", "false"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value("SUCCESS"));
    ...
    @Test
    void shouldReturn200WhenDeleteChannel() throws Exception {
        doNothing().when(applicationService).delete(anyLong(), anyString());

        mockMvc.perform(delete("/api/console/file-channels/1").param("tenantId", "t1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value("SUCCESS"));
    }
    ...
    void shouldReturn200WhenUpdateChannel() throws Exception {
        when(applicationService.update(anyLong(), any())).thenReturn(Map.of("id", 1L, "tenantId", "t1"));

        mockMvc.perform(put("/api/console/file-channels/1")
                        .contentType(APPLICATION_JSON)
                        .content("{\"tenantId\":\"t1\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value("SUCCESS"));
