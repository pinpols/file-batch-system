...
class ConsoleResourceQueueExcelControllerTest {
  ...
  void shouldExportTemplateUploadPreviewWorkbookAndApply() throws Exception {
    ...
    mockMvc
        .perform(
            multipart("/api/console/config/resource-queues/excel/upload")
                .file(
    ...
    mockMvc
        .perform(get("/api/console/config/resource-queues/excel/preview/token-1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.totalRows").value(1))
        .andExpect(jsonPath("$.data.validRows").value(1));

    mockMvc
        .perform(get("/api/console/config/resource-queues/excel/preview/token-1/workbook"))
        .andExpect(status().isOk())
        .andExpect(content().bytes(workbookBytes));

    mockMvc
        .perform(
            post("/api/console/config/resource-queues/excel/apply/token-1")
