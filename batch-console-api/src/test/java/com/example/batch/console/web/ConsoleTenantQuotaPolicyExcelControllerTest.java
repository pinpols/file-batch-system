...
class ConsoleTenantQuotaPolicyExcelControllerTest {
    ...
    private final ConsoleTenantQuotaPolicyExcelApplicationService excelService = mock(ConsoleTenantQuotaPolicyExcelApplicationService.class);
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(new ConsoleTenantQuotaPolicyExcelController(excelService, responseFactory))
    ...
    void shouldExportTemplateUploadPreviewWorkbookAndApply() throws Exception {
        ...
        when(excelService.exportQuotaPolicies(any(), any(), any())).thenReturn(ResponseEntity.ok(new InputStreamResource(new ByteArrayInputStream(exportBytes))));
        ...
        when(excelService.upload(any())).thenReturn(new ConsoleTenantQuotaPolicyExcelUploadResponse("token-1", "quota.xlsx", "tenant_quota_policy", 1));
        when(excelService.preview(anyString())).thenReturn(new ConsoleTenantQuotaPolicyExcelPreviewResponse(
        ...
        when(excelService.apply(anyString(), any())).thenReturn(new ConsoleTenantQuotaPolicyExcelApplyResponse("token-1", "t1", 1, 1, 0));

        mockMvc.perform(get("/api/console/config/quota-policies/excel/export")
                        .param("tenantId", "t1")
                        .param("policyCode", "P1")
                        .param("enabled", "true"))
        ...
        mockMvc.perform(get("/api/console/config/quota-policies/excel/template"))
                .andExpect(status().isOk())
                .andExpect(content().bytes(templateBytes));

        mockMvc.perform(multipart("/api/console/config/quota-policies/excel/upload")
                        .file(new MockMultipartFile("file", "quota.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", new byte[] {1, 2, 3})))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.rowCount").value(1));

        mockMvc.perform(get("/api/console/config/quota-policies/excel/preview/token-1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.totalRows").value(1))
                .andExpect(jsonPath("$.data.validRows").value(1));

        mockMvc.perform(get("/api/console/config/quota-policies/excel/preview/token-1/workbook"))
                .andExpect(status().isOk())
                .andExpect(content().bytes(workbookBytes));

        mockMvc.perform(post("/api/console/config/quota-policies/excel/apply/token-1")
