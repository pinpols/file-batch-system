...
class DefaultConsoleOpsApplicationServiceTest {
    ...
    @Mock
    private OutboxRetryLogMapper outboxRetryLogMapper;

    @Mock
    private OutboxDeliveryLogMapper outboxDeliveryLogMapper;
    ...
    void shouldAggregateOpsSummaryByTenant() {
        ...
        when(jobInstanceMapper.countByStatuses("tenant-a", List.of(JobInstanceStatus.RUNNING.code()))).thenReturn(7L);
        ...
        when(workerRegistryMapper.countByStatus("tenant-a", WorkerRegistryStatus.ONLINE.code())).thenReturn(9L);
        when(workerRegistryMapper.countByStatus("tenant-a", WorkerRegistryStatus.DRAINING.code())).thenReturn(1L);
        when(workerRegistryMapper.countByStatus("tenant-a", WorkerRegistryStatus.OFFLINE.code())).thenReturn(2L);
        when(workerRegistryMapper.countByStatus("tenant-a", WorkerRegistryStatus.DECOMMISSIONED.code())).thenReturn(1L);
        when(outboxRetryLogMapper.countByStatuses("tenant-a", List.of("WAITING", "RUNNING", "FAILED"))).thenReturn(11L);
