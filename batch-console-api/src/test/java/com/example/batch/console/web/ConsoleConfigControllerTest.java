...
class ConsoleConfigControllerTest {
    ...
    private final ConsoleConfigApplicationService configApplicationService = org.mockito.Mockito.mock(ConsoleConfigApplicationService.class);
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(new ConsoleConfigController(configApplicationService, responseFactory))
    ...
    void shouldReturnConfigReleaseListAsDtos() throws Exception {
        when(configApplicationService.configReleases(any())).thenReturn(List.of(
                new ConsoleConfigReleaseResponse(1L, "t1", "FILE", "KEY", "Name", "DRAFT", 1, "{}", "{\"a\":1}",
                        Instant.EPOCH, Instant.EPOCH, null, null, "u1", "u2", Instant.EPOCH, Instant.EPOCH)
        ));

        mockMvc.perform(get("/api/console/config/releases").param("tenantId", "t1"))
    ...
    void shouldPublishConfigRelease() throws Exception {
        ...
        mockMvc.perform(post("/api/console/config/releases/1/publish")
    ...
    void shouldReturnSecretVersionAndChangeLogDtos() throws Exception {
        when(configApplicationService.secretVersions(any())).thenReturn(List.of(
                new ConsoleSecretVersionResponse(1L, "t1", "REF", "Secret", 2, "ACTIVE", true,
                        Instant.EPOCH, Instant.EPOCH, Instant.EPOCH, Instant.EPOCH, "{}", "reason",
                        "u1", "u2", Instant.EPOCH, Instant.EPOCH)
        ));
        when(configApplicationService.configChangeLogs(any())).thenReturn(List.of(
                new ConsoleConfigChangeLogResponse(2L, "t1", "FILE", "KEY", 1, "CREATE", "SUCCESS",
                        "API", "u1", "trace-1", "{}", Instant.EPOCH)
        ));

        mockMvc.perform(get("/api/console/config/secrets").param("tenantId", "t1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[0].secretRef").value("REF"));

        mockMvc.perform(get("/api/console/config/change-logs").param("tenantId", "t1"))
                .andExpect(status().isOk())
