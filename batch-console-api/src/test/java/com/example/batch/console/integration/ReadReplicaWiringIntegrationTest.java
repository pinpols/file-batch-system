...
/**
 * P2-4 smoke IT：覆盖 application-test.yml 默认的 read-replica=false，验证生产默认（true）路径在
 * 完整 Spring 上下文里能正常 wire。replica URL 指向不可达端口触发 fail-open，断言 readOnly 查询
 * 仍然成功（降级到主库）且 {@code batch.console.replica.failover.count} 计数器递增。
 *
 * <p>本类是「prod 默认行为最小覆盖」用：unit 层 {@code ReadReplicaRoutingDataSourceTest} 已用
 * mock DataSource 覆盖九条核心逻辑，本 IT 不重复，只验 Spring 装配 + 计数器对接是真实可用。
 */
@SpringBootTest(
...
class ReadReplicaWiringIntegrationTest extends AbstractIntegrationTest {
  ...
  static void registerReadReplicaPrimary(DynamicPropertyRegistry registry) {
    registry.add("batch.console.read-replica.primary.url", AbstractIntegrationTest::platformJdbcUrl);
  ...
  void wiringInjectsRoutingDataSourceAndFailOpenOnReplicaUnreachable() {
    assertThat(dataSource)
        .as("read-replica.enabled=true 应让 @Primary DataSource 走 LazyConnectionDataSourceProxy 包裹的 routing DS")
