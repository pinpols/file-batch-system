...
class DefaultConsoleWorkflowExcelApplicationServiceTest {
    ...
    private final InMemoryWorkflowExcelImportStore importStore = new InMemoryWorkflowExcelImportStore();
    private DefaultConsoleWorkflowExcelApplicationService service;

    @BeforeEach
    void setUp() {
        service = new DefaultConsoleWorkflowExcelApplicationService(
                tenantGuard,
    ...
    void shouldExportWorkflowMaintenanceWorkbook() throws Exception {
        ...
        ResponseEntity<InputStreamResource> response = service.exportWorkflowExcel(new WorkflowDefinitionQueryRequest());
    ...
    void shouldUploadPreviewAndApplyWorkflowWorkbook() throws Exception {
        ...
        when(workflowDefinitionMapper.selectByUniqueKey("t1", "WF1", 1)).thenReturn(null, savedDefinition);
    ...
    void shouldMarkWorkflowRowsInvalidWhenDefinitionReferenceIsMissing() throws Exception {
        ...
        assertThat(preview.issues().get(0).messages().stream().anyMatch(message -> message.contains("missing definition"))).isTrue();
    }
    ...
    private byte[] buildWorkbook() throws Exception {
        try (XSSFWorkbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            writeRow(definitionSheet, 0, List.of(
                    "tenant_id", "workflow_code", "workflow_name", "workflow_type", "version", "enabled", "description"
            ));
            writeRow(definitionSheet, 1, List.of(
                    "t1", "WF1", "Workflow 1", "DAG", 1, true, "desc"
            ));
            writeRow(nodeSheet, 0, List.of(
                    "tenant_id", "workflow_code", "workflow_version", "node_code", "node_name", "node_type",
                    "related_job_code", "related_pipeline_code", "worker_group", "window_code", "node_order",
                    "retry_policy", "retry_max_count", "timeout_seconds", "node_params", "enabled"
            ));
            writeRow(nodeSheet, 1, List.of(
                    "t1", "WF1", 1, "NODE1", "Node 1", "TASK", "JOB1", "PIPE1", "WG1", "W1", 1,
                    "FIXED", 3, 30, "{\"mode\":\"test\"}", true
            ));
            writeRow(edgeSheet, 0, List.of(
                    "tenant_id", "workflow_code", "workflow_version", "from_node_code", "to_node_code", "edge_type",
                    "condition_expr", "enabled"
            ));
            writeRow(edgeSheet, 1, List.of(
    ...
    private byte[] buildWorkbookWithMissingDefinitionReference() throws Exception {
        try (XSSFWorkbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            writeRow(definitionSheet, 1, List.of(
                    "t1", "WF1", "Workflow 1", "DAG", 2, true, "desc"
            ));
            writeRow(nodeSheet, 0, List.of(
                    "tenant_id", "workflow_code", "workflow_version", "node_code", "node_name", "node_type",
                    "related_job_code", "related_pipeline_code", "worker_group", "window_code", "node_order",
