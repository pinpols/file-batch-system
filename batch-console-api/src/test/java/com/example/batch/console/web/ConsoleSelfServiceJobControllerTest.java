...
class ConsoleSelfServiceJobControllerTest {
    ...
    private final ConsoleSelfServiceJobService selfServiceJobService = org.mockito.Mockito.mock(ConsoleSelfServiceJobService.class);
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(new ConsoleSelfServiceJobController(selfServiceJobService, responseFactory, requestMetadataResolver))
    ...
    void shouldSubmitRerunRequest() throws Exception {
        when(selfServiceJobService.requestRerun(any(ConsoleSelfServiceJobService.RerunParam.class), anyString(), anyString()))
                .thenReturn("APR-20260101-001");

        mockMvc.perform(post("/api/console/self-service/jobs/rerun-request")
    ...
    void shouldSubmitCompensationRequest() throws Exception {
        when(selfServiceJobService.requestCompensation(any(ConsoleSelfServiceJobService.CompensationParam.class), anyString(), anyString()))
                .thenReturn("APR-20260101-002");

        mockMvc.perform(post("/api/console/self-service/jobs/compensation-request")
