...
import com.example.batch.console.support.InMemoryJobDefinitionExcelImportStore;
...
class DefaultConsoleJobDefinitionExcelApplicationServiceTest {
    ...
    @BeforeEach
    void setUp() {
        service = new DefaultConsoleJobDefinitionExcelApplicationService(
                tenantGuard,
    ...
    void shouldExportJobDefinitionsAsWorkbook() throws Exception {
        ...
        ResponseEntity<InputStreamResource> response = service.exportJobDefinitions(new JobDefinitionQueryRequest());
    ...
    private byte[] buildWorkbook(String jobType) throws Exception {
        try (XSSFWorkbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            writeRow(sheet, 0, List.of(
                    "tenant_id", "job_code", "job_name", "job_type", "queue_code", "worker_group",
            ...
            writeRow(sheet, 1, List.of(
                    "t1", "JOB1", "Job 1 updated", jobType, "default", "wg1",
                    "CRON", "0 0 * * *", "cal1", "window1", "FIXED",
                    3, 30, "NONE", "handler", "{\"type\":\"object\"}", "{\"x\":1}", true, "desc updated"
            ));
            workbook.write(out);
