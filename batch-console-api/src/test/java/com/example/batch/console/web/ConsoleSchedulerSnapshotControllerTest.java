...
class ConsoleSchedulerSnapshotControllerTest {
    ...
    private final ConsoleOrchestratorProxyService orchestratorProxyService = org.mockito.Mockito.mock(ConsoleOrchestratorProxyService.class);
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(new ConsoleSchedulerSnapshotController(orchestratorProxyService, responseFactory))
    ...
    void shouldReturn200WhenGetLiveSnapshot() throws Exception {
        when(orchestratorProxyService.schedulerSnapshot(anyString()))
                .thenReturn(new ConsoleSchedulerSnapshotResponse(Instant.now(), "t1", List.of(), List.of(), List.of()));

        mockMvc.perform(get("/api/console/scheduler/snapshot").param("tenantId", "t1"))
    ...
    void shouldReturn200WhenGetSnapshotHistory() throws Exception {
        when(orchestratorProxyService.schedulerSnapshotHistory(anyString(), anyInt())).thenReturn(List.of());

        mockMvc.perform(get("/api/console/scheduler/snapshot/history")
                        .param("tenantId", "t1")
                        .param("limit", "10"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value("SUCCESS"));
