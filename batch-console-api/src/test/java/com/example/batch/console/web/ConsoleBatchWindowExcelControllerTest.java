...
class ConsoleBatchWindowExcelControllerTest {
  ...
  private final ConsoleBatchWindowExcelApplicationService excelService =
      mock(ConsoleBatchWindowExcelApplicationService.class);
  ...
  void setUp() {
    ...
    mockMvc =
        MockMvcBuilders.standaloneSetup(
                new ConsoleBatchWindowExcelController(excelService, responseFactory))
  ...
  void shouldExportTemplateUploadPreviewWorkbookAndApply() throws Exception {
    ...
    when(excelService.exportBatchWindows(any()))
        .thenReturn(
    ...
    mockMvc
        .perform(get("/api/console/config/batch-windows/excel/template"))
        .andExpect(status().isOk())
        .andExpect(content().bytes(templateBytes));

    mockMvc
        .perform(
            multipart("/api/console/config/batch-windows/excel/upload")
                .file(
    ...
    mockMvc
        .perform(get("/api/console/config/batch-windows/excel/preview/token-1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.totalRows").value(1))
        .andExpect(jsonPath("$.data.invalidRows").value(0));

    mockMvc
        .perform(get("/api/console/config/batch-windows/excel/preview/token-1/workbook"))
        .andExpect(status().isOk())
        .andExpect(content().bytes(workbookBytes));

    mockMvc
        .perform(
            post("/api/console/config/batch-windows/excel/apply/token-1")
