...
class ConsoleFileTemplateExcelControllerTest {
    ...
    private final ConsoleFileTemplateExcelApplicationService excelService = org.mockito.Mockito.mock(ConsoleFileTemplateExcelApplicationService.class);
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(new ConsoleFileTemplateExcelController(excelService, responseFactory))
    ...
    void shouldUploadPreviewAndApplyExcelTemplate() throws Exception {
        when(excelService.upload(any())).thenReturn(new ConsoleFileTemplateExcelUploadResponse("token-1", "template.xlsx", "file_template_config", 1));
        when(excelService.preview(anyString())).thenReturn(new ConsoleFileTemplateExcelPreviewResponse(
                "token-1",
                "template.xlsx",
        ...
        mockMvc.perform(post("/api/console/config/file-templates/excel/apply/token-1")
