...
class DefaultConsoleAlertRoutingExcelApplicationServiceTest {
    ...
    private final AlertRoutingConfigMapper alertRoutingConfigMapper = mock(AlertRoutingConfigMapper.class);
    private final ConfigChangeLogMapper configChangeLogMapper = mock(ConfigChangeLogMapper.class);
    private final AlertRoutingExcelImportStore importStore = new InMemoryAlertRoutingExcelImportStore();
    private DefaultConsoleAlertRoutingExcelApplicationService service;

    @BeforeEach
    void setUp() {
        service = new DefaultConsoleAlertRoutingExcelApplicationService(
                tenantGuard,
        ...
        when(requestMetadataResolver.current()).thenReturn(new ConsoleRequestMetadata(
                "req-1", "trace-1", "t1", "u1", "idem-1", "127.0.0.1"));
    ...
    void shouldExportAlertRoutingsAsWorkbook() throws Exception {
        ...
        when(alertRoutingConfigMapper.selectByQuery(any(), any(), any(), any(), any(), any())).thenReturn(List.of(row));

        ResponseEntity<InputStreamResource> response = service.exportAlertRoutings(new AlertRoutingQueryRequest());
    ...
    void shouldUploadPreviewAndApplyRoutingWorkbook() throws Exception {
        ...
        MockMultipartFile file = new MockMultipartFile("file", "routing.xlsx",
