...
class ConsoleJobDefinitionExcelControllerTest {
    ...
    private final ConsoleJobDefinitionExcelApplicationService excelService = org.mockito.Mockito.mock(ConsoleJobDefinitionExcelApplicationService.class);
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(new ConsoleJobDefinitionExcelController(excelService, responseFactory))
    ...
    void shouldExportUploadPreviewAndApplyJobDefinitionExcel() throws Exception {
        ...
        when(excelService.exportJobDefinitions(any())).thenReturn(ResponseEntity.ok(new InputStreamResource(new ByteArrayInputStream(exportBytes))));
        when(excelService.upload(any())).thenReturn(new ConsoleJobDefinitionExcelUploadResponse("token-1", "job.xlsx", 1));
        when(excelService.preview(anyString())).thenReturn(new ConsoleJobDefinitionExcelPreviewResponse(
                "token-1",
                "job.xlsx",
                1,
                1,
                0,
                List.of(new ConsoleJobDefinitionExcelRowResponse(
                        "t1",
                        "JOB1",
                        "Job 1",
                        "GENERAL",
                ...
                List.of(new ConsoleJobDefinitionExcelRowIssueResponse("job_definition", 2, "JOB1", "JOB1", List.of("warn")))
        ));
        when(excelService.apply(anyString(), any())).thenReturn(new ConsoleJobDefinitionExcelApplyResponse("token-1", "t1", 1, 1));

        mockMvc.perform(get("/api/console/config/job-definitions/excel/export").param("tenantId", "t1"))
                .andExpect(status().isOk())
                .andExpect(content().bytes(exportBytes));

        mockMvc.perform(multipart("/api/console/config/job-definitions/excel/upload")
                        .file(new MockMultipartFile("file", "job.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", new byte[] {1, 2, 3})))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.rowCount").value(1));

        mockMvc.perform(get("/api/console/config/job-definitions/excel/preview/token-1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.totalRows").value(1))
                .andExpect(jsonPath("$.data.rows[0].jobCode").value("JOB1"));

        mockMvc.perform(post("/api/console/config/job-definitions/excel/apply/token-1")
