...
public interface ConsoleWebhookSubscriptionRepository extends Repository<WebhookSubscriptionEntity, Long> {
    ...
    @Query("""
            SELECT id, tenant_id, name, callback_url, event_types, secret, enabled,
             ...
             LIMIT 1
            """)
    Optional<WebhookSubscriptionEntity> findByTenantAndId(@Param("tenantId") String tenantId,
                                                           @Param("id") Long id);
    ...
    @Query("""
            UPDATE batch.webhook_subscription
             ...
             WHERE tenant_id = :tenantId AND id = :id
            """)
    void update(@Param("tenantId") String tenantId,
                @Param("id") Long id,
                @Param("callbackUrl") String callbackUrl,
                @Param("eventTypes") String eventTypes,
                @Param("secret") String secret,
