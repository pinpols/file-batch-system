 ...
 *
 * <p>使用 Lua 脚本保证"计数检查 + 写入"的原子性，消除并发竞态。 每个限流 key 对应一个 Sorted Set：score = 请求时间戳（ms），member = 唯一标识。
 *
 * <p>算法：
 *
 * <ol>
 *   <li>移除窗口外的旧成员（{@code ZREMRANGEBYSCORE key 0 now-window}）
 *   <li>计数当前窗口内成员数（{@code ZCARD key}）
 *   <li>未超限则写入本次请求记录（{@code ZADD key now member}）并刷新 TTL
 *   <li>返回 1（允许）或 0（超限）
 * </ol>
 *
 * <p>相比 INCR+EXPIRE（固定窗口），滑动窗口不存在边界突破问题： 任意时刻往回看 60 秒，窗口内永远最多 {@code limit} 次。
 */
@Component
