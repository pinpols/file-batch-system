 ...
 *
 * <p><b>R-4.8 · 缓存失效 AOP 化</b>：
 * 旧模式是业务代码手动调 {@code ConsoleConfigCacheInvalidationService.evictXxx(...)}，
 * 写路径漏调就静默留下陈旧缓存。本注解 + {@link ConsoleCacheInvalidationAspect} 把失效
 * 逻辑收到 AOP 切面：方法返回后自动调对应 evict，降低漏掉概率。
 *
 * <p><b>使用示例</b>：
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>{@code tenantSpel} / {@code codeSpel} 支持 SpEL 表达式引用方法入参（按 index 或名字）；
 * 不填则表示 target 语义上是"全租户清"（如 {@link Target#ALL_JOB_DEFINITIONS_BY_TENANT}，
 * 只需要 tenantSpel）。
 *
 * <p>配合守护测试 {@code ConsoleCacheInvalidationCoverageTest} 扫描 batch-console-api 的
 * write-path service 方法，确认凡是动 job_definition / workflow_definition / business_calendar /
 * batch_window / file_channel_config / tenant_quota_policy 等受缓存表的方法都加了本注解
 * （或手动调 evict）。
 */
...
public @interface InvalidatesConsoleCache {
  ...
  /**
   * 缓存目标分类；对应 {@link com.example.batch.console.infrastructure.ConsoleConfigCacheInvalidationService}
   * 的 evictXxx 方法族。新增目标前先在 Invalidation Service 补齐对应 evict 方法。
   */
  enum Target {
