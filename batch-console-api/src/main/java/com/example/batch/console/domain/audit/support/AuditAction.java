...
public @interface AuditAction {
   ...
   *
   * <p><b>用途:</b>ROLE_ADMIN 跨租操作(如 {@code ConsoleTenantController.update(tenantId, ...)})的
   * {@link com.example.batch.console.domain.rbac.support.ConsolePrincipal#tenantId()} 为 null,
   * 默认会兜底到 {@code "system"},导致审计行 tenant_id="system",取证按目标租户查询时漏掉这些行。
   * 显式指定 {@code targetTenantParam} 后,审计行的 tenant_id 取该入参值,而非 principal 的租户。
   *
