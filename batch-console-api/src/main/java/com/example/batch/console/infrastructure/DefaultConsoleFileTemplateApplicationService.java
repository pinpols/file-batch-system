...
public class DefaultConsoleFileTemplateApplicationService
    ...
    public Map<String, Object> get(Long id, String tenantId) {
        ...
        return Guard.requireFound(
                mapper.selectById(resolved, id), "file template not found: " + id);
    }
    ...
    public Map<String, Object> create(FileTemplateCreateRequest request) {
        ...
        if (existing != null) {
            throw new BizException(
                    ResultCode.CONFLICT,
                    "template code + version already exists: "
                            + request.getTemplateCode()
                            + "/"
                            + version);
        }
    ...
    public Map<String, Object> update(Long id, FileTemplateUpdateRequest request) {
        ...
        Map<String, Object> existing =
                Guard.requireFound(
                        mapper.selectById(tenantId, id), "file template not found: " + id);
        String operator = requestMetadataResolver.current().operatorId();
        String templateCode = (String) existing.get("template_code");
        int version =
                request.getVersion() != null
                        ? request.getVersion()
                        : existing.get("version") != null
                                ? ((Number) existing.get("version")).intValue()
                                : 1;
