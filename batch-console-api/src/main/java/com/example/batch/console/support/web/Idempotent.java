 ...
 *
 * <p>{@link ConsoleIdempotencyInterceptor} 对 {@code /api/console/**} 下的写请求（POST / PUT / PATCH /
 * DELETE）都会读取该头，有则去重、没有则放行（opt-in）。加 {@code @Idempotent} 的方法转为 <b>fail-close</b>：缺失 header 直接返回
 * 400，避免客户端遗漏导致"首次成功第二次又成功"的重复执行。
 *
 * <p>适用场景：副作用不可回滚的写接口（如手工补偿、强制下线、死信重放、触发重新执行等）。
 *
