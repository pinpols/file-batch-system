...
public class ConsoleRealtimeReplayStore {
    ...
    public void append(ConsoleRealtimeStreamEnvelope envelope) {
        if (envelope == null || envelope.tenantId() == null || envelope.stream() == null || envelope.cursor() == null) {
            return;
        }
        ...
        long ttlSeconds = realtimeProperties.getReplayTtl() != null && !realtimeProperties.getReplayTtl().isZero()
                && !realtimeProperties.getReplayTtl().isNegative()
                ? realtimeProperties.getReplayTtl().getSeconds() : 0L;
    ...
    public ReplayBatch replay(String tenantId, String stream, String afterCursor, String eventType) {
        if (tenantId == null || tenantId.isBlank() || stream == null || stream.isBlank() || afterCursor == null || afterCursor.isBlank()) {
        ...
        for (String raw : rawEntries) {
            try {
                ConsoleRealtimeStreamEnvelope envelope = JsonUtils.fromJson(raw, ConsoleRealtimeStreamEnvelope.class);
                if (envelope != null) {
                    envelopes.add(envelope);
                }
            } catch (Exception exception) {
                log.warn("console realtime replay buffer decode failed: tenantId={}, stream={}, reason={}",
                        tenantId, stream, exception.getMessage(), exception);
