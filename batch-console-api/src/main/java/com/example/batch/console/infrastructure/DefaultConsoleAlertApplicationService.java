...
/**
 * {@link ConsoleAlertApplicationService} 的默认实现：
 * 仅执行告警状态流转，不引入额外的告警事件表。
 */
@Service
...
public class DefaultConsoleAlertApplicationService implements ConsoleAlertApplicationService {
    ...
    @Override
    @Transactional
    public ConsoleAlertActionResponse ack(Long alertId, AlertActionRequest request, String idempotencyKey) {
        return transition(alertId, request, STATUS_ACKED, "ack");
    }

    @Override
    @Transactional
    public ConsoleAlertActionResponse silence(Long alertId, AlertActionRequest request, String idempotencyKey) {
        return transition(alertId, request, STATUS_SUPPRESSED, "silence");
    }

    @Override
    @Transactional
    public ConsoleAlertActionResponse close(Long alertId, AlertActionRequest request, String idempotencyKey) {
        return transition(alertId, request, STATUS_CLOSED, "close");
    }

    private ConsoleAlertActionResponse transition(Long alertId,
        ...
        if (entity == null) {
            throw new BizException(ResultCode.NOT_FOUND, "alert event not found");
        }
        ...
        if (STATUS_CLOSED.equals(currentStatus) && !STATUS_CLOSED.equals(nextStatus)) {
            throw new BizException(ResultCode.STATE_CONFLICT, "closed alert cannot be reopened by this action");
        }
