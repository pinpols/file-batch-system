...
public class ConsoleCalendarController {
    ...
    public CommonResponse<PageResponse<Map<String, Object>>> list(
        ...
        return responseFactory.success(calendarApplicationService.list(tenantId, calendarCode, enabled, pageNo, pageSize));
    }

    @PostMapping
    public CommonResponse<Map<String, Object>> create(@Valid @RequestBody CalendarSaveRequest request) {
        return responseFactory.success(calendarApplicationService.create(request));
    }

    @PutMapping("/{id}")
    public CommonResponse<Map<String, Object>> update(@PathVariable Long id,
    ...
    @PreAuthorize("hasAnyAuthority('ROLE_ADMIN', 'ROLE_AUDITOR', 'ROLE_CONFIG_ADMIN')")
    public CommonResponse<List<Map<String, Object>>> holidays(@PathVariable Long id,
                                                              @RequestParam("tenantId") String tenantId) {
        return responseFactory.success(calendarApplicationService.holidays(id, tenantId));
    }

    @PostMapping("/{id}/holidays")
    public CommonResponse<Void> importHolidays(@PathVariable Long id,
    ...
    public CommonResponse<Map<String, Object>> updateHoliday(@PathVariable Long id,
                                                             @PathVariable Long holidayId,
                                                             @Valid @RequestBody HolidaySaveRequest request) {
        return responseFactory.success(calendarApplicationService.updateHoliday(id, holidayId, request));
    }

    @DeleteMapping("/{id}/holidays/{holidayId}")
    public CommonResponse<Void> deleteHoliday(@PathVariable Long id,
                                              @PathVariable Long holidayId,
                                              @RequestParam("tenantId") String tenantId) {
