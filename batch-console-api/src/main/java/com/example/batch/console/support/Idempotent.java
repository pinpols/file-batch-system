 ...
 * 有则去重、没有则放行（opt-in）。加 {@code @Idempotent} 的方法转为 <b>fail-close</b>：缺失 header
 * 直接返回 400，避免客户端遗漏导致"首次成功第二次又成功"的重复执行。
 *
 * <p>适用场景：副作用不可回滚的 POST（如手工补偿、强制下线、死信重放、触发重新执行等）。
 *
 * <p>参考：<a href="https://stripe.com/docs/api/idempotent_requests">Stripe Idempotent Requests</a> 语义。
 */
