...
public class ConsoleClusterDiagnosticService {
    ...
    public Map<String, Object> shedLockStatus(String tenantId) {
        ...
        List<Map<String, Object>> locks = jdbcTemplate.queryForList(
                "SELECT name, lock_until, locked_at, locked_by FROM batch.shedlock ORDER BY name");
        long activeLocks = locks.stream()
                .filter(row -> row.get("lock_until") != null)
                .count();
        Map<String, Object> result = new LinkedHashMap<>();
    ...
    public Map<String, Object> workerConsistency(String tenantId) {
        ...
        long draining = workerRegistryMapper.countByStatus(resolved, WorkerRegistryStatus.DRAINING.code());
        long offline = workerRegistryMapper.countByStatus(resolved, WorkerRegistryStatus.OFFLINE.code());
        long running = jobInstanceMapper.countByStatuses(resolved, List.of(JobInstanceStatus.RUNNING.code()));
        Map<String, Object> result = new LinkedHashMap<>();
    ...
    public Map<String, Object> outboxHealth(String tenantId) {
        ...
        long pendingCount = jdbcTemplate.queryForObject(
                "SELECT count(*) FROM batch.outbox_event WHERE tenant_id = ? AND status = 'PENDING'",
                Long.class, resolved);
        Map<String, Object> result = new LinkedHashMap<>();
