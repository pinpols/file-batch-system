...
public class DefaultConsoleBatchWindowApplicationService implements ConsoleBatchWindowApplicationService {
    ...
    public PageResponse<Map<String, Object>> list(String tenantId, String windowCode, Boolean enabled,
        ...
        List<Map<String, Object>> items = batchWindowMapper.selectByQuery(resolved, windowCode, enabled, pageRequest);
    ...
    public Map<String, Object> create(BatchWindowCreateRequest request) {
        ...
        Map<String, Object> existing = batchWindowMapper.selectByUniqueKey(tenantId, request.getWindowCode());
        if (existing != null) {
            throw new BizException(ResultCode.CONFLICT, "window code already exists: " + request.getWindowCode());
        }
        ...
        params.put("end_strategy", request.getEndStrategy() != null ? request.getEndStrategy() : "FINISH_RUNNING");
        params.put("out_of_window_action", request.getOutOfWindowAction() != null ? request.getOutOfWindowAction() : "WAIT");
        params.put("allow_cross_day", request.getAllowCrossDay() != null ? request.getAllowCrossDay() : false);
    ...
    public Map<String, Object> update(Long id, BatchWindowUpdateRequest request) {
        String tenantId = tenantGuard.resolveTenant(request.getTenantId());
        Map<String, Object> existing = batchWindowMapper.selectById(tenantId, id);
        if (existing == null) {
        ...
        params.put("tenant_id", tenantId);
        params.put("id", id);
        params.put("window_name", request.getWindowName() != null ? request.getWindowName() : existing.get("window_name"));
        params.put("timezone", request.getTimezone() != null ? request.getTimezone() : existing.get("timezone"));
        params.put("start_time", request.getStartTime() != null ? request.getStartTime() : String.valueOf(existing.get("start_time")));
        params.put("end_time", request.getEndTime() != null ? request.getEndTime() : String.valueOf(existing.get("end_time")));
        params.put("end_strategy", request.getEndStrategy() != null ? request.getEndStrategy() : existing.get("end_strategy"));
        params.put("out_of_window_action", request.getOutOfWindowAction() != null ? request.getOutOfWindowAction() : existing.get("out_of_window_action"));
        params.put("allow_cross_day", request.getAllowCrossDay() != null ? request.getAllowCrossDay() : existing.get("allow_cross_day"));
