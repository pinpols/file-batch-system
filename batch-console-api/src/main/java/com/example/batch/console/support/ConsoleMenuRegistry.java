 ...
 *
 * <p>菜单可见性由 {@code minRole} 控制，角色等级：VIEWER &lt; OPERATOR &lt; ADMIN。
 * 后端 authorities 映射规则：
 * <ul>
 *   <li>ROLE_ADMIN → ADMIN
...
public final class ConsoleMenuRegistry {
  ...
  public record MenuGroup(String key, String title, String icon, String minRole,
                          List<MenuItem> children) {}

  private static final List<MenuGroup> ALL_GROUPS = List.of(
      new MenuGroup("ops", "运营概览", "Histogram", "VIEWER", List.of(
      ...
      new MenuGroup("config", "配置与发布", "Operation", "OPERATOR", List.of(
      ...
      new MenuGroup("files", "文件中心", "FolderOpened", "VIEWER", List.of(
      ...
      new MenuGroup("jobs", "任务管理", "Management", "VIEWER", List.of(
      ...
      new MenuGroup("monitor", "执行与观测", "Monitor", "VIEWER", List.of(
      ...
      new MenuGroup("scheduler", "调度与治理", "DataAnalysis", "VIEWER", List.of(
      ...
      new MenuGroup("system", "系统", "Setting", "OPERATOR", List.of(
  ...
  public static List<MenuGroup> filterByAuthorities(Set<String> authorities) {
    ...
    for (MenuGroup group : ALL_GROUPS) {
      ...
      if (!visibleItems.isEmpty()) {
        result.add(new MenuGroup(group.key(), group.title(), group.icon(),
            group.minRole(), visibleItems));
      }
    }
    return result;
  }
