...
public class DefaultConsoleResourceQueueApplicationService implements ConsoleResourceQueueApplicationService {

    private final ResourceQueueMapper resourceQueueMapper;
    private final ConsoleTenantGuard tenantGuard;

    @Override
    public PageResponse<Map<String, Object>> list(String tenantId, String queueCode, String queueType,
        ...
        List<Map<String, Object>> items = resourceQueueMapper.selectByQuery(resolved, queueCode, queueType, enabled, pageRequest);
    ...
    public Map<String, Object> create(ResourceQueueCreateRequest request) {
        ...
        Map<String, Object> existing = resourceQueueMapper.selectByUniqueKey(tenantId, request.getQueueCode());
        if (existing != null) {
            throw new BizException(ResultCode.CONFLICT, "queue code already exists: " + request.getQueueCode());
        }
        ...
        params.put("max_running_partitions", request.getMaxRunningPartitions() != null ? request.getMaxRunningPartitions() : 0);
        ...
        params.put("priority_policy", request.getPriorityPolicy() != null ? request.getPriorityPolicy() : "FIFO");
        params.put("fair_share_weight", request.getFairShareWeight() != null ? request.getFairShareWeight() : 1);
    ...
    public Map<String, Object> update(Long id, ResourceQueueUpdateRequest request) {
        String tenantId = tenantGuard.resolveTenant(request.getTenantId());
        Map<String, Object> existing = resourceQueueMapper.selectById(tenantId, id);
        if (existing == null) {
        ...
        params.put("tenant_id", tenantId);
        params.put("id", id);
        params.put("queue_name", request.getQueueName() != null ? request.getQueueName() : existing.get("queue_name"));
        params.put("queue_type", request.getQueueType() != null ? request.getQueueType() : existing.get("queue_type"));
        params.put("max_running_jobs", request.getMaxRunningJobs() != null ? request.getMaxRunningJobs() : existing.get("max_running_jobs"));
        params.put("max_running_partitions", request.getMaxRunningPartitions() != null ? request.getMaxRunningPartitions() : existing.get("max_running_partitions"));
        params.put("max_qps", request.getMaxQps() != null ? request.getMaxQps() : existing.get("max_qps"));
        params.put("worker_group", request.getWorkerGroup() != null ? request.getWorkerGroup() : existing.get("worker_group"));
        params.put("resource_tag", request.getResourceTag() != null ? request.getResourceTag() : existing.get("resource_tag"));
        params.put("priority_policy", request.getPriorityPolicy() != null ? request.getPriorityPolicy() : existing.get("priority_policy"));
        params.put("fair_share_weight", request.getFairShareWeight() != null ? request.getFairShareWeight() : existing.get("fair_share_weight"));
