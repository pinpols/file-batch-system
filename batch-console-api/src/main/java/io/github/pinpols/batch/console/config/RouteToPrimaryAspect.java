 ...
 *
 * <p>{@link Order} 设为 {@code Ordered.HIGHEST_PRECEDENCE + 10}，**优先于** Spring 的 {@code
 * TransactionInterceptor}（默认 {@code Ordered.LOWEST_PRECEDENCE}）。这样 ThreadLocal hint 在
 * {@code @Transactional} 真正开事务前已经设好，{@link ReadReplicaRoutingDataSource} 在 {@code
 * determineCurrentLookupKey} 第一时间就能读到。
 *
...
public class RouteToPrimaryAspect {
  ...
  @Around("@annotation(io.github.pinpols.batch.console.config.RouteToPrimary)")
