# sample-tenant-worker-java — ADR-035 Phase 1.4 示范

租户自托管 Worker 最小示范,集成 `batch-worker-sdk`。业务方按此模板复制到自己 repo,把 handler 换成业务实现即可。

> **同一自托管能力的 4 种接入,按租户技术栈选**:
> - **`sample-tenant-worker-java`(本目录)** — 纯 Java + 手写 `main` wiring
> - [`../sample-tenant-worker-java-spring`](../sample-tenant-worker-java-spring/) — Java + Spring Boot starter(自动装配)
