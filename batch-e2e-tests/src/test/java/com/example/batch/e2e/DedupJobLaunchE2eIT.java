 ...
 *
 * <p>测试意图：验证“应用层去重查询 + DB 唯一约束”这两道闸门能共同保证：
 * 对于同一个 {@code (tenant_id, dedup_key)}，系统最终只会存在一条 {@code job_instance}。
 * 这能抵御“上游重放/网络重试/并发重复请求”等真实生产场景。
 *
 * <p>覆盖场景：
 * <ol>
 *   <li><b>串行重复</b>：第二次 launch 使用不同 request_id 但相同 dedup_key，必须被识别为 DUPLICATE。</li>
 *   <li><b>并发重复</b>：两个线程在 {@link CountDownLatch} 栅栏后同时 launch，相同 dedup_key 下最终只落一条 instance。</li>
 * </ol>
 */
@SpringBootTest(
...
class DedupJobLaunchE2eIT extends AbstractIntegrationTest {
    ...
    void sequentialDuplicateLaunchIsIdempotent() {
        ...
        // Insert a second trigger_request that shares the same dedup_key but uses a different request_id
        ...
        launchService.launch(new LaunchRequest(
                TENANT, seed.jobCode(), LocalDate.of(2026, 1, 15),
                TriggerType.API, seed.requestId(), "e2e-tr-dedup-seq-1", Map.of()));

        // Second launch with same dedup_key (different request_id) — must be treated as duplicate
        launchService.launch(new LaunchRequest(
                TENANT, seed.jobCode(), LocalDate.of(2026, 1, 15),
                TriggerType.API, requestId2, "e2e-tr-dedup-seq-2", Map.of()));
     ...
     * CountDownLatch barrier.  Regardless of which thread wins the insert, exactly one
     * {@code job_instance} row must exist.
     */
    @Test
        ...
        try {
            Future<?> f1 = executor.submit(() -> {
                ...
                try {
                    launchService.launch(new LaunchRequest(
                            TENANT, seed.jobCode(), LocalDate.of(2026, 1, 15),
                            TriggerType.API, seed.requestId(), "e2e-tr-dedup-con-1", Map.of()));
            ...
            Future<?> f2 = executor.submit(() -> {
                ...
                try {
                    launchService.launch(new LaunchRequest(
                            TENANT, seed.jobCode(), LocalDate.of(2026, 1, 15),
                            TriggerType.API, requestId2, "e2e-tr-dedup-con-2", Map.of()));
                    successCount.incrementAndGet();
                } catch (Exception e) {
                    errorCount.incrementAndGet();
                }
