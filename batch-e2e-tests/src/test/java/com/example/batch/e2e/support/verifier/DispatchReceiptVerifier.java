 ...
 * <ol>
 *   <li><b>File status</b>: {@code file_record.file_status} has transitioned to the expected
 *       value (default: {@code DISPATCHED})</li>
 *   <li><b>Receipt record</b>: a {@code file_dispatch_record} row exists with the expected
 *       {@code receipt_code} and matches the {@code channel_code} when configured</li>
 *   <li><b>Audit log</b>: at least N {@code file_audit_log} entries exist for this file</li>
 * </ol>
 *
...
public final class DispatchReceiptVerifier implements E2eVerifier {
    ...
    private void verifyFileStatus() {
        String status = platformJdbc.queryForObject(
                ...
                fileId);
        assertThat(status)
                .as("dispatch: file_record.file_status must be '%s' for file id=%d", expectedFileStatus, fileId)
    ...
    private void verifyAuditLog() {
        ...
        assertThat(auditCount)
                .as("dispatch: file_audit_log must have at least %d entries for fileId=%d",
