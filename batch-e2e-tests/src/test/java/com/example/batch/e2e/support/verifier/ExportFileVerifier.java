 ...
 * <ol>
 *   <li><b>File record</b>: {@code file_record} row has {@code storage_path} set and
 *       {@code file_status = 'GENERATED'} (or 'COMPLETED')</li>
 *   <li><b>Amount rollup</b>: {@code settlement_batch.total_amount} meets the expected minimum</li>
 ...
 * The MinIO content check is silently skipped for LOCAL-type storage paths.
 */
    ...
    private void tryAssertMinioContent(String storagePath) {
        ...
        try {
            ...
            try (var stream = client.getObject(
                ...
                assertThat(lines)
                        .as("export: MinIO file must be non-empty")
                        .isNotEmpty();
