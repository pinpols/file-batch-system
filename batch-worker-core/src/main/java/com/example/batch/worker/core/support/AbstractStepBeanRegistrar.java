 ...
 *
 * <p>采用"快照覆盖"策略：先按 module 清空，再一次性 INSERT 当前集合。这样已删除的类在下一次
 * 启动后自动从表里消失；多实例并发启动也只会生成相同内容的覆盖，最终一致。
 *
 * <p>子类只需指定 module 名（{@code IMPORT / EXPORT / DISPATCH}）与 Step bean 类型。console-api 的
 * Excel 上传校验会按 pipeline 的 worker_type 查本表，拦住 {@code impl_code} 指向未注册 Spring bean
...
public abstract class AbstractStepBeanRegistrar<T> {
  ...
  /**
   * REQUIRES_NEW 独立事务：本段登记失败不影响应用正常启动继续处理其他监听器；登记失败时 console-api
   * 的 Excel 校验会退化为不校验 impl_code 白名单（与修复前一致，保持兼容），运维收到 ERROR 日志即可定位。
   */
