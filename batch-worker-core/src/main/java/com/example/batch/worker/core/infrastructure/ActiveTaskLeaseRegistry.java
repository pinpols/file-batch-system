...
public class ActiveTaskLeaseRegistry {
     ...
     * <p>语义约束：
     * <ul>
     *   <li>只用于优雅停机阶段：停止拉取新任务后，等待已有任务自然结束</li>
     *   <li>不抛异常：避免 shutdown hook 因异常导致更糟的退出路径</li>
     *   <li>超时返回：保证不会无限阻塞进程退出</li>
     * </ul>
     */
    public void awaitDrain(Duration timeout) {
        ...
        if (!snapshot().isEmpty()) {
            log.warn("awaitDrain timeout; remainingLeases={}, timeoutMs={}", snapshot().size(), effective.toMillis());
        }
    }
