 ...
 * Executors.newSingleThreadScheduledExecutor} 提升为 Spring 管理的 {@code ThreadPoolTaskScheduler},
 * 与 {@code batch-common} 统一调度治理 (Actuator 指标、线程命名、生命周期、拒绝策略) 对齐。
 *
 * <p>注意: 这里**不复用** {@code batch-common} 的 {@code taskScheduler} bean (架构硬约束: 禁覆盖
 * batch-common 基础设施 bean), 而是注册一个专用的 {@code workerWatchdogScheduler} 供 watchdog 使用。
 */
@Configuration
