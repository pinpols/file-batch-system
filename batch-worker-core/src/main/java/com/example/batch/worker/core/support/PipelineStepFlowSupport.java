...
public final class PipelineStepFlowSupport {
    ...
    public static PipelineStepDefinition resolveNextStep(PipelineStepDefinition current,
                                                         boolean success,
        ...
        if (!StringUtils.hasText(selector) && current != null) {
            selector = success
                    ? current.textParam("onSuccessNextStepCode", "onSuccessNextStageCode", "nextStepCode", "nextStageCode")
    ...
    private static PipelineStepDefinition findByStepCodeOrStageCode(List<PipelineStepDefinition> steps, String selector) {
        if (steps == null || !StringUtils.hasText(selector)) {
            return null;
        }
        for (PipelineStepDefinition step : steps) {
            if (selector.equalsIgnoreCase(step.stepCode()) || selector.equalsIgnoreCase(step.stageCode())) {
                return step;
            }
        }
        return null;
