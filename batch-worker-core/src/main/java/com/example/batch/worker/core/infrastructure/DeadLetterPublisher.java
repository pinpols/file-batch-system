...
/**
 * D-3: 死信队列发布器，将处理失败的消息转发到 {@link BatchTopics#TASK_DEAD_LETTER}，
 * 防止毒丸消息阻塞主派发队列。运维可从 DLQ 查看并重放。
 */
@Slf4j
@Component
public class DeadLetterPublisher {
    ...
    /** 将失败消息发布到死信 topic。 */
    public void publish(String originalPayload, String sourceTopic, String workerType, String errorMessage) {
        try {
            ...
            log.error("failed to publish to DLQ: sourceTopic={}, workerType={}, dlqError={}",
                    sourceTopic, workerType, ex.getMessage(), ex);
        }
    }
