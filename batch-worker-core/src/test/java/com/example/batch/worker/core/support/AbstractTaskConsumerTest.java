...
/**
 * Tests for AbstractTaskConsumer covering:
 * - malformed message dropped silently
 * - worker type mismatch → message skipped without execution
 * - selectedWorkerId mismatch → message skipped
 * - executor returns null (CLAIM race lost) → treated as skip
 * - executor throws exception → DLQ published, returns true (no requeue)
 * - executor throws exception with no DLQ → still returns true
 * - MDC fields cleared after processing
 * - topics() resolution logic
 */
class AbstractTaskConsumerTest {
    ...
    void doConsume_dropsMessageWhenTaskIdMissing() {
        ...
        boolean result = (boolean) ReflectionTestUtils.invokeMethod(consumer, "doConsume", JsonUtils.toJson(msg));
    ...
    void doConsume_skipsWhenSelectedWorkerIdDoesNotMatch() {
        ...
        TaskDispatchMessage msg = new TaskDispatchMessage(
                "v1", "t1", 1L, null, 1L, null, null, "EXECUTION", 1,
                "IMPORT", "worker-B", null, null, "{}", "tr", "k", null);
        boolean result = (boolean) ReflectionTestUtils.invokeMethod(consumer, "doConsume", JsonUtils.toJson(msg));
    ...
    void doConsume_executesWhenSelectedWorkerIdMatches() {
        ...
        TaskDispatchMessage msg = new TaskDispatchMessage(
                "v1", "t1", 1L, null, 1L, null, null, "EXECUTION", 1,
                "IMPORT", "worker-A", null, null, "{}", "tr", "k", null);
    ...
    void doConsume_treatsNullExecutorResultAsSkip() {
        ...
        boolean result = (boolean) ReflectionTestUtils.invokeMethod(consumer, "doConsume", buildImportMessage());
    ...
    void topics_returnsBothBaseAndDirectTopicWhenWorkerCodePresent() {
        AbstractTaskConsumer consumer = buildConsumer("IMPORT", mock(TaskDispatchExecutor.class), null, "w1");
        String[] topics = consumer.topics();
    ...
    private AbstractTaskConsumer buildConsumer(String workerType, TaskDispatchExecutor executor,
                                               DeadLetterPublisher dlq) {
        return buildConsumer(workerType, executor, dlq, null);
    }

    private AbstractTaskConsumer buildConsumer(String workerType, TaskDispatchExecutor executorArg,
                                               DeadLetterPublisher dlqArg, String workerCode) {
    ...
    private TaskDispatchMessage buildMessage(Long taskId, String tenantId, String workerType, String selectedWorkerId) {
        // field order: schemaVersion, tenantId, jobInstanceId, jobPartitionId, taskId, instanceNo, jobCode,
        ...
        return new TaskDispatchMessage(
                "v1", tenantId, 1L, null, taskId, null, null, "EXECUTION", 1,
                workerType, selectedWorkerId, null, null, "{}", "tr", "k", null);
    }
}
