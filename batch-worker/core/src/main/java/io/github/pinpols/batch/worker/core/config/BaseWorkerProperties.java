 ...
 *
 * <p>各 worker 模块的 {@code @ConfigurationProperties} record 需保持自有类型以绑定独立前缀 （{@code
 * batch.worker.process} / {@code batch.worker.dispatch} 等），但字段与 capabilityTags 的 null →
 * 空列表规范化逻辑完全相同。把规范化静态化为 {@link #normalizeCapabilityTags(List)} 后， 各模块的 compact constructor
 * 一行调用即可，避免每个 record 写一遍同样的 override。
 *
 * <p>新增 worker 类型时可直接用本 record 作为 {@code @ConfigurationProperties} 目标（命名上若不需要 与模块强绑定），免去再 copy
 * 一份字段列表。
 */
