...
| 2026-04-30 | **ADR-010 trigger 异步路径**:新增内部 Kafka topic `batch.trigger.launch.v1`(版本化,key=`tenantId:requestId`,value=`LaunchEnvelope` JSON,headers `X-Trace-Id`/`X-Tenant-Id`/`X-Envelope-Version`)。trigger → orchestrator 调度链可由 `batch.trigger.async-launch.enabled=true` 切到 outbox+Kafka 异步模式,原 `POST /internal/orchestrator/launch` HTTP 端点保留向后兼容(`HttpOrchestratorTriggerAdapter` 标 `@Deprecated forRemoval=true`,等灰度全量切稳定 1 minor 后物理删除) |
