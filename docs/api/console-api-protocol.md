...
| 日期       | 变更摘要                                                                                                                                      |
|------------|-----------------------------------------------------------------------------------------------------------------------------------------------|
| 2026-07-11 | **修正 `AiChatResponse` schema 与实现脱节**:`POST /api/console/ai/chat` 的响应 schema 之前写的 `conversationId/model/promptTokens/completionTokens` 后端从不返回,而真实返回的 `sessionId/promptCategory/promptDecision/modelName/refusalReason` 又缺失。现按 Java DTO `AiChatResponse` 校正为 8 字段:`requestId/traceId/sessionId/promptCategory/promptDecision/modelName/answer/refusalReason`;`promptCategory`(PLATFORM/FILE_GOVERNANCE/WORKFLOW/OPERATIONS/OUT_OF_SCOPE)与 `promptDecision`(APPROVED/REJECTED_SCOPE/REJECTED_AUTH/REJECTED_DISABLED/REJECTED_SAFETY/FAILED)标 enum;`refusalReason` 可空(APPROVED 时 null,被门禁拒绝时给原因)。**此前 schema 从未与实现相符,属修正错 schema**——前端应改用 `sessionId` 续多轮对话、据 `promptDecision` 判断是否被拒并展示 `refusalReason`,去掉对 `conversationId/model/tokens` 的依赖。仅改 OpenAPI + 本协议,Java DTO 未动。 |
