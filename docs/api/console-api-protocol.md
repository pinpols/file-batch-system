...
| 日期       | 变更摘要                                                                                                                                      |
|------------|-----------------------------------------------------------------------------------------------------------------------------------------------|
| 2026-09-20 | **租户配置包 Excel 友好度增强**：`GET /api/console/config/tenant-package/excel/guide` 返回 11 张配置表字段说明，包含必填、只读、类型、枚举、默认/留空行为、示例和适用范围；新增 `GET /api/console/config/tenant-package/excel/sample-template?scenario=ALL\|IMPORT\|EXPORT\|PROCESS\|DISPATCH\|WORKFLOW` 下载场景化示例模板。原 11-Sheet 全量模板、上传、预览、应用契约保持不变。 |
