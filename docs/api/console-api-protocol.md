...
| 2026-09-02 | **工作流设计器全量保存保留 ADR-018 跨批量日依赖**：`WorkflowDefinitionSaveRequest.NodeItem` 与 `WorkflowDefinitionSaveNodeItem` 增加 `crossDayDependencies`、`crossDayDependencyTimeoutSeconds`。全量更新会删除并重写节点，原契约只读回这两个字段却不能回写，编辑后会丢失跨日依赖；现已贯通 DTO、MyBatis 写入和前端生成类型。未传 timeout 时保持数据库默认 86400 秒。 |
...
| 2026-07-11 | **Console Map 响应收敛批次 4(notification + workflow + file + rbac 域,收尾批次)**:共 23 处 `Map<String,Object>` 响应换类型化 record schema(字段名与历史 wire 一字不差,service 返回类型不动、仅 controller 边界 `from(map)` 转换,告警查询数据源 `ConsoleAiTools` 路径不变)。**转换**:notification 渠道/规则/投递日志(mapper 显式列,`snake_case` 键,`config_json/payload_json/error_args` 为 jsonb 原样透传的 `Object` 字段)+ 渠道测试结果(camelCase,`httpStatus/errorSummary` 键恒存在值可空)、alert-routing list/create/update(`alert_routing_config` 全列标量,按 `select *` 全列枚举一个 `snake_case` record 同服务三端点,list 缺省 `created_by/updated_by/is_deleted` 由 `NON_NULL` 省略)、event-catalog event-types/topics、workflow-runs 生命周期动作(cancel/terminate/pause/resume `{id,status}`、skip-node `{id,nodeCode,nodeStatus}`,编排内部代理透传)、pipeline-definitions list(`snake_case` 全列标量)、files/presign-upload(编排托管上传会话 9 固定 camelCase 字段)、auth public-key/stream-ticket、captcha config、api-keys create、tenants usage(系统参数缺失键由 `NON_NULL` 省略)。**保留 Map(合理保留)**:meta enums/pipeline-stages(字典名/流水线类型→列表的真动态键,additionalProperties)、file-channel/file-template list/get/create/update(宽 jsonb 配置表,get/create/update 走 `select *` 无法安全枚举,list 亦为同表 jsonb 全列透传,与批次3 保留同表详情一致)、governance settings(点分设置键→值动态映射)。OpenAPI 将对应 `CommonResponseObject`/`CommonResponseMapStringObject`/`CommonResponseString` 替换为具体 object/array schema;alert-routing 新增 `ConsoleAlertRoutingConfigResponse`(区别于 Excel 预览已有的 camelCase `ConsoleAlertRoutingResponse`)。breaking gate:object→object 澄清无 ERR;workflow-runs 动作 `string→object` 收窄进 path 白名单(同批次2 修正历史误声明)。**至此全仓 console `CommonResponse<...Map<...>>` 除上述合理保留项外清零。** |
| 2026-07-11 | **Console Map 响应收敛批次 3(observability + ops 域)**:dashboard(job-stats/trigger-stats/worker-load/alert-trend/sla-compliance/sla-report/execution-progress/tenant-usage)、system-parameters/value、cluster-diagnostic(聚合/shedlock/workers/outbox/terminal-children/instances)、config 审批(submit/detail/approve/reject 同构 detail)、ops kafka-lag、forensic export、queries workerCodes pipeline-progress 共 22 处 `Map<String,Object>` 响应换类型化 record schema(字段名与历史 wire 一字不差,service 仍返回 Map、仅 controller 边界转换,`ConsoleAiTools.getClusterDiagnostics` 读取路径不变)。**保留 Map(真动态/半结构化)**:job-stats `byStatus`(状态→计数动态维度键,additionalProperties)、instance-diagnosis `findings[].evidence`(逐 finding 异构负载)、admin/test-data 清理结果(表名→行数,已有 `CommonResponseTestDataCleanupResult`)、queries file-channel/template/record 详情(`select *` 宽表含 jsonb 列,snake_case 全列透传,原样 `CommonResponseObject`)。dashboard/kafka-lag 的 `Map<String,Object>` `CommonResponseObject` 替换为具体 object/array schema;forensic `CommonResponseMapStringObject` → `CommonResponseForensicExport`(与 orchestrator 实体 1:1,`downloadUrl` v0.1 恒 null 键保留)。breaking gate 无 ERR(object→object 澄清,不触发 batch2 的 string→object 白名单)。 |
| 2026-07-11 | **Console Map 响应收敛批次 2(job 域)**:instance 动作(cancel/terminate/pause/resume/partition cancel/retry/retry-failed)、batch-day 动作、batch-day-replay(session/preview/entry)、result-version、calendar/holiday、trigger/scheduler、job-bundle、batch-window、batch-trigger、dry-run-plan 共 38 处 `Map<String,Object>` 响应换类型化 record schema(字段名与历史 wire 一字不差,batch-window 保持历史 snake_case 键);dry-run-plan `summary` 保留 additionalProperties(真动态负载);12 条历史误声明 `CommonResponseString` 的端点修正为对象 schema |
| 2026-07-11 | **Console Map 响应收敛批次 1**:配置缓存、配置同步、资源队列、配额策略、配置依赖/版本差异、容量画像、资产分区 readiness、lineage 共 22 个响应由顶层 `Map<String,Object>` 改为固定 DTO；JSON 业务字段统一为既有前端已兼容的 camelCase，配置 payload 与 lineage 关联表行等天然半结构化字段仍保留在具名 DTO 字段内。OpenAPI 将对应 `CommonResponseObject` 替换为 13 组具体 response schema，`openapi-typescript` 可生成字段类型。breaking gate 同步精确放行“旧 data 无类型 → 新 data 明确 object/array”的纯契约澄清；明确类型之间的变化仍阻断。 |
| 2026-07-11 | **修正 `AiChatRequest` / `AiChatResponse` schema 与实现脱节**:`POST /api/console/ai/chat` 的请求与响应 schema 之前均与 Java DTO 脱节,现一并按 DTO 校正。**Request**(`AiChatRequest`):去掉后端不读的 `model/conversationId`,补 `sessionId`(多轮续接要传)与 `context`(`Map<String,Object>` → `additionalProperties`);`required` 由 `[tenantId, prompt]` 改为 `[prompt]`(DTO 仅 `prompt` 标 `@NotBlank`,`tenantId` 可由请求头携带,故请求体不强制)。**Response**(`AiChatResponse`):之前写的 `conversationId/model/promptTokens/completionTokens` 后端从不返回,真实返回的 `sessionId/promptCategory/promptDecision/modelName/refusalReason` 又缺失;现校正为 8 字段 `requestId/traceId/sessionId/promptCategory/promptDecision/modelName/answer/refusalReason`,`promptCategory`(PLATFORM/FILE_GOVERNANCE/WORKFLOW/OPERATIONS/OUT_OF_SCOPE)与 `promptDecision`(APPROVED/REJECTED_SCOPE/REJECTED_AUTH/REJECTED_DISABLED/REJECTED_SAFETY/FAILED)标 enum;`refusalReason` 标 `nullable: true` 且列入 `required`(DTO 无 `@JsonInclude(NON_NULL)`,APPROVED 时 Jackson 仍序列化 `refusalReason: null`,key 始终存在)。**此前 schema 从未与实现相符,属修正错 schema**——前端应传 `sessionId` 续多轮对话、据 `promptDecision` 判断是否被拒并展示 `refusalReason`,去掉对 `conversationId/model/tokens` 的依赖。仅改 OpenAPI + 本协议,Java DTO 未动。 |
| 2026-07-09 | **分页/仪表盘数据层收尾**:所有绑定 `PageQueryRequest` 的列表端点 `pageNo` 新增上限 `@Max(10000)`(共享参数 `PageNoQuery` 补 `maximum: 10000`),深翻页请改用 cursor(ADR-031);仪表盘 `job-stats/trigger-stats/alert-trend` 的 byStatus/byTriggerType/bySeverity 聚合由全史改为与同卡片 dailyTrend 相同的 `days` 时间窗(响应外形不变,数值语义变化,**请前端复核文案是否需标注"近 N 天"**)。 |
| 2026-07-09 | **console 写路径类型化(通知/重放/演练)**:`POST/PUT /api/console/notifications/channels[/{channelCode}]`、`POST/PUT /api/console/notifications/rules[/{ruleId}]`、`POST /api/console/ops/batch-day-replay/sessions[/preview]`、`POST /api/console/ops/dry-run/plan` 的 `@RequestBody` 由 `Map<String,Object>` 类型化为 DTO(`NotificationChannelUpsertRequest/NotificationChannelUpdateRequest/SubscriptionRuleUpsertRequest/BatchDayReplaySubmitRequest/DryRunPlanRequest`),**JSON 字段名与原 Map 消费键逐一相同,wire 不变**;OpenAPI 补齐 4 个通知端点的 request schema(原 `type: object` 自由体)。行为收紧点:缺失必填字段由服务内 4xx 提前为框架层 400(`@NotBlank`);dry-run `jobCode`、replay `calendarCode/bizDate/scope/reason/requestedBy` 现于 console 层校验(orchestrator 侧本就拒绝);渠道更新 body 不再接受 `channelCode`(路径参数为准,多传被忽略,原行为亦忽略)。 |
| 2026-07-02 | **API Key 撤销授权对称化**:`DELETE /api/console/api-keys/{id}` 由仅 `ROLE_ADMIN` 放宽为 `ROLE_ADMIN`+`ROLE_TENANT_USER`(与创建对称),path/请求/响应外形不变,仅授权矩阵变化。租户隔离由 mapper `WHERE tenant_id` 保证。修复"能建 key 不能撤销"的不对称安全洞。 |
| 2026-07-01 | **JOB asset freshness policy Console 管理闭环**:新增 `/api/console/asset-freshness-policies` 最小 CRUD: `GET` 列表、`POST` 按 `(tenantId,assetCode,assetType)` upsert、`GET /{id}` 查询、`PUT /{id}` 按 id 更新、`PATCH /{id}/enabled` 启停。当前只支持 `assetType=JOB`，字段与 `batch.asset_freshness_policy` 对齐:`assetCode/expectedByLocalTime/timezone/staleAfterSeconds/lookbackDays/severity/enabled`。该接口只管理 orchestrator freshness 扫描策略,用于产生 `ASSET_FRESHNESS_MISSING/STALE` 告警;明确不扩展为数据目录、字段级 lineage、MDM 或业务正确性裁定。 |
...
- `POST /api/console/webhooks` — create subscription (body: `name`, `callbackUrl`, `eventTypes[]`, `secret`, `enabled`); `secret` is write-only and is never returned by list/detail responses
