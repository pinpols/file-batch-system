...
### 2.1 可观测性:从「有指标」到「降级可见」(P1,本轮已落一阶段)
...
| 项 | 人天 | 说明 |
|---|---|---|
| **Checkpoint / 断点续跑** | **15–25** | 最大单项;契约 + 各 worker 适配 + 状态机集成 |
...
**判断**:第一梯队是 P1(≈半月,能立刻动)。P2 的盘子由两个大头决定——Checkpoint(3–5 周)和 AM 完整迁移(3–4 周,卡在真实环境影子期)。god class 不单独排期。
