...
| # | 决策点 | 选择 | 落地位置 |
|---|---|---|---|
| 12 | Phase 4 是否保留 checkpoint/progress | **W8 时根据真实租户任务时长决定**:都 <5min → 暂缓;>10min → 全做 | Phase 4 §6.2 决策点 |
...
**估时**:2 周 · **依赖**:Phase 2/3 · **默认状态**:🚫 排除(YAGNI)
...
**合计 10 周**,Phase 5 选择性 / Phase 6 默认暂缓。
...
| "全部通过"骗自己 | 全部 | 每 phase 出口对照 [`sdk-industry-benchmark`](../design/sdk-industry-benchmark.md) §2 矩阵核对(memory: `feedback_audit_vs_architecture_review`) |
...
- Phase 4 完整版 —— 任务都 < 5min 时可暂缓 checkpoint / progress,保留 cancel + timeout
...
| Agent | 模块范围 | 不碰的文件 |
|---|---|---|
| **Agent-SDK** | `sdk/java/core/**` | orchestrator / console-api / FE 全部 |
...
| 文件类型 | 多 agent 并行? | 备注 |
|---|---|---|
| `sdk/java/core/src/main/java/**` | 不同子目录可并行 | 同文件必须 sequence |
...
```
  ...
  #SDK-P1-4 metrics + isHealthy            #SDK-P2-2 4 态状态机
                            ...
                            Phase 3 M3.1 (schema → upsert → effective → SDK 上报)
                              #ORCH-P3-1 registry 表 + Flyway V159
                                          ▼
...
| 6 | Controller 返回值一律 `CommonResponse<T>`,**禁**直接返回 DTO | `return user;` | `grep -rn "public.*Controller" src/main --include="*.java"` 后人工核 |
...
这两类是历史上**最频繁被 CI pr-gate 拦下**的(memory: `feedback_mvn_build_gotchas` 同类教训),**push 前本地过一遍**,优于等 CI 反馈。

### 15.9 单对话连续执行模式(自动跑完 + checkpoint)

> 注:本节是内部 agent 操作约定(无人值守连续执行 + checkpoint),非路线图内容,仅供自动化执行时参考。
>
> 适用场景:不想盯进度,希望执行方在一个对话里**连续做完所有 phase**,只在关键节点停下来 review + merge PR。
...
| AC-0.6 | 156 现有测试全部通过 + 5 新增 | `mvn test` 全模块 |
...
| AC-3.2 | Flyway V159 applied + `archive_drift_check` 启动期不 fail | 平台启动日志 |
