...
## 6. 工作量估算（单人粗估,人天;标注外部依赖）

### Phase 1 — 推向生产可用

| 项 | 人天 | 依赖/风险 |
|---|---|---|
| 前端聊天入口 | 3–5 | **在 batch-console 另一个仓做**(本仓无 FE) |
| Spring AI M3 → GA 稳定化 | 2–3 | **被动等 Spring AI GA** |
| provider 契约进 CI | 2–3 | **需 GH secrets,本地不可验** |
| 成本/限流/降级 | 3–4 | 复用 bucket4j + token 计量 |
| 上线判定/受控试生产 | 1–2 | |
| **Phase 1 小计** | **11–17** | 约半月,但三项是协调/等待非纯写码 |

### Phase 2 — 扩展接入点（可增量各个击破）
...
*依据:2026-07 对 origin/main 的 AI 现状勘查(console AI 助手已接 Spring AI + RAG + 只读工具 + 审计,默认关闭),及既有安全/边界设计文档。工作量为单人粗估。*
