...
- [x] Step 1: 看 `batch-console-api/src/main/resources/mapper/TenantMapper.xml` 确认 batch.tenant 的租户标识列与启用列的真实列名(预期类似 tenant_code/code + enabled/status)
- [x] Step 2: 写失败 IT:`ActiveTenantProviderIntegrationTest extends AbstractIntegrationTest`,@SpringBootTest 注解按仓库 IT 惯例;arrange 直插 2 条 tenant(1 启用 1 停用)→ assert `provider.activeTenantIds()` 只含启用者且带 30s 缓存语义(连续两次调用第二次不打库——用 @SpyBean mapper verify times(1));断言带 .as()
- [x] Step 3: 实现:mapper `selectActiveTenantIds()`(select 标识列 from batch.tenant where 启用条件 order by 1);Provider 构造器注入 + `volatile` 缓存 30_000ms(System.nanoTime 比较,无外部依赖);CLAUDE.md 红线遵守(构造器注入/无 FQN)
- [x] Step 4: 跑测试绿 → commit `feat(orchestrator): 租户路由基础设施 ActiveTenantProvider(batch.tenant reference 30s 缓存)`
...
- [x] Step 1: 接口+XML 加 `tenantId` 首参,SQL `where tenant_id = #{tenantId} and workflow_run_id = ...`(其余不动)
- [x] Step 2: 两个调用方传 tenantId(从各自已加载的 workflowRun 实体取;若局部只有 workflowRunId,沿调用链最近的 tenant 上下文取,**禁止**额外查库)
- [x] Step 3: `mvn -q test -pl batch-orchestrator -Dtest='*Workflow*,*TaskOutcome*' -DfailIfNoTests=false` 绿 → commit
...
- [x] Step 1: 改 SQL+接口 → Step 2: reconciler 循环+隔离 catch → Step 3: 测试绿 → commit
...
- [x] 同 Task 3 三步 → commit `feat(orchestrator): 传感器/补偿全局扫改租户路由(Citus 单分片 FOR UPDATE 合法化)`
...
- [x] Step 1: 写脚本(bash -n 过)→ Step 2: `citus-cluster.sh up && init` 实跑,3 节点就绪 + GUC 校验(SHOW 两项)→ commit

### Task 6: 真集群迭代验证(harvest 循环,我=controller 亲自驱动)

- [x] Step 1: worktree 重打包 8 jar(含全部新提交)
- [x] Step 2: coordinator 建 batch_platform(Flyway 经 orchestrator 首启自动跑 V1..V179)→ 跑 `scripts/db/citus/01-distribute.sql` → 重启 orchestrator
- [x] Step 3: **harvest 循环**:观察 logs/app/orchestrator.log 5 分钟;出现新 Citus 运行时错误 → 按"租户路由/配置/降级"三分法修 → 重启复测;直到 orchestrator 稳定 UP 且调度器 tick 无 ERROR
- [x] Step 4: 扩到 trigger+console+5 worker(business 库连分支专用库 batch_business_part;停主服务窗口内做,完毕恢复)→ 8/8 health
- [x] Step 5: 真链冒烟:console/API 触发一个 import job(或直插 trigger_request 走调度),验证 job_instance/outbox 落分布式表、Kafka 流转、worker CLAIM 执行终态 SUCCESS
- [x] Step 6: guard:共享库零污染核验;成果追加进 w8-findings 文档 → commit

### Task 7: 双栈零回归终审 + 收尾

- [x] Step 1: 普通 PG 全量:`mvn -B clean test` 全绿(15 模块)
- [x] Step 2: e2e:install 上游 + `run-tests.sh --e2e --skip-build` 41/41
- [x] Step 3: 计划勾选、推分支(不合 main)、主环境恢复(8 服务+m2)
...
---
## 执行结果(2026-06-12)
T1-T5 全部完成并双栈验证;T6/T7 真集群:schema 部署+8服务UP+FOR UPDATE 消除+调度链路贯通至 LAUNCHED;worker 执行侧端到端余 2 项 SQL 方言适配(IMMUTABLE upsert + seed),详见 citus-w8-runtime-findings backlog。普通 PG 双栈零回归 BUILD SUCCESS(e2e 41/41)。
