...
> **历史环境说明**:2026-05-31 记录发生时曾使用 JDK 25 临时环境；平台已于 #698 回到 JDK 21。下方历史记录中的 JDK 25 事项仅解释当时执行背景，不作为当前环境要求。
...
- **实际 vs plan**:工作量 ~2.5h(plan 估 3h)略低。比 plan 多做:`isHealthy()` 显式定义 drain 期=healthy(语义在 javadoc 固化)避免后续 caller 误读;`SdkClientMetrics` 是 record(不可变快照)且字段以 javadoc 钉住 Prometheus label 用途。比 plan 少做:**没**真把 fatal/crashed 串到 `System.exit` 或 K8s liveness endpoint —— SDK 不假设宿主进程的 health 暴露方式,只提供 boolean(memory: `feedback_yagni`),由租户自决怎么暴露。
- **环境问题**:当时存在 JDK 25 临时前置(见顶部历史环境说明),本次第 4 次重复;后续已随 #698 回到 JDK 21。
- **测试结果**:`mvn -pl sdk/java/core test` 170/170 绿(原 162 + 新 5 `BatchPlatformClientMetricsTest` + 新 3 `KafkaTaskConsumerCrashTest`);`KafkaTaskConsumerCrashTest` 用 Mockito mock `Consumer`(MockConsumer 在 SDK 测试集只用于 rebalance listener 单测,不真跑 run loop 避免 pattern subscribe 兼容性问题)。
...
- **环境问题**:当时存在 JDK 25 临时前置(见顶部历史环境说明)。该问题已随 #698 回到 JDK 21 后失效。
- **测试结果**:`mvn -pl sdk/java/core test` 161/161 绿(原 146 + 新 10 `TaskDispatcherClaimRetryTest` + 5 `PlatformHttpExceptionTest`);旧 `TaskDispatcherTest.claimFailureDoesNotExecuteOrReport` 把裸 `IOException("409")` 换成 `PlatformHttpException(409, ...)` 才准。
...
- **环境问题**:当时存在 JDK 25 临时前置(见顶部历史环境说明)。该问题已随 #698 回到 JDK 21 后失效。
- **测试结果**:`mvn -pl sdk/java/core test` 146/146 绿(原 140 + 新 6)。
...
- **状态**:本地分支 `feature/sdk-phase-0-protocol-foundation` 已 commit 1 个 commit;`git push` 与 `mcp__github__push_files` / `create_branch` 均返回 `403 Permission to pinpols/file-batch-system.git denied to idengzhao`。GitHub MCP 鉴权身份是 `idengzhao`,但目标仓库 `pinpols/file-batch-system` 上没有 write 权限,无法走自动 push + create PR 流程。
- **退出原因**(对照自动执行流程的停下条件):**触红线(pre-push 拒绝)**;需人工介入授权或改鉴权身份。
- **本地交付**:见下方 Phase 0 完成段;`mvn -pl sdk/java/core test` 140/140 绿;`SdkWireContractTest` 9/9 绿;pre-push self-check 全过。
...
- **环境问题**:当时存在 JDK 25 临时前置(见顶部历史环境说明)。该问题已随 #698 回到 JDK 21 后失效。
- **测试结果**:`mvn -pl sdk/java/core test` 140/140 绿(原 135 + 新 5);`SdkWireContractTest` 9 测试绿;**未跑** orchestrator 全模块 verify(memory: `feedback_p1a_no_full_test`)。
