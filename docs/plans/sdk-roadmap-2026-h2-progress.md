...
- **环境问题**:JDK 25 前置(见顶部共享前置),本次第 4 次重复;下次起手时一并评估归档下载脚本。
...
- **环境问题**:JDK 25 前置(见顶部共享前置)。已成 routine,前 3 个 PR 重复多次,可考虑归档下载脚本到 `scripts/local/setup-jdk25.sh`(留作 #SDK-P1-4 顺手做)。
...
- **环境问题**:JDK 25 前置(见顶部共享前置)。容器重建会丢,下次重复一遍即可。
...
- **环境问题**:JDK 25 前置(见顶部共享前置)。后续 routine 启动复用已解包的 JDK 25 即可。
...
- **环境问题**:JDK 25 前置(见顶部共享前置)。后续 routine 启动需复用已解包的 JDK 25。
