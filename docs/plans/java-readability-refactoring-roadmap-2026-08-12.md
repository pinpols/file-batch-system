...
- `AbstractExportFormat`：已完成 cursor 分页、页边界 fsync、checkpoint 推进和进度节流职责拆分；格式列解析与编码策略保持原边界。
