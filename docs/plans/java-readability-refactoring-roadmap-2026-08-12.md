...
| 1 表达一致性 | 部分完成 | #921：安全的 Spring 配置轻量化 | 其余局部表达和注释仅随触及模块处理，不单独全仓改写 |
...
| 3 固定 Map 契约 | 进行中 | #924：Console/内部 API/Java SDK 首批固定响应 DTO；#933：文件维护投影；#936：batch-day replay 影响投影 | 固定 Mapper 投影分簇处理；JSONB、metadata、插件参数和动态聚合 Map 保留 |
| 4 复杂类拆分 | 已启动 | #923：Task Outcome 状态策略提取 | 仅对列出的复杂类做 characterization 后按职责提取 |
| 5 测试风格 | 未开始 | — | 在相应生产类拆分后就近治理 fixture 与命名 |
| 6 例外治理 | 未开始 | — | 审计 suppression、白名单与长期理由 |
| 7 最终验收 | 未开始 | — | 前述阶段结束后跑 Full Gate、关键 sim 与容器启动验收 |

近期合入记录：#918 `529b9a319`、#921 `557897f4b`、#922 `c053c4544`、#923 `8e5e1342`、#924 `cfa33e871`、#933 `911fd15d8`、#935 `fb2c7ad59`、#936 `490d02c73`。#935 后已用 `@Lazy/@Autowired`、`ObjectProvider` 及常见 self/proxy 标识复扫生产源码，结果为 0；#936 保持 batch-day replay 预览 JSON 字段不变，仅将两组固定 SQL 投影移至 record 边界。
...
**验收**：主流程可在一个屏幕内读出阶段顺序；构造依赖按职责下降；原测试和新增 characterization test 全绿；状态机、事务和性能基线不退化。
