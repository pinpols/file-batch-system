...
| 2 自注入迁移 | 进行中 | #918：低风险自注入；#922：Console 租户初始化事务协作者 | 高风险状态链路按一类一 PR 迁移，不能批量替换 |
| 3 固定 Map 契约 | 进行中 | #924：Console/内部 API/Java SDK 首批固定响应 DTO；文件投影与 JSON 契约回归测试 | 固定 Mapper 投影分簇处理；JSONB、metadata、插件参数和动态聚合 Map 保留 |
...
近期合入记录：#918 `529b9a319`、#921 `557897f4b`、#922 `c053c4544`、#923 `8e5e1342`、#924 `cfa33e871`。#924 的完整 CI 均为绿色，其中 `unit-it-b2` 覆盖了文件配置真实写入链路；它曾暴露的无时区 PostgreSQL 时间戳兼容问题已由回归测试锁定。
