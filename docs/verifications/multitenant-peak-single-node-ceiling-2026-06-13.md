# 多租户并发峰值流量 — 单机控制面吞吐上限实测(2026-06-13)

> 对应 `docs/analysis/scaling-state-and-biz-path-2026-06-14.md` §4 解冻条件①②(**单机榨干**)。
> 结论先行:**单机完成吞吐上限 ≈ 20–22 jobs/s,瓶颈在控制面串行处理(有效并发被压到 ~7),
> 与 PG 写入能力无关、与 Citus 分片无关。** 故门槛③(Citus 甜点验证)在本地单主机无意义,
> 真正的 go/no-go 须放多主机;但门槛①② 给出明确结论:**先调控制面并发,远未到该上 Citus 的地步**。

## 为什么这场是"单机上限"而非"A/B 对比"
...
- 驱动:`load-tests/scripts/peak_ceiling.py`(本轮新增)。对 ta/tb/tc 等比并发 launch 轻量
  ...
  按并发梯度抬升 offered load,测每级 launch 吞吐/p95 + 完成排空曲线 + **峰值持续完成率**(真上限)。
...
两个最可疑的节流点抬高后上限**纹丝不动** → 瓶颈在更深的**控制面串行环节**。
...
`worker maxConcurrentTasks` **本就是配置项**(无需改代码),默认 4 对多租峰值流量偏保守;调高即放量,但
...
launch+worker 双开后峰值 62/s 仍随并发上升(48.7→61.7)未饱和,曾疑第三层是 report/claim 争用。
**针对性探测(加载时采样 `pg_stat_activity` + 主机 load)给出结论:本地无法继续测**:

- **PG 零锁争用**:加载中活跃后端仅 1–4 个,全是 `WalSenderMain`/`WalSync`(复制/IO),**无任何 Lock /
  行锁等待**。PG 基本闲着——再次证明不是 DB/Citus 问题。
- **本地约束是主机 CPU**:8 核机器跑 8 JVM + PG + Kafka + 3 Citus 容器,load 飙到 14–19(远超核数)。
- **数字已失真**:同样 launch=4+worker=32,主机轻载时峰值 62/s、重载时掉到 6.6/s——**10× 波动纯由
  host load 决定**,第三层(若存在)埋在本地资源饱和噪声之下,测不出。

**结论**:第三层必须到**真多主机**(每组件独占资源)才测得出 report/claim 是否真争用;本地继续榨是和宿主
CPU 较劲,无意义。前两层(launch + worker 并发)是真·代码/配置级可移植收益(3×),到此为止是干净的停点。
**本文主结论不变:PG/Citus 始终不是瓶颈,杠杆全在控制面分层并发。**
