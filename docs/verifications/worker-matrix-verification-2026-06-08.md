...
本轮之后 P0 已继续补齐：mixed 压力复验、atomic 1w storm、trigger misfire/cron/outbox 专项、PG 写入参数矩阵均已形成报告。P2 本地容量画像也已补跑：process kill/PG 断链恢复可恢复，10w storm 达到本机上限且不通过，多租户最终全成功但公平性/恢复延迟不达标。仍不能声称“生产同类全矩阵已通过”的是 P1 外部依赖：真实云 S3、真实 SFTP/NAS/HTTP 故障注入。
...
| 1w / 10w task storm | 已覆盖本地画像 | `preprod-p0-atomic1w-20260608140503` 已跑 1w 全终态；`p2-capacity-20260608171944-10w` 已跑 10w，本机 OK=41096、KO=58904，结论是不通过且不能作为生产容量承诺。 |
| process 大数据失败恢复、幂等重跑、中途失败恢复 | 已覆盖本地 fault profile | 10w aggregate/copy/idempotency 已跑；`process-stage4c-20260608163544` 复跑分片和 RUNNING cancel；`p2-process-failure-20260608171651` 覆盖 kill worker、PG backend terminate、同 batchKey 恢复重跑。 |
...
| export 更高分片数、真实 S3、多租户混压 | 部分覆盖 | `sim-export-stage3c-20260608143456` 已复跑本地 MinIO/S3-compatible 8 分片、三租户、幂等；`p2-fairness-trigger-20260608180000` 已覆盖 trigger/orchestrator 多租户混压，最终成功但公平性不达标；真实云 S3/OSS multipart abort/retry 未配置。 |

## 上线前仍建议补的硬项

1. 建一个可重复的故障注入 harness：暂停 Kafka/Redis/PG、模拟下游 5xx/timeout、验证重试和 DLQ；Import worker crash after chunk 已由 Stage 2e 覆盖，process worker kill / PG backend terminate 已由 P2 profile 覆盖。
