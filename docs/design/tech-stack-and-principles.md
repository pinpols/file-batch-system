...
## 2. 持久层选型原则（MyBatis 单一栈）

**全业务表**主写入口只能是 **MyBatis**（同一表、同一语义写路径 **禁止** 双栈：不得再并存 `Repository` + `Mapper`）。

- **MyBatis** → 配置态 / 定义态 / 运行态 / 实例态 / 文件链路 / 调度推进 / 补偿 / 审计 / 报表
  - 目录：`mapper/*.java + resources/mapper/*.xml`
  - 行载体：`domain/entity/*Entity`（`record` 或 `@Data` class）
- **`JdbcTemplate`** → 仅锁表、极薄查询等基础设施（默认业务 CRUD 仍走 Mapper）

**推荐落位**：

| 数据类别 | 推荐技术 | 典型对象 |
|---|---|---|
| 任务定义 / 流程定义 / 资源配置 / 告警规则 | MyBatis | 与运行态同一套 Mapper |
| 任务实例 / 分片实例 / 重试 / 死信 / 文件流转 / 审计日志 | MyBatis | `job_instance`、`job_partition`、`retry_schedule`、`dead_letter_task`、`file_record` |
| 控制台读写与复杂筛选 / 统计 | MyBatis | 配置表、实例中心、文件中心、告警中心、SLA 检索 |
| 小型聚合维护 | MyBatis | 模板主表 + 子项、规则主表 + 子项 |
...
- 与 Spring Boot / MyBatis / Quartz / Kafka / Flyway / Prometheus 主依赖方向一致
...
- [`../architecture/adr/`](../architecture/adr/README.md) — 架构决策记录（含 ADR-001 持久层决策）
