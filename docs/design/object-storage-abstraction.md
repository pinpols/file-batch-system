...
> ② `list` / `copy` **纳入接口**(共 9 方法),13 处直连全收口。下文已据此定稿(原"待定/可选"措辞改为已定)。
...
现状:全系统都走 `io.minio.MinioClient`,**没有别的后端**。收口成 `BatchObjectStore` 接口后,**实现很少**:
...
> - **`getFrom(offset)` 取代 `getRange(offset,length)`**(对标 Iceberg `SeekableInputStream`/Hadoop `seek` 的单 seek 退化形):前版"length 当下界"语义糊。range-slice 要"从 offset 正向读、自己在行边界停",`getFrom` 正合此意,与今天 `getObject(offset)` 行为一致;要"精确 N 字节"的调用方读 N 字节即可。
> - **统一异常层级**:`ObjectStoreException` + `ObjectNotFoundException`(typed NoSuchKey)+ `ObjectStoreAccessException`,消除 S3/FS 抛异常不一致。
>
> **9 方法**:put/copy/delete/get/getFrom/statSize/exists/list/presign。13 处直连全部收口。

## 2. 实现清单

| # | 实现 | 角色 | 必要性 |
|---|---|---|---|
| 1 | **`S3ObjectStore`**(MinIO/AWS S3/阿里 OSS/GCS 全覆盖) | **唯一生产实现** | **必须**。13 处裸 `MinioClient` 全收口到它 |
...
存储后端不是这回事:**生产侧全是 S3 协议(配置切换)**,只有"本地文件系统"机制不同。所以它**该收口成一个窄接口,不该做成渠道式 SPI**——
...
分区放大优化两后端通用,本地版省网络往返。**与今天只传 offset 的 `getObject(offset)` 行为完全一致**(故弃用前版"length 当下界"的糊契约)。
...
**工作量**:阶段一 ~3-5 天(13 处收口 + S3 实现 + 契约测试);阶段二 ~1-1.5 周(FS 实现 + 4 难点 + 测试);阶段三 ~2-3 天。合计约 **2-3 周**。若只做阶段一(解耦+DRY,不上 FS),~1 周。

**风险**:
- 13 处收口面广,阶段一虽零行为变化仍需全回归(ingress 扫描 / export 上传 / console 下载 / 治理对账 / dispatch 读取)。
...
**收敛策略:不做独立的 cosmetic 改名 pass,靠 Phase 2 收口顺带清。**
...
本地 FS 1 个可选(兼测试替身),加密是装饰层。**封顶 2 个实现**,不做渠道式 SPI。先做阶段一(纯收口)兑现解耦+DRY,FS 按需再上。
