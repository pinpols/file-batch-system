...
- OpenAPI / Swagger 文档
- 优雅停机（Orchestrator / Trigger drain + graceful shutdown）
- Kafka 消费积压查询
- 配置 Diff
- 配置依赖分析
- 批量启停
...
- 系统参数热更新：已有 CRUD + Redis 缓存，运行时消费者尚未接入
- Webhook 回调注册：已支持订阅 CRUD、异步投递、签名与投递日志，但未覆盖所有关键状态变更点
- 沙箱 / 模拟执行：console 入口支持 dry-run 校验不执行，trigger 服务原生接口未增加 dryRun
...
| 优雅停机 / 预热（部分完成） | 已实现：Orchestrator / Trigger graceful shutdown + drain 端点；预热/摘流量恢复能力仍缺 | P1 |
...
| 系统参数热更新（部分完成） | 已有 system_parameter CRUD + Redis 缓存；运行时消费者尚未接入，暂未形成真正热更新 | P1 |
...
| Webhook 回调注册（部分完成） | 已支持 webhook 订阅 CRUD、异步投递、签名与投递日志；仍未覆盖所有关键状态变更点 | P1 |
| SDK / 客户端生成 | OpenAPI 已具备，但仓库内仍未提供官方生成/发布的 SDK | P2 |
| 沙箱 / 模拟执行（部分完成） | console 触发入口支持 dry-run 校验不执行；trigger 服务原生接口尚无 dryRun 参数 | P1 |
...
| 缺口 | 改动点 | 预估工作量 |
|---|---|---|
| Webhook 回调注册（部分完成） | 已实现 `webhook_subscription` / `webhook_delivery_log`、订阅 CRUD、异步投递、HMAC 签名与投递日志；当前主要基于 console realtime event，尚未覆盖 orchestrator/worker/trigger 全量关键事件，也未做独立 outbox/死信 | 1-2 周（剩余） |
...
| ~~优雅停机~~ | ~~已完成：Orchestrator / Trigger 已支持 graceful shutdown + drain 端点；后台调度在 drain 态停止接新工作~~ | ~~已完成~~ |
| 系统参数热更新（部分完成） | 已实现 `system_parameter` 表、Console CRUD、Redis 缓存；运行时参数消费链路待接入 | 1-2 天（剩余） |
...
| 沙箱模拟执行（部分完成） | 已实现 console 侧 `dryRun` 校验不执行；trigger 服务原生接口未接 `dryRun` | 1 天（剩余） |
...
1. ~~健康检查 + 优雅停机~~ — ~~已完成~~
2. ~~OpenAPI 文档~~ — ~~已完成~~
3. **系统参数热更新（剩运行时接入）** — 当前仅完成参数 CRUD + Redis 缓存

### 第二批（集成与运营效率）

4. **Webhook 回调注册（剩事件覆盖与可靠投递）** — 当前为基于 console realtime event 的 MVP
5. ~~配置依赖分析 + Diff~~ — ~~已完成~~
6. ~~批量 API + 幂等 key~~ — ~~已完成~~
...
11. **沙箱模拟执行（剩 trigger 原生 dryRun）** — 当前仅 console 入口支持 dry-run 校验
