...
- ~~通知订阅管理~~ → V49 migration + `ConsoleNotificationController`（渠道 CRUD + 订阅规则 CRUD + 投递日志 + 测试发送）
- ~~配置审批流~~ → V49 migration + `ConsoleConfigApprovalController`（submit/approve/reject） + 审批状态机（DRAFT→PENDING_APPROVAL→PUBLISHED/DRAFT）
- ~~环境间配置同步~~ → `ConsoleConfigSyncController`（export/preview/import） + `ConfigSyncLogMapper` 操作日志
...
| ~~环境间配置同步~~ | ~~已完成：`ConsoleConfigSyncController`（export/preview/import）+ `ConfigSyncLogMapper` 操作日志~~ | ~~已完成~~ |
| ~~配置审批流~~ | ~~已完成：`ConsoleConfigApprovalController`（submit-approval/approve/reject）+ 状态机（DRAFT→PENDING_APPROVAL→PUBLISHED/DRAFT）~~ | ~~已完成~~ |
...
| ~~通知订阅管理~~ | ~~已完成：`ConsoleNotificationController`（渠道 CRUD + 订阅规则 CRUD + 投递日志 + 测试发送）~~ | ~~已完成~~ |
...
| ~~配置审批流~~ | ~~已完成：V49 `config_approval` 表 + `ConsoleConfigApprovalController`（submit/approve/reject）+ 状态机（DRAFT→PENDING_APPROVAL→PUBLISHED/DRAFT）+ `ConfigChangeLogMapper` 审计日志~~ | ~~已完成~~ |
| ~~环境间配置同步~~ | ~~已完成：V49 `config_sync_log` 表 + `ConsoleConfigSyncController`（export/preview/import）+ `ConsoleTenantConfigCopyService.buildBundle()` 序列化 + 操作日志~~ | ~~已完成~~ |
| ~~通知订阅管理~~ | ~~已完成：V49 `notification_channel` + `subscription_rule` + `notification_delivery_log` 表 + `ConsoleNotificationController`（渠道/规则 CRUD + 投递日志 + 测试发送）~~ | ~~已完成~~ |
