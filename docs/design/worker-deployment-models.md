...
| **故障 blast radius（影响半径）** | 多租户共享(handler 挂 = 全租户卡) | 单租户隔离 |
| **成本归属** | 平台账单 | 租户账单 |
| **运维方** | 平台团队 | 租户团队 |
...
**典型选型路径**:租户初期上云全走平台 worker(开箱即用,运维简化处理);随业务变重 / 合规收紧 / 数据规模变大,**逐步把敏感任务迁到自托管 SDK**。
...
1. 🔴 **数据不出租户网络** —— 平台只看到 Kafka 派单 + HTTP 报告,业务数据**全在租户域内流转**。合规 / GDPR / 数据本地化要求通过统一部署边界处理。**这一条构成 ADR-035 的核心价值。**
...
自托管 SDK 默认缺少调度上下文 —— 当前 `TaskDispatchMessage` 只有 `parameters / runtimeAttributes`,没有任何"业务日 + 调度元信息"。本节说明哪些上下文**应当下沉**,哪些**不能下沉**。
...
**反例(租户自己算 bizDate)的问题**:跨午夜执行错位、节假日规则不同步、跨时区错位。
...
| **REPORT 连续失败 N 次** | fail-fast(平台可能异常退出) |
...
⚠️ **跨进程同 task 并发**:Worker A GC 长期停滞 → lease 超时 → orchestrator 派给 B → A 恢复后继续跑 → 双进程并行。
