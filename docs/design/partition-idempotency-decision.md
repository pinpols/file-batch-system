...
3. 极端竞态下重复事件的下游代价:Kafka 重复投递 → Worker CLAIM 唯一性回退,
   不会双执行。
4. 残余风险等级:低;监控抓手:
...
2. **event_key 含与被锁聚合 version 绑定的判别位**(如 PartitionReclaimUnit 把 partition.version 嵌入 key)
   ...
   > —— 月分区后全局 `(tenant_id,event_key)` UNIQUE 不再硬回退(改 NOT EXISTS),这三条是唯一防线。
...
( > 0 持续即 warning)。窗口默认 24 小时,可用
`batch.metrics.backlog.outbox-duplicate-window-hours` 调整。任何实际漏过的重复都会被第一时间发现,
而非等到对账。
