# 租户配置包 Excel 9+2 优化设计

> 状态：方案设计 v2。
>
> **当前实现仍是 8 个业务 sheet**：
> `job_definition / file_channel_config / alert_routing_config / pipeline_definition /
> pipeline_step_definition / workflow_definition / workflow_node / workflow_edge`。
>
> **本次设计相对现状的两类变化**：
> - **新增** 4 个 sheet：`file_template_config`（必填）+ `resource_queue / business_calendar / batch_window`（均可选，引用时必填且允许 DB fallback）
> - **移除** 1 个 sheet：`alert_routing_config`（理由见 §设计目标 / §兼容策略）
...
- **不把 `alert_routing_config` 放入配置包**：告警路由（team / receiver / severity → channel 映射）是**环境侧治理资产**，prod / staging / dev 各有自己的 oncall 拓扑，跨环境照搬反而是反模式。`alert_routing` 改走独立 Excel 入口（`/api/console/config/alert-routings/excel`）。
...
> 用户列表里只看到 8 个？是的。**原"核心 9"里的 `alert_routing_config` 已剔除**（理由见 §设计目标），但 1-8 都是真正属于"作业链路自身"的核心定义。
...
- **不出现在配置包内的相关 sheet**：`alert_routing_config`（独立维护，见 §兼容策略）、`tenant_quota_policy`（租户治理）、`worker_registry / step_registry`（运行时上报）。
...
- 对 `alert_routing_config`：旧 8-sheet 包上传时 preview 显示一条 INFO "本包含 alert_routing_config sheet，已忽略；如需维护告警路由，请前往 [独立 Excel 入口]"，不阻断 apply。
...
- 引用不存在时 preview 报错测试。
- 11 sheet 为空的兼容测试。
- **旧 8-sheet 包兼容测试**：含 `alert_routing_config` 上传时 preview 出 INFO 不阻断 apply。
...
- 后端兼容期 **3 个 minor 版本**（约 6 个月）：旧 8-sheet 包可上传，preview/apply 正常工作。
- `alert_routing_config` sheet 在旧包里**忽略不解析**，preview 显示一条 INFO："本包含 alert_routing_config sheet，从 vX.Y 起不再随配置包维护；如需修改，请走 [/api/console/config/alert-routings/excel] 独立入口"。
- 新增 sheet 缺失时按引用关系按需检查：
  - `file_template_config` 缺失：允许上传，但若发现 `templateCode` 引用无法在数据库中解析，preview 必须报错。
  - `resource_queue / business_calendar / batch_window` 缺失：仅当被引用且数据库也不存在时才报错。
- 新导出的配置包一律使用 11 sheet 结构（8 业务 + 3 可选），**不导出 `alert_routing_config`**。
- 兼容期结束后，下一个 major 版本去除 `alert_routing_config` 上传忽略逻辑，直接 preview 报错。
...
- 推荐迁移流程（doc 引用，不强制）：
  ...
  4. 单独走 alert-routing Excel 入口配置目标 env 的告警路由
