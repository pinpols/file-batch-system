...
- 查询优先读 `asset_partition`,但物化行必须重新关联 `result_version.status='EFFECTIVE'` 且不存在同 `businessKey` 更高 `version_no` 的版本;本地旧数据或迁移前数据未物化时,回退到 `result_version` 当前 EFFECTIVE 投影。
...
`asset_partition` upsert 只允许指向 `EFFECTIVE` 版本,且只在新版本号不小于当前物化版本号时更新。这个单调守卫是防终态回退的一部分;任何修改 `asset_partition` 唯一键或 `ON CONFLICT` 语义的 PR,必须同步复核 readiness 最新版本判定。
...
Console 最小管理入口已落在 `/api/console/asset-freshness-policies`:

- `GET` 列表、`GET /{id}` 查询。
- `POST` 按 `(tenantId, assetCode, assetType)` upsert。
- `PUT /{id}` 按 id 更新。
- `PATCH /{id}/enabled` 启停。

该入口只支持 `assetType=JOB`,只管理扫描策略,不做 FILE/TABLE 资产目录、不做字段级 lineage、不裁定业务数据是否正确。
...
- 同 businessKey 已存在更高版本但尚未 EFFECTIVE 时,旧 `asset_partition` 不能把下游判为 ready。
