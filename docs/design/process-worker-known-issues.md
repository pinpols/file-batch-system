...
`SqlTransformComputePlugin.commit()` 发布 target，`FeedbackStep` 再调用 `plugin.feedback()` 删除 staging。`AbstractStageExecutor` 没有事务包裹整个 stage loop，两个阶段是独立 SQL 调用。`FeedbackStep` 还会捕获并抑制 runtime exception，以免 target 已发布后把任务标失败。
...
2. 同时实现 P1-6 的 orphan cleanup scheduler，作为回退。
3. 文档改写失败语义：VALIDATE 失败保留 staging 是设计，COMMIT 成功后的 staging 残留是异常，需要自动清。

**验收**

- 已完成：`COMMIT` 在业务库事务中执行 publish + cleanup；`FEEDBACK` 保持幂等清理回退。后续仍建议补 orphan cleanup scheduler 处理历史残留。
...
`buildStagingInsertSql` 是无上限的 `INSERT INTO staging SELECT ... FROM (<sourceSql>)`。配置 SQL 写错、笛卡尔积或数据激增时，会直接耗尽业务库和 staging 表资源。
