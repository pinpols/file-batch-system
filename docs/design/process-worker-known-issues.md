...
- 生产部署文档不再出现“staging 在 platform 但代码走 business”的矛盾。
- 双库 E2E 通过。
...
- 单测构造相同 `batch_key`、不同 tenant 的 staging 行，验证 COMMIT/FEEDBACK 只处理本 tenant。
...
- 单测或集成测试模拟 `feedback()` 抛异常，确认不会无限遗留已发布 staging，或 orphan cleaner 可清除。
...
- 测试在 COMPUTE 后插入更高 watermark 源数据，确认上报的 watermark 仍来自本批 staging。
