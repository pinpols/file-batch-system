...
> 到达组「成员都正确」由各成员各自的 MANIFEST 校验保证（一文件一 `.chk`）。组判定默认按 `requiredFileSet` 文件名凑齐触发；开 `file-governance.arrival.require-verified=true` 后，**还要求每个成员都已完整性背书**（`checksum_type != NONE`），否则文件名虽齐也保持 `WAITING_ARRIVAL`（reason `ARRIVED_PENDING_VERIFY`），超时仍走 `timeout-action`。
...
- 累加范围 = 到达的全量数据记录(含将被判无效的行),与上游 trailer「发出的总额」口径对齐;JSON 反序列化失败的畸形行无法取金额,不计入(由笔数校验 P1.1 兜底)。
