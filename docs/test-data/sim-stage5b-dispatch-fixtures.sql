...
--
-- 这俩 fixture 在单机由 created_by='sim-e2e' 的外部 seed 建立,但常驻分支
-- (feature/partition-readiness) 的 docs/test-data 漏带定义文件,导致 14 的
-- preflight "missing TB_DISPATCH_STAGE5_FAIL_ONCE fixture" 失败。此处补为
-- self-contained(值取自单机 main batch_platform 的权威行,去 id/时间戳让默认)。
--
-- Citus:job_definition / file_channel_config 均 reference 表,INSERT VALUES +
-- ON CONFLICT 在协调器执行,双栈等价。
-- ============================================================================
