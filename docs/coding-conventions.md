...
- 生产代码中，`Command` / `Context` / `Param` 等承载业务语义的对象，不要直接写在另一个方法的实参位置；先以语义明确的局部变量接住，再传给目标方法。
- 这样做是为了让审查者能在调用前看到完整的业务边界，也便于后续增加校验、日志字段或断点，而不把复杂度隐藏在括号嵌套中。
- 简短的返回值工厂、构造器内部组合、枚举/注册表中的声明式配置，以及测试 fixture 可以保留 inline；安全扫描步骤目录中的 `ExternalCommand` 属于声明式配置，适用同一例外。
...
| sim / e2e fixture | `docs/test-data/*.sql` / JSON / YAML |
