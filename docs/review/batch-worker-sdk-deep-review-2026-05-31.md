...
- 依赖收敛到 4 个,无 Spring / 无 batch-common —— ADR-035 P0 项最难做对的事做到了。
...
- `register()` 用 `capabilityTags` 表达 `taskTypes` 是临时兼容方案(注释自承),平台早晚分两个字段;SDK 没准备好接 `taskTypes` 独立化。
...
- ❌ **`inFlight` set 跨进程不互通**:Worker A 长期停滞(GC / 长 Full GC)→ lease 超时 → orchestrator 派给 Worker B → A 恢复后继续跑 → **同 task 双进程并行**。SDK 当前没机制让 A 主动 abort handler(没传 cancellation token)。
...
| Orchestrator 服务降级 / 异常退出 | ❌ | SDK 继续消费,REPORT 失败堆积 |
| 平台全局限流 | ❌ | SDK 还按配置满载 |
| 租户被冻结 | ❌ | 仍在接活 |
...
- **但** 401(API Key 失效)/ 5xx 走同一路径 → 租户进程陷入"poll → claim 401 → 静默 → 下一条"安静无限循环,heartbeat 还在跑(平台以为活着)。
