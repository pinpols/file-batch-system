...
注释说 `partitionPlan.partitionNo` 为 null 要 fail-fast，但代码先拼字符串: `jobInstanceId + ":" + partitionPlan.getPartitionNo()`。当 partitionNo 为 null 时结果是 `"123:null"`，`Guard.requireText` 仍会通过，无法达到注释中的兜底效果。

修复: 先单独校验 `partitionPlan.getPartitionNo() != null`，再拼 idempotencyKey。

### 8. V136/V137 同批 VALIDATE 可能卡住已有脏数据环境
