...
| Quartz JobStore 单独库 | D | ❌ 不改（仅配置） | ✅ 完成 | `QuartzDataSourceConfiguration` + `QuartzDataSourceProperties`；`@QuartzDataSource` 切换；开关 `batch.trigger.quartz-datasource.enabled` |
| quota Redis token bucket | F | ✅ 改（替换实现） | ✅ **2026-04-25 完成** |

**Phase 2：5 项里 4 项要改代码** ✅✅✅✅(改)❌ — 全部交付，详见上表

**Phase 2 落地策略统一为 opt-in scaffolding**：所有开关默认关闭（`enabled=false` / `mode=SINGLE`），与历史行为一致；运维侧需要规模化时按需翻开关，避免一次性把基础设施依赖全部硬编码进生产链路。
