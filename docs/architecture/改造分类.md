...
|---|---|---|
| outbox 自动归档 | B | ❌ 不改 |
| file_record / job_instance 归档 | B + C | ❌ 不改业务代码（仅 DDL）|
| 配置缓存 Redis pub/sub 失效 | F | ✅ 改 |
| 完整观测三块板 | A | ❌ 不改（micrometer 已就位）|
| worker auto-restart | A | ❌ 不改 |

**Phase 1：5 项里只有 1 项要改代码** ✅✅✅✅✅(改)
**预估工作量**：1 人 2-3 周（80% 是 SQL 脚本 + 运维配置 + 仪表盘）

### Phase 2 — 百万 → 千万（5 项）

| 项 | 类型 | 改代码？ |
|---|---|---|
| Read replica | D + E | ✅ 改（DataSource routing） |
| WorkerSelector 加 Redis cache | F | ✅ 改 |
| Kafka topic 按租户/优先级分 | D + F | ✅ 改（producer/consumer 分流） |
| Quartz JobStore 单独库 | D | ❌ 不改（仅配置） |
| quota Redis token bucket | F | ✅ 改（替换实现） |

**Phase 2：5 项里 4 项要改代码** ✅✅✅✅(改)❌
**预估工作量**：1-2 人 4-6 周（都是替换/扩展现有组件，不重写架构）
...
| workflow archive 自动化 | B + F | ✅ 改（archive 逻辑）+ ❌（cron）|
...
- ✅ Quartz JobStore 单独库（D）

**5 项全部不动业务代码**。1 人 1-2 周。能把"无意识技术债"挡住。
