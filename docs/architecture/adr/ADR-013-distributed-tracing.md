...
落地前已有：业务字段 `trace_id` 透传（V77/V78）；Kafka `setObservationEnabled(true)`；Spring Boot 4.x `ObservationRegistry`；`deploy/docker/compose/observability.yml`（Collector / Tempo / Jaeger / Loki / Grafana）。
...
- **推荐手工验收**：`docker compose -f docker-compose.yml -f deploy/docker/compose/observability.yml up -d`，触发一次 launch，在 Jaeger/Tempo 查 `orch.launch` 是否串联 Kafka send/receive
...
| 日期         | 演化                                                                                                                      |
| ---------- | ----------------------------------------------------------------------------------------------------------------------- |
| ~2026-04   | deploy/docker/compose/observability.yml 落地（Tempo/Jaeger/Loki/Grafana/OTel Collector）                                           |
