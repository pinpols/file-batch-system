...
| 046 | [ADR-046-file-bundle-aggregation.md](./ADR-046-file-bundle-aggregation.md) | 文件束聚合:在单文件单元之上加一层批量编排,低千~上万 fan-out(多表导入/多下游导出)下把控制面 churn O(N)→O(N/K),束包住独立 file-unit(两层幂等/部分失败/文件级 checkpoint),三根墙不破。**配置层**:1 束作业(BUNDLE_IMPORT/DISPATCH)+ M 模板(批量导/可复用)+ 提交 manifest,**不 per-表配 job**。已拍板:扩展 partition(异构 file-unit)/ PARTIAL_FAILED 独立重试 / request-scoped 分组+大小上限 / Phase1 先行（Proposed,方向已定） |
