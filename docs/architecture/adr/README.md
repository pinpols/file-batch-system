...
| 046 | [ADR-046-file-bundle-aggregation.md](./ADR-046-file-bundle-aggregation.md) | 文件束聚合:低千~上万 fan-out(多表导入/多下游导出)下把控制面 churn O(N)→O(N/K)。**轻量版(评审收敛)**:束=一个 job_instance + K 个**异构且各自独立**的 partition;省 churn 靠 **claim/report 多行批量**,**不加束状态机/不加 worker 束执行/不新表**——partition 三根墙原样,纯加法。**配置层**:1 束作业(BUNDLE_IMPORT/DISPATCH)+ M 模板(批量导/可复用)+ 提交 manifest,**不 per-表配 job**。Phase1 配置收口+launch批量先行,Phase2 多行 claim/report 命中上万再上（Proposed,方向已定） |
