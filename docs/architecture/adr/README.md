...
| 046 | [ADR-046-file-bundle-aggregation.md](./ADR-046-file-bundle-aggregation.md) | 文件束聚合:在单文件单元之上加一层批量编排,低千~上万 fan-out(多表导入/多下游导出)下把控制面 churn O(N)→O(N/K),束包住独立 file-unit(两层幂等/部分失败/文件级 checkpoint),三根墙不破;倾向扩展 partition;Phase1 操作批量化先行（Proposed） |
