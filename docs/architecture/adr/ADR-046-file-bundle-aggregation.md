...
1. **Phase 0 — 设计定稿**:✅ 已拍板(见 §已决)+ §配置模型。
2. **Phase 1 — 配置收口 + launch/outbox 批量化(低风险先行)**:新增 `BUNDLE_IMPORT` 束作业 + 提交 manifest(配置 N→1);launch 一次展成 N 个异构 partition + outbox 批写。**运行时仍 N 个独立 partition**,claim/report 暂不动。验证低千量级是否已够(很可能够)。
3. **Phase 2 — 多行 claim / 多行 report(命中上万再上)**:把 claim/report 改成一次领/报 K 个独立 partition,控制面往返 O(N)→O(N/K)。**仅此一项优化,partition 语义/状态机/worker 执行全不变**;无束状态机、无 worker 束循环。Phase 1 实测在上万仍撞墙才上。
4. **Phase 3 — export/dispatch 侧同构**:`BUNDLE_DISPATCH` 束作业,每 partition 生成自己的文件 + per-下游回执。
