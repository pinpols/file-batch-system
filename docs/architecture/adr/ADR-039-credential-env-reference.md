...
- 入口同 #242:`workflow_definition` / `task_param` JSON 写入数据库前
...
1. **schema 维护负担**:新 taskType 必须在 schema 标注 `x-credential`,漏标 = 漏检 —— 用 PR 模板 + reviewer checklist 回退,加 schema lint 守护
