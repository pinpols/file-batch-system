...
如果未来满足上述条件并重新启动 Phase 1，时间轮内核必须优先采用经过生产验证的开源实现（例如 Netty `HashedWheelTimer`），不得在本项目内自研时间轮算法或重新实现调度内核。项目自身只负责适配调度语义、持久化事实、租户隔离、幂等、misfire、failover、观测和灰度切换；只有在开源实现无法满足已验证的业务约束时，才允许提交单独 ADR 评估替代方案。
