...
- `PipelineCompensationHook`:补偿动作**成功反向删业务行(`REVERSED`)后**才清该实例全部 stage 位点;`SKIPPED`(未能安全 scope,业务行原样保留)/ `FAILED`(反向出错,状态不明)**不清位点**(保持可诊断)。时序:先补偿删业务行、成功后再清位点。
- 该组合(compensate_on_failure × checkpoint)经本补丁后**安全**;补偿成功清位点会落 `pipeline compensation cleared checkpoint positions after reverse` info 日志作为运维信号。
