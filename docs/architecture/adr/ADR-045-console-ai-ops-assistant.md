...
- **背景**: 2026-06-21「形式化落地」体检维度9(边界纪律)指出 Console AI 模块默认关、只读、隔离在控制面,够不上「越界」,但够得上「为体验加的、占维护成本(~850 行 + 2 个 LLM SDK 依赖)、缺 ADR 背书」的轻度红旗。本 ADR 补齐背书,把它的定位和边界固化,消除「来路不明的 demo 模块」疑虑。
