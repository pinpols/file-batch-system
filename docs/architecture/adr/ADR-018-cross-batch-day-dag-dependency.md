...
1. **多 workflow 嵌套**：workflow A 的输出被 workflow B 跨日消费，是否要做 workflow → result_version 一等公民映射？倾向"workflow 整体 SUCCESS 时由 orchestrator 写 result_version 的 `business_key=workflow:{wfCode}:{bizDate}`"，与 job 维度共用主模型；
2. **依赖图可视化**：跨日依赖在 console 怎么展示？拓扑里塞 T-N 节点会爆炸，倾向"依赖列表 + 抽屉式展开"，UI 设计单独跟进；
3. **bizDateRange 上限**：`PREV_30_BIZ_DAYS` 之类是否需要硬上限防滥用？倾向限 90 天，超出走业务定制 resolver；
4. **circular dependency 检测**：跨日依赖天然不形成环（时间单向），但 OPTIONAL 配错可能让节点跳过依赖却写入下一日依赖项，导致传染性退化。需要 graph validator 在 workflow 启用时检查。
