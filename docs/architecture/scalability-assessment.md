...
当前 5 个主 topic（`batch.task.dispatch.{import|export|process|dispatch|atomic}`）+ 每 worker 节点的 `.node.<worker_code>` 子 topic 已经够用。海量场景需要：
...
| **海量** | > 1 亿 | 🔴 当前架构无法限制 | 需要分库分表 + 多 Kafka 集群 + 观测中台 |
