...
> **架构基线**：本文档描述系统的 As-Is、To-Be 和差距清单。涉及版本、模块数量、迁移版本、端口和部署状态的事实，必须以当前根 `pom.xml`、部署文件、迁移目录和最新验证记录复核；旧日期段落不能单独作为当前结论。
> 核心名词的统一定义请同时参考 [core-model.md](./core-model.md)。
> 最后全面核查：2026-04-09（历史基线）；当前待办和状态以 [`../analysis/todo-master.md`](../analysis/todo-master.md) 为准。
