...
状态：**已完成门禁收紧，存量继续分批治理**。`check-sql-config-boundaries.sh` 已取消整文件放行，改为按文件登记可执行 SQL 命中行数预算；纯注释、日志展示和 HTTP curl 不计为 SQL。首轮已把 atomic sim 的动态查询迁入 `scripts/sim/sql`，当前 9 个历史文件只能减少、不能增加，未登记文件预算为 0；只保存检测表达式且不执行 SQL 的门禁自身明确豁免。现有压测探针、sim reset/quiesce、SDK E2E seed/断言 SQL 仍应继续迁到 `load-tests/sql`、`scripts/*/sql` 和 `docs/test-data`，但后续无法借历史文件白名单继续扩张。
...
6. ~~将 shell 整文件白名单改成只减不增基线~~（已完成）；继续逐批清理剩余 9 个历史文件。
