...
- 将跨多个上下文复用且无业务方法的 Ops 只读响应 DTO 移入 `shared.view`，将跨 Job/Ops 使用的补跑审批请求移入 `shared.command`。
- 不改变 JSON 字段、REST 路由、SQL、分页、租户过滤、事务和状态机语义。
- 跨域直接依赖降至 **974**，本批减少 **146 条**；ratchet 已下调至 974。

第十七批完成后：

- 将编排器代理和触发器代理提升为顶层应用端口，跨域调用方不再依赖 Ops 应用接口；报表导出复用编排器查询端口，不再直接构造内部 HTTP Client。
- HTTP/RPC 适配器、下游熔断、缓存、错误映射和实时刷新仍由 Ops 基础设施实现，业务执行语义不变。
- 跨域直接依赖降至 **927**，本批减少 **47 条**；ratchet 已下调至 927。

第十八批完成后：

- 将跨 Job、治理和 Ops 使用的命令请求 DTO 移入 `shared.command`，保留请求字段、校验、序列化和 REST 路由不变。
- 跨域直接依赖降至 **893**，本批减少 **34 条**；ratchet 已下调至 893。

第十九批完成后：

- 将编排器和触发器内部 HTTP Client 适配器移入 `shared.client`，调用方不再依赖 Ops 基础设施实现；认证、熔断、缓存、错误映射和下游协议不变。
- `file -> ops` 依赖降至 0，跨域直接依赖降至 **875**，本批减少 **18 条**；ratchet 已下调至 875。

第二十批完成后：

- 新增 `ConsoleJobOperationsPort`，Job 的审批、补偿、恢复和触发服务只依赖应用层端口；审批客户端、内部 HTTP、实时事件和事务实现仍由 Ops 组件持有。
...
- 将 Ops 只读查询服务和聚合 Mapper 提升到 `application.ops`，因为它们负责控制台跨域只读投影编排，而不是 Ops 状态机或写模型。
- 保留 Ops 自有查询、各领域 Mapper、分页、租户过滤、错误渲染和查询结果语义；跨域直接依赖降至 **199**，本批减少 **173 条**。
