...
| **dast** | OWASP ZAP baseline (docker) | OK | 147s | 1 High + 1 Med + 2 Low + 7 Info |
...
## 6. DAST — ZAP baseline (1 High + 1 Med + 2 Low + 7 Info)

打的是 `http://host.docker.internal:18080`（本机 console-api）。
...
详细 alert 名 / 实例位置见 `target/security-scan-report/zap-report.html`（ZAP HTML 复杂结构未自动提取）。

**典型 ZAP baseline 高危覆盖项**：CSP header 缺失 / X-Frame-Options / Cookie HttpOnly+Secure / Mixed content / 关注 console-api 是否有 CSP + HSTS。
...
7. **ZAP High 告警**核查（看 HTML 报告确认是哪条）
