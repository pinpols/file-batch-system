...
- 默认 `BATCH_PG_CLIENT_MODE=auto`：依次尝试宿主机 `psql`、Python `psycopg`、Docker 容器内 `psql`。
- `BATCH_PG_CLIENT_MODE=host`：只使用宿主机客户端；缺失时立即失败。
- `BATCH_PG_CLIENT_MODE=python`：只使用 Python `psycopg` fallback，依赖 `scripts/requirements-postgres.txt`。
- `BATCH_PG_CLIENT_MODE=docker`：使用 `PG_CONTAINER` 内的 `psql`，不依赖宿主机安装 PostgreSQL 客户端。
- 也可以使用 `BATCH_PSQL_BIN=/path/to/psql` 指定宿主机客户端。

宿主机、Python fallback 和 Docker 均不可用时，依赖 SQL 的脚本必须失败并提示环境问题，不能跳过 SQL 后继续报告成功。
...
| 数据初始化 | 支持 | 支持 | 支持 | 支持 | 支持 Python `psycopg` 或 Docker 客户端模式 |
| Load Test | 支持 | 支持宿主机客户端或 Python fallback | 支持 | 支持宿主机客户端或 Python fallback | 支持 Python `psycopg` 或 Docker 客户端模式 |
| 运维巡检/自愈 | 支持 | 支持远程 PG | 支持 | 支持远程 PG | 支持 Python `psycopg` 或 Docker 客户端模式 |
...
3. SQL 文件本身不是可执行程序；执行 SQL 必须依赖宿主机 `psql`、Python `psycopg` 或容器内 `psql`。
