...
- **15.5% 组件带 aria 标签** — 顶栏/侧栏头部覆盖较好,但大量 form / 列表 / 卡片缺 aria-label / role,屏阅读器路径裸奔
...
- ✅ **mobile 现在跟 html.dark 同步**(`mobile-common.css:42` 注释"原先只听 prefers-color-scheme,用户手动切浅色但系统是暗色时表现错乱")— 这是历史已踩坑后的修复
...
- 顶栏(LayoutHeader)icon-only 按钮**全部带 `aria-label`** 且包 `el-tooltip` — 这一块做得很好
...
| **P1-4** | aria-label / role 覆盖率仅 15.5%(25/161),业务页屏阅读器路径裸奔 | 全站 .vue |
...
- ✅ mobile 已踩过"只听 prefers-color-scheme"的坑并修复(注释保留 — 防回归)
