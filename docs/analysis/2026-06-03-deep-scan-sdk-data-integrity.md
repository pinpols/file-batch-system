...
orchestrator 实际 `@PostMapping` 见 `batch-orchestrator/src/main/java/io/github/pinpols/batch/orchestrator/controller/{Worker,Task}Controller.java`,openapi 见 `docs/api/orchestrator-internal.openapi.yaml`。
...
源:`batch-worker-sdk/src/main/java/io/github/pinpols/batch/sdk/wire/*.java` vs Python `dispatcher/dispatcher.py` + `scheduler/_heartbeat.py` + `client/client.py` 构建的 body dict。
...
`batch-worker-sdk/src/main/java/io/github/pinpols/batch/sdk/scheduler/HeartbeatScheduler.java`(经 `BatchPlatformClient.buildHeartbeatBody` 构造)与 Python `_heartbeat.py` L142-148 形态差异表:
...
- `batch-orchestrator/src/main/java/io/github/pinpols/batch/orchestrator/infrastructure/archive/ArchiveSchemaDriftCheck.java`(P1-4 NOT_ARCHIVED 注释)
- `batch-worker-sdk/src/test/java/io/github/pinpols/batch/sdk/contract/SharedConstantsParityTest.java`(P1-5 顺序断言)
