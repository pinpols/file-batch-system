...
- 类似上,但 webhook 是对外承诺的回调,丢失更严重。已有 `WebhookDeliveryRelay` 回退,但需确认 relay 路径覆盖 AbortPolicy 的丢失场景。
...
✅ 已有上限,流式开启。Import `ReceiveStep` 还做 heap-ratio 回退(`Runtime.maxMemory * heapRatio` cap)。
...
✅ 多档退避覆盖完整,worker-side 48 次 × 5min 给了 worker → orchestrator REPORT 大约 4 小时窗口的回退。
...
| P2-6 | Thread | WebhookDispatcher AbortPolicy 需复核 relay 是否回退 |
