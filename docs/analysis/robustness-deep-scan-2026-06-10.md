...
| 4 | 并发·OOM | OSS dispatch `in.readAllBytes()` 无上限,GB 级文件 + 多路并发 → OOM(import 侧有 `MAX_OBJECT_BYTES` 防护,此处缺) | `RemoteFilesystemDispatchSupport.java` | bounded `readNBytes(limit+1)` 检测超限拒绝(默认 512MiB,由 `batch.worker.dispatch.runtime.oss-max-inline-mib` 配置),不把整文件读进堆 |
