...
| A-3.4 | B | WorkflowDagService JOIN 边删后死循环 | ✅ 已修 | currentNode null 立即 return false；非-START 空入边加 WARN 日志 |
...
| R-4.1 Redis 挂了 500 | 假设 Redis HA；fail-open/fail-closed 需产品决策 |
...
**修复方向**：分片 + 任务放同一事务的同一 UPDATE 链，或用 select-for-update 锁行；禁止"内存回滚"这种兜底。
...
`resolveToState` default case `return fromState`，拼写错的事件名（`SUCESS` vs `SUCCESS`）被吞掉，任务卡错态难排查。

#### A-3.4 DefaultWorkflowDagService JOIN 终态判定与入度脱钩

**文件**：`batch-orchestrator/.../DefaultWorkflowDagService.java:96-115`

`incomingEdges.size()` 为真 DB 查询，若 workflow_edge 软删 / 定义变更后缺边 → JOIN ALL 模式永远判不出满足，孤立节点死循环。
...
| 位置 | 现象 |
|---|---|
| C-2.3 `DefaultPartitionLifecycleService:151-177` | 分片 → 任务跨事务 + 内存回滚兜底 |
...
2. **R-4.1 Redis 异常兜底** —— 限流 fail-open / 幂等 fail-closed，明确写文档
