# 硬化与遗留问题 Backlog —— v6 归档快照（2026-06-15 封存）

> **这是 v6 的完整封存快照（含已完成清账）**。活清单已滚动到 v7：[`docs/analysis/hardening-backlog.md`](../../analysis/hardening-backlog.md)。
> 2026-06-15 归档时核实：v6 的 🟡 项多数已落地（DBA-P0-1/P0-2 分区由 V172/V173 落地；OPS-1 由 `scripts/ci/check-env-prod-sync.sh` 落地；POSITIONAL-ARGS 由 `PositionalArgsConventionTest` 守护落地），仅 3 项真未决滚入 v7。
> 本文件只读，不再更新。
>
...
- **背景**：docs/agent-baseline.md "方法参数 ≥7 必须封装" 第一阶段落地后，参数臃肿从方法签名搬到 `new XxxParam(a,b,...,n)` inline 调用，留下 main 反例
...
- **规约同步**：docs/agent-baseline.md §方法参数约束 追加"调用方约束"子节 + `docs/changelog.md` 2026-05-01 条目 + 守护 `PositionalArgsConventionTest` 白名单方式拦回潮
