...
| 编号 | 缺口 | 影响 | 建议方案 | 优先级 |
|------|------|------|---------|--------|
| 5.1 | ~~**无跨租户复制**~~ | ~~新租户入驻最高频场景——从"模板租户"复制一套标准配置到 N 个新租户，目前必须手工拼完整 JSON~~ | ✅ 已实现 `POST /api/console/config/tenant-copy`（`ConsoleTenantConfigCopyService`） | ~~高~~ ✅ |
| 5.2 | ~~**无 dry-run 验证**~~ | ~~batch-init 没有预检，失败只能从响应 ItemStats 里看，大批量推送前无法评估影响~~ | ✅ 已实现 `dryRun: true` 请求字段，所有写操作由 `if (!dryRun)` 守卫 | ~~高~~ ✅ |
| 5.3 | ~~**响应只有计数，没有明细**~~ | ~~运维不知道具体哪些 code 被 created / skipped / failed~~ | ✅ 已实现 `ItemDetail(code, action, errorMessage)` + `ItemStatsAccumulator` | ~~中~~ ✅ |
| 5.4 | **Excel 不支持多租户** | 每次 upload 绑定当前登录租户，不能一份 Excel 推多租户 | 不改 Excel 流程；多租户场景统一走 batch-init API，Excel 只用于单租户微调 | 低（方案已明确） |
| 5.5 | ~~**无批次关联审计**~~ | ~~同一次 init 产生的 change-log 之间没有关联 ID，无法追溯某批操作全貌~~ | ✅ 已实现 Controller 生成 UUID `batchOperationId`，透传到响应 | ~~中~~ ✅ |
