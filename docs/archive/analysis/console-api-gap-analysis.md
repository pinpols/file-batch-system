...
## P5 — 多租户配置维护缺口（5.1–5.3 & 5.5 已实现）

> 日期：2026-04-09

### 现状

`POST /api/console/config/tenant-init` 已可用（`ROLE_ADMIN`），支持一次向最多 50 个租户推送 5 类配置（Job Definition / Workflow Definition / Pipeline Definition / File Channel / File Template），模式 `SKIP_EXISTING`（默认）或 `UPSERT`。单租户 Excel 维护（upload → preview → apply → export）覆盖 file-template、file-channel、workflow、job-definition 四类。

### 缺口清单

| 编号 | 缺口 | 影响 | 建议方案 | 优先级 |
|------|------|------|---------|--------|
| 5.1 | **无跨租户复制** | 新租户入驻最高频场景——从"模板租户"复制一套标准配置到 N 个新租户，目前必须手工拼完整 JSON | 新增 `POST /api/console/config/tenant-copy`，入参：`sourceTenantId` + `targetTenantIds` + `configTypes[]` + `mode`；后端读源租户配置 → 转 spec → 调 batch-init 逻辑 | **高** |
| 5.2 | **无 dry-run 验证** | batch-init 没有预检，失败只能从响应 ItemStats 里看，大批量推送前无法评估影响 | 增加 `dryRun: true` 请求字段或独立 `POST /api/console/config/tenant-init/validate` 端点，返回 would-create / would-update / would-skip 计数和冲突明细 | **高** |
| 5.3 | **响应只有计数，没有明细** | 运维不知道具体哪些 code 被 created / skipped / failed | `TenantInitResult` 增加 `details: List<ItemDetail>`（configType + code + action + errorMessage） | **中** |
| 5.4 | **Excel 不支持多租户** | 每次 upload 绑定当前登录租户，不能一份 Excel 推多租户 | 不改 Excel 流程；多租户场景统一走 batch-init API，Excel 只用于单租户微调 | 低（方案已明确） |
| 5.5 | **无批次关联审计** | 同一次 init 产生的 change-log 之间没有关联 ID，无法追溯某批操作全貌 | audit log 增加 `batchOperationId` 字段，batch-init 生成 UUID 写入所有相关 change-log | **中** |
| 5.6 | **init 和 config-release 流程割裂** | init 只写原始配置，不会触发发布；运维需额外手动创建 release → publish | 可选：init 请求增加 `autoRelease: boolean`，成功后自动创建 release draft 或直接 publish | 低 |
...
```json
{
  "sourceTenantId": "tenant-template",
  "targetTenantIds": ["tenant-new-1", "tenant-new-2"],
  "configTypes": ["JOB_DEFINITION", "WORKFLOW_DEFINITION", "PIPELINE_DEFINITION", "FILE_CHANNEL", "FILE_TEMPLATE"],
  "mode": "SKIP_EXISTING",
  "codeFilter": {
    "jobCodes": ["daily-settlement", "file-import-*"],
    "workflowCodes": null,
    "pipelineCodes": null,
    "channelCodes": null,
    "templateCodes": null
...
- `configTypes` 为空则复制全部 5 类。
- `codeFilter` 为空则复制该类所有配置；支持精确 code 列表或通配符。
- `mode` 语义同 tenant-init：`SKIP_EXISTING` / `UPSERT`。
- 权限：`ROLE_ADMIN`。
- 实现：后端读源租户配置 → 转为 Spec 列表 → 复用 `ConsoleTenantConfigInitApplicationService` 逻辑。
- 响应复用 `TenantConfigBatchInitResponse`。

### 5.2 dry-run 验证

在 `TenantConfigBatchInitRequest` 增加 `dryRun` 字段（默认 `false`）：

- `dryRun=true` 时走完全部查询和校验逻辑，但不执行 insert/update/delete。
- 响应结构不变，ItemStats 反映 would-create / would-update / would-skip / would-fail 计数。
- 5.3 的明细列表在 dry-run 模式下尤其有价值。
