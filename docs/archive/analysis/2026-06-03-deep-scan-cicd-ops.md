...
step `exit 0` 硬写,后面再加 `continue-on-error: true`,**双重回退等于永不挂红**。
...
**为何仍 P0**:工作流本身 OK,但**容易让人误以为有回退**。`scripts/local/strict-verify.sh`
...
| full-ci-gate | ✓ (paths-ignore docs) | — | — | — | ✓ | main push 回退 unit+IT+e2e |
