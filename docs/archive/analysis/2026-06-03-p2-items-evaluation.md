...
- 读写分离 docs/agent-baseline.md 明定**仅 `batch-console-api`**;`batch-trigger` / `batch-orchestrator` / `batch-worker-*` 禁引入(状态机依赖 read-after-write 强一致)。
...
## P2-3:RoutingHints + `@Async` 互动 ADR + docs/agent-baseline.md — **不做**
...
- docs/agent-baseline.md 是高频违反红线集中地,加这条会稀释信号噪音比。
...
| P2-3 RoutingHints × @Async ADR | 不做 | 无真实 conflict surface,立规矩稀释 docs/agent-baseline.md 信号 |
