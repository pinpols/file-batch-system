...
- `batch-trigger/src/main/java/io/github/pinpols/batch/trigger/config/TriggerSecurityConfiguration.java:41` 定义安全链。
- `batch-trigger/src/main/java/io/github/pinpols/batch/trigger/config/TriggerSecurityConfiguration.java:45` 将非 actuator 请求设为 `authenticated()`。
- `batch-trigger/src/main/java/io/github/pinpols/batch/trigger/config/TriggerSecurityConfiguration.java:86` 读取 `X-Internal-Secret`。
- `batch-trigger/src/main/java/io/github/pinpols/batch/trigger/config/TriggerSecurityConfiguration.java:93` 密钥缺失或错误直接 401。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/query/ConsoleJobOpsSupport.java:98` 构造 Trigger `RestClient`。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/query/ConsoleJobOpsSupport.java:101` 调 `POST /api/triggers/launch`，只带幂等键、requestId、traceId。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/job/DefaultConsoleJobApprovalService.java:149` 构造 Trigger `RestClient`。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/job/DefaultConsoleJobApprovalService.java:152` 调 `POST /api/triggers/catch-up/approve`，同样缺内部密钥。
...
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/config/DefaultConsoleCalendarApplicationService.java:121` 解析出 `tenantId`。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/config/DefaultConsoleCalendarApplicationService.java:129` 到 `:134` 构造导入行，但未写 `tenantId`。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/config/DefaultConsoleCalendarApplicationService.java:138` 调用 `holidayMapper.batchInsert(list)`。
...
- 同步补单条 `insert` 的 `tenant_id` 支持，避免未来复用单插路径再次遇到问题。
...
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/config/DefaultConsoleCalendarApplicationService.java:144` 进入更新节假日。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/config/DefaultConsoleCalendarApplicationService.java:146` 校验 calendar 属于租户。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/config/DefaultConsoleCalendarApplicationService.java:148` 仅按 `holidayId` 查询 holiday。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/config/DefaultConsoleCalendarApplicationService.java:156` 仅按 `holidayId` 更新。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/config/DefaultConsoleCalendarApplicationService.java:163` 进入删除节假日。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/config/DefaultConsoleCalendarApplicationService.java:167` 仅按 `holidayId` 删除。
...
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/web/ConsoleWorkflowDefinitionController.java:80` 暴露 `GET /api/console/workflow-definitions/{id}/mermaid`。
...
- `batch-common/src/main/java/io/github/pinpols/batch/common/kafka/BatchTopics.java:23` 定义 `VERIFIER_FAILURE_V1`。
- `batch-orchestrator/src/main/java/io/github/pinpols/batch/orchestrator/infrastructure/mq/KafkaOutboxPublisher.java:130` 到 `:135` 将 verifier failure 发送到专用 topic。
- `batch-orchestrator/src/main/java/io/github/pinpols/batch/orchestrator/infrastructure/mq/KafkaOutboxPublisher.java:211` 到 `:215` 映射 `verifier.failure.v1`。
...
- `batch-common/src/main/java/io/github/pinpols/batch/common/config/BatchRestClientAutoConfiguration.java:38` 到 `:41` 声明 prototype builder。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/ops/OrchestratorInternalRestClient.java:34` 持有 builder 字段。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/ops/OrchestratorInternalRestClient.java:51` 到 `:63` 每次 build 都 mutate 同一字段实例。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/ops/DefaultConsoleTriggerProxyService.java:25` 持有 builder 字段，`:31` 到 `:35` mutate。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/query/ConsoleJobOpsSupport.java:65` 持有 builder 字段，`:98` 到 `:99` mutate。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/infrastructure/job/DefaultConsoleJobApprovalService.java:42` 持有 builder 字段，`:149` 到 `:150` mutate。
- `batch-worker-core/src/main/java/io/github/pinpols/batch/worker/core/infrastructure/HttpTaskExecutionClient.java:49` 持有 builder 字段，`:469` 到 `:474` mutate。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/service/WebhookDispatcher.java:67` 持有 builder 字段，`:222` 到 `:231` mutate requestFactory。
...
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/web/request/auth/FrontendTelemetryRequest.java:10` 到 `:20` 定义请求体。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/web/ConsoleTelemetryController.java:29` 接收 telemetry。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/web/ConsoleTelemetryController.java:52` 到 `:59` 直接将 props 序列化进日志。
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/support/ratelimit/ConsoleRateLimitFilter.java:57` 到 `:82` 目前限流仅覆盖登录和触发类路径，不覆盖 telemetry。
...
- `batch-console-api/src/main/java/io/github/pinpols/batch/console/web/ConsoleEventCatalogController.java:52` 到 `:61` 构造 topic 列表。
- `batch-common/src/main/java/io/github/pinpols/batch/common/kafka/BatchTopics.java:7` 定义 `TASK_DISPATCH_PROCESS`。
- `batch-common/src/main/java/io/github/pinpols/batch/common/kafka/BatchTopics.java:16` 定义 `TRIGGER_LAUNCH_V1`。
- `batch-common/src/main/java/io/github/pinpols/batch/common/kafka/BatchTopics.java:23` 定义 `VERIFIER_FAILURE_V1`。
