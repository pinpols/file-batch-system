...
> 现状:发布**管线已搭好**(`.github/workflows/sdk-release-validation.yml` 全 5 语言 conformance gate → breaking-change gate → package+smoke → supply-chain → gated publish;`sdk-python-publish.yml` 单独的 PyPI 通道)。Java groupId 已是真实命名空间 `io.github.pinpols.batch`,**仅 TypeScript 包名 `@batch/worker-sdk` 仍是占位**(registry 不接受未注册的 scope);且各 registry 凭据需仓库管理员配 secrets。本文件列清"从占位到真发"的待办。
...
| 语言 | 占位 | 改成(示例) | 位置 |
|---|---|---|---|
| Java | `io.github.pinpols.batch`(groupId,已是真实命名空间) | `io.yourorg.batch`(若改用自有命名空间) | 根 `pom.xml` `<groupId>`(级联所有模块);用自有命名空间时 Central 要求验证其所有权 |
...
| Go | `github.com/pinpols/file-batch-system/batch-worker-sdk-go` | 你的仓库实际路径 | `sdk/go/go.mod` module 路径 |

同时把 `sdk/java/core/pom.xml` 里发布元数据占位(`<url>` / `<scm>` / `<developers>` 的 `your-org`)改成真实值——**Maven Central 强制校验**这些字段。
...
`sdk/java/core/pom.xml` 的 `release` profile(默认构建不激活)接入:
- **flatten-maven-plugin** 覆写为 `ossrh` 模式——产出去父化(父 pom `io.github.pinpols.batch:batch-platform` 不在 Central)、含 Central 必填元数据、剥掉 test 依赖的**独立 pom**。
...
本地验证(不签名):
```bash
mvn -pl sdk/java/core -am -DskipTests -Prelease -Dgpg.skip=true package
# 产出 target/*-sources.jar、*-javadoc.jar,以及去父化的 .flattened-pom.xml
```

> ⚠ 真发前务必先做 §0.1:把根 `<groupId>` 从 `io.github.pinpols.batch` 改成自有已验证命名空间,否则 Central 上传会被拒。
