...
> sample 工程([`examples/sample-tenant-worker/`](../../examples/sample-tenant-worker/))走 builder 风格 + `BATCH_*` 短前缀,两种都行,二选一。
...
// 或 builder 风格,见 examples/sample-tenant-worker/.../SampleTenantWorker.java
```

`fromEnv()` 默认前缀 `BATCH_SDK_`,必填变量缺一即一次性报全;builder 风格示例(用 `BATCH_*` 短前缀)见 [`examples/sample-tenant-worker/README.md`](../../examples/sample-tenant-worker/README.md)。
...
- **Spring Boot 用户**:用 `batch-worker-sdk-spring-boot-starter`,configuration 走 `batch.worker-sdk.*` properties,handler 是 bean 即自动注册。示例:[`examples/sample-tenant-worker-spring/`](../../examples/sample-tenant-worker-spring/)。设计见 ADR-035 §1.1。
