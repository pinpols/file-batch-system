...
> 文档性质：历史实施计划，不是当前密码安全待办清单。表格中的 `todo` 是 2026-05-20～23 计划时点的占位状态；当前实现和未闭环事项必须以最新代码、前端仓及 [`../analysis/todo-master.md`](../analysis/todo-master.md) 复核。新事项不得继续追加到本文。
...
> 2026-09-21 决策更新：本节是历史方案，已不再执行强制跳转或写操作拦截。现行实现保留
> `must_change_password` 作为非阻断提醒标志；用户可继续使用控制台，自助改密后清除提醒。
