...
连续负载采样；任一项不符即停止。8 核本地环境允许 `load1 <= 20` 执行稳定性或过载取证，但只有连续
采样均不超过 6 的轮次才标记为可与标准历史基线比较。提高执行门槛不会放宽请求零错误、全终态和
环境可比性判据。报告同时记录 Docker、负载发生器
（OS/架构/CPU/内存/JDK）和实际运行容器集合的环境签名。换机器或升级 Docker/JDK 后可以运行，但必须
建立新容量基线；签名不同或被标记为不可比时，不能直接做跨环境性能增减结论。
