...
> **License 提示**:MinIO Server = AGPL v3(自建场景);AWS SDK for Java v2(`software.amazon.awssdk:s3`)= Apache 2.0;AWS S3 / 阿里 OSS / 腾讯 COS 是商业云服务无 license 问题。**本系统自托管 MinIO + 不改源码 + 用户不直连 → 无 license 风险**(同 Citus / Grafana / Loki / Tempo,license 风险结论一致:无)。
