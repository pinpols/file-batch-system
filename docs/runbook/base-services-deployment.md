...
| MinIO | `${MINIO_IMAGE_REPOSITORY}:${MINIO_IMAGE_TAG}` | 19000 / 19001 | 9000 / 9001 |
...
- `BATCH_S3_ENDPOINT` / `BATCH_S3_ACCESS_KEY` / `BATCH_S3_SECRET_KEY`
...
BATCH_S3_ENDPOINT=https://s3.us-east-1.amazonaws.com        # 或自托管 MinIO 的实际地址
...
BATCH_S3_ENDPOINT=http://minio.storage-system.svc:9000
...
BATCH_S3_ENDPOINT=http://localhost:19000
...
| **worker-import / export / process / dispatch / atomic** | **HPA/KEDA 已配置**，按 Kafka lag 或 CPU 自动伸缩 | Kafka partition 自动再平衡；新 Pod 加入 consumer group 自动分到任务；Atomic 仍须满足独立安全隔离 |
