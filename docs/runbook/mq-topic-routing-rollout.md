...
| `TENANT`（默认） | 多租户，存在大租户挤占小租户的风险 | topic 数 = 租户数 × 5（import/export/process/dispatch/atomic）；万级租户会让 broker 元数据膨胀 |
| `PRIORITY` | 业务有 HIGH/NORMAL/LOW 三档优先级且高优不能被低优阻塞 | topic 数 = 5 × 3 = 15（每个 worker 类型 3 个 priorityBand）；consumer group 需按 band 拆分配合才有效果 |
...
for TYPE in import export process dispatch atomic; do
...
docker exec batch-kafka kafka-topics --bootstrap-server localhost:9092 --list \
  | grep -E "batch.task.dispatch.(import|export|process|dispatch|atomic).(default-tenant|tenant-a|tenant-b)"
```
