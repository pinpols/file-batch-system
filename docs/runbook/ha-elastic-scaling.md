...
> **trigger 不依赖 Redis 锁**：trigger 模块的协调走 PG —— Quartz JobStore 用 `QRTZ_LOCKS`（`SELECT ... FOR UPDATE`），辅助调度（reconciler / forward-retry / cutoff 等）用 ShedLock JDBC（`batch.shedlock` 表）。Redis 故障**不影响** trigger 的集群协调；但会让 orchestrator 的 ShedLock 阻塞 / 退化（archive 调度器在 Redis 恢复前不会 acquire 锁，但因 `lockAtMostFor` 过期机制不会长期停滞）。
