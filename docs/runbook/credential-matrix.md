...
| 3 | **console 用户密码** | `console_user_account` 表,**Argon2id 哈希**(`ConsolePasswordHasher`,Spring Security 默认参数,含随机盐) | 建租户/建用户时录入明文 → 服务端 Argon2id 哈希写入数据库;首次登录/管理员重置后通过 `must_change_password` **非阻断提醒**用户自助改密(`POST /api/console/auth/change-password`) | `ConsoleDefaultPasswordGuard` 拦出厂默认弱口令;prod 不允许沿用默认 | 必配(admin + 各租户用户);批量建租户密码最低 12 位 | 平台管理员 / 租户管理员 |
