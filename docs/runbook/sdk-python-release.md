...
> 包名:`batch-worker-sdk`(PyPI distribution)
...
1. 登录 `https://pypi.org/manage/project/batch-worker-sdk/settings/publishing/`
2. **Add a new pending publisher**:
   - Owner:`pinpols`
   - Repository name:`file-batch-system`
   - Workflow filename:`sdk-python-publish.yml`
   - Environment name:(留空)
3. TestPyPI 同样配一遍:`https://test.pypi.org/manage/project/batch-worker-sdk/settings/publishing/`
...
`sdk/python/src/batch_worker_sdk/_version.py`:
...
```bash
git tag sdk-python-v0.5.0a0
git push origin sdk-python-v0.5.0a0
```
...
```bash
pip install batch-worker-sdk==0.5.0a0
...
完成后:
```bash
pip install -i https://test.pypi.org/simple/ batch-worker-sdk
```
...
1. `_version.py` → `0.5.0a1`
2. 修 + PR + merge
3. `git tag sdk-python-v0.5.0a1 && git push --tags`
4. PyPI 项目页手动 yank 旧版
...
|---|---|
| 仓内目录 | `sdk/python/` |
| pyproject distribution name | `batch-worker-sdk` |
| PyPI 项目名 | `batch-worker-sdk` |
| `pip install` 名 | `batch-worker-sdk` |
