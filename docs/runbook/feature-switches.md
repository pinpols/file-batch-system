...
| `batch.citus.enabled` | 全模块（batch-common） | **false** | **false** | 🟢 低 | `BATCH_CITUS_ENABLED`：true 时加载 `CitusRuntimeStartupCheck`，启动期 fail-fast 校验 coordinator 的 `propagate_set_commands=local` / `enable_unsafe_triggers=on` / 确是 Citus 库。仅 Citus 部署开启。**详见 [`citus-deployment.md`](./citus-deployment.md) §5** |
