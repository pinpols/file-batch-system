...
3. **导入有数量级数量级优化点(COPY,5-20×);导出没有**(瓶颈分散,只能 2-3×)。导出的横向加速(keyset 分片)已在 PR #393 落地。
...
所以严格说:**有"瓶颈现象",还不是"PG 单机写吞吐上限"的证据**。在跳到 Citus / 分库等重决策前,
**必须先把单机榨干**——这是决策树第 ① 步,绕不开。

可以肯定的混淆因素:
- worker JVM `-XX:+UseSerialGC -XX:TieredStopAtLevel=1`(低配快启档,GC + C1 only)
...
→ 真实 PG 单机写吞吐**未被测量**;数十万行/s 是常见量级,低配快启档配置吃不到。
...
- **有数量级数量级优化点吗**:✅ **有**——COPY 替代 batchUpdate 是 5-20× 量级,本章主线
...
不是 10× 数量级优化点。要让 COPY 真正吃满,需要进一步测更强形态:新建空 staging/新分区 → COPY → 建索引 → attach/swap,
...
| 阶段 | 跑什么 | 目标值 | 不达标后续 |
|---|---|---|---|
| Baseline 导入 | 现状(低配快启档 JVM + 500 chunk + 默认 PG + 单机) | 复现 ~182.5s | 基线锚点 |
...
- **有数量级数量级优化点吗**:❌ **没有**——导出本就是流式 keyset 游标分页,**单段优化最多 1.5-2×,无 10× 台阶**
...
| 数量级数量级优化点 | ✅ `batchUpdate` → 多值 INSERT → COPY,**5-20×** | ❌ 无等价物 |
...
| 阶段 | 跑什么 | 目标值 | 不达标后续 |
|---|---|---|---|
| Baseline 导出 | 现状(低配快启档 JVM + 默认 page/fetch + 单机) | 复现 ~160.6s | 基线锚点 |
...
- **K/R/O 总收益**:个位数百分比 ~ 几十百分比,**没有数量级提升**。属于"卫生"层级,不是"数量级优化点"层级。
- **真正的数量级优化点**在 §1(导入 COPY / 大 batch / JVM)与 §2.3(导出 keyset 分片已落)。
...
- 导入单流:**≈ 182.5s**(低配快启档 JVM + 500 chunk + 默认 PG)
- 导出单流:**≈ 160.6s**
