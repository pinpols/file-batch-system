...
| 2 | 23 import skip-threshold | worker-import 坏行容错阈值在 Citus 未按预期跳过校验(under_threshold 1 坏行应 SUCCESS) | 查 worker-import 容错阈值逻辑/配置(badRecordCount vs skipThreshold),确认 Citus 下行为 | 可能(worker 代码) | 中 |
...
| 4 | 25 checkpoint ON CONFLICT | checkpoint 模式 insertPipelineInstance 时 related_job_instance_id 为 null,Citus 对 partial-index(带 WHERE)ON CONFLICT 推断比单机严格 | worker-core: checkpoint 创建逻辑保证 related_job_instance_id 非 null / 或调整 uk 谓词;脚本 restart 已改 helper | 是(worker-core) | 中 |
| 5 | 14/20 dispatch | worker-dispatch 改 mapper 后重 build 的 jar 在 JDK25 AspectJ 类解析病态慢(10min+ 起不来) | 升级 AspectJ / 或编译期织入 / 或用旧 jar 热替 mapper.xml;circuit-breaker excluded bug 本身已修+SQL验证 | 是(worker-dispatch) | 中 |

**共性**:#2-5 都是 partition-readiness 的 worker/scheduler **业务逻辑** Citus 适配 + JDK25 AspectJ rebuild 工具链坑,非 SQL 方言/环境(那些已全部解决)。每项可独立成 PR。#1 最轻(脚本/配置)。
