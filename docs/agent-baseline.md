...
> **历史引用兼容：** 已发布的 Flyway 迁移必须保持字节不变，以免已部署环境出现 checksum
> mismatch。历史迁移注释中的 `CLAUDE.md` 只是冻结的旧文件名，其约束语义统一映射到本文件；
> 不得据此恢复 `CLAUDE.md` 或继续新增该引用。新代码、文档和迁移只能引用
...
- 是否继续遵守“配置进配置类、状态进枚举、核心常量不写硬编码字符串”
