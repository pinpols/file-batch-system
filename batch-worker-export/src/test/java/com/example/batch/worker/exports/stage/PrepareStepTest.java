...
class PrepareStepTest {
    ...
    void execute_parsesPayload_andLoadsTemplateConfig_whenTemplateCodeProvided() throws Exception {
        ...
        when(runtimeRepository.loadLatestTemplateConfig(eq("t1"), eq("TPL_1"), eq(ExportWorkerType.EXPORT)))
                .thenReturn(Map.of(
        ...
        assertThat(ctx.getAttributes().get(PipelineRuntimeKeys.TEMPLATE_CONFIG)).isInstanceOf(Map.class);
        assertThat(String.valueOf(ctx.getAttributes().get("exportFileFormatType"))).isEqualTo("DELIMITED");
        ...
        assertThat(ctx.getAttributes().get(PipelineRuntimeKeys.EXPORT_SNAPSHOT)).isInstanceOf(Map.class);
    }
    ...
    void execute_usesExistingExportPayloadFromAttributes_withoutJsonParse() {
        ...
        when(runtimeRepository.loadLatestTemplateConfig(any(), any(), any())).thenReturn(Map.of("file_format_type", "JSON"));
