...
class SqlTemplateExportSpecTest {
    ...
    void parse_shouldDefaultCursorToId() {
        SqlTemplateExportSpec spec = SqlTemplateExportSpec.parse(Map.of(
                "default_query_sql", "select id, name from t where tenant_id = :tenantId and batch_no = :batchNo"
        ), new ObjectMapper());
    ...
    void parse_shouldReadCursorFromQueryParamSchema() {
        SqlTemplateExportSpec spec = SqlTemplateExportSpec.parse(Map.of(
                "default_query_sql", "select id, name from t where tenant_id = :tenantId and batch_no = :batchNo",
                "query_param_schema", Map.of("sqlTemplateExport", Map.of("cursorColumn", "created_at"))
        ), new ObjectMapper());
