...
@SpringBootTest(classes = BatchTriggerApplication.class, webEnvironment = SpringBootTest.WebEnvironment.NONE)
class QuartzJdbcClusterIntegrationTest extends AbstractIntegrationTest {
    ...
    void shouldPersistQuartzJobAndTriggerIntoJdbcStore() throws Exception {
        ...
        try {
            ...
            Integer jobDetailCount = jdbcTemplate.queryForObject(
                    """
                            select count(*)::int
            ...
            jdbcTemplate.update("delete from batch.job_definition where tenant_id = ? and job_code = ?", tenantId, jobCode);
            jdbcTemplate.update("delete from batch.business_calendar where tenant_id = ? and calendar_code = ?", tenantId, calendarCode);
        }
    }
    ...
    private void insertCronJobDefinition(String tenantId,
                                         String jobCode,
                                         String calendarCode,
                                         String cronExpr) {
        jdbcTemplate.update(
    ...
    private int waitForSchedulerStateRows(String schedulerName, int expected) throws InterruptedException {
        int observed = 0;
        for (int i = 0; i < 20; i++) {
