 ...
 *
 * <p>测试通过把 next_fire_time 设到 misfireThreshold 之前,模拟"trigger 已经过期未 fire",
 * 然后触发 scanAndSchedule + 等待 fire,验证状态机走对。
 *
  ...
  private long insertJobDefinition(String catchUpPolicy) {
    long jdId = jdbcTemplate.queryForObject(
        ...
        insert into batch.job_definition (
          ...
          true, 'it', 'it')
        returning id
        """, Long.class, tenantId, jobCode, jobCode, "cal-" + tenantId);
    ...
    jdbcTemplate.update(
        ...
        """, tenantId, "cal-" + tenantId, "cal-" + tenantId, catchUpPolicy);
    return jdId;
  }
