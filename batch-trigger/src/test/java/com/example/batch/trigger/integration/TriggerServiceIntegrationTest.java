...
class TriggerServiceIntegrationTest extends AbstractIntegrationTest {
    ...
    void shouldPersistAcceptedRequestAndDeduplicateRepeatedLaunch() {
        when(orchestratorTriggerAdapter.sendTrigger(any())).thenReturn(new LaunchResponse("inst-001", "trace-001"));
        ...
        TriggerLaunchCommand command = new TriggerLaunchCommand(request, "idem-001", "req-001", "trace-001");
    ...
    void shouldApprovePendingCatchUpAndMarkRowLaunched() {
        ...
        when(orchestratorTriggerAdapter.sendTrigger(any())).thenReturn(new LaunchResponse("inst-002", "trace-pending-001"));
