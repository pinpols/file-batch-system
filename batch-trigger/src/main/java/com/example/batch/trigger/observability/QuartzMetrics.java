 ...
 *
 * <p>暴露给 Prometheus / Grafana 的 4 类指标，用于 Quartz 拐点预警（详见
 * {@code docs/architecture/quartz-replacement-evaluation.md} §7）：
 *
 * <ul>
 ...
 *   <li>{@code batch.trigger.quartz.triggers.active} — Gauge，当前 enabled trigger 数（由
 *       {@code QuartzMetricsConfiguration} 通过 {@link MeterRegistry#gauge} 直接注册，本类不持有）
 * </ul>
 *
 * <p>未暴露 {@code wal.byte.ratio}：该指标更适合通过 pg_exporter + Grafana 直接查
 * {@code pg_stat_database} 拿，不在 Java 侧维护。
 */
@Getter
