...
/**
 * 批处理日切（cutoff）扫描器：定时检查状态为 OPEN 的 {@code batch_day_instance}，
 * 当当前本地时间超过租户配置的 cutoffTime 后将其标记为已切日。
 *
 * <p>默认 cutoffTime 为 06:00；每条候选记录带自己的时区，确保多地域租户的切日时间各自独立。
 * ShedLock 防止多节点重复处理同一批次；每条记录的 {@code markCutoff} 用 CAS 更新，
 * 即使并发也只有一次会成功（避免重复切日）。
 */
@Slf4j
@Component
