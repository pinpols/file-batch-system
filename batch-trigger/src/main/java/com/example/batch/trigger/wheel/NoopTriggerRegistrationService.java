 ...
 * 而是直接由 {@link HashedWheelTriggerScheduler} + {@link WheelTriggerReconciler} 根据
 * {@code trigger_runtime_state} 表 + cron 表达式驱动 fire。
 *
 * <p>历史上 {@link TriggerRegistrationService} 接口的实现是 {@code TriggerSchedulerFacade} (仅在 Quartz
 * 模式装配),wheel 模式下 {@link TriggerRegistrationService} 没有对应 bean → 任何依赖它的代码
 * (如 {@link com.example.batch.trigger.infrastructure.QuartzLaunchJob} 的 tenant-suspend 自愈分支、
 * {@link com.example.batch.trigger.web.TriggerManagementController} 的运维 API)无法注入。
 *
 * <p>本类提供空实现 — 所有 Quartz-specific 操作降级为 no-op + warn 日志,
 * {@link #schedulerStatus()} 返回 {@code "WHEEL"} 表示本服务运行在时间轮模式。
 * 运维 API 在 wheel 模式下应使用 wheel-specific 端点(待补);本接口下的操作仅保留兼容性。
 */
@Slf4j
@Component
@ConditionalOnProperty(name = "batch.trigger.scheduler-impl", havingValue = "wheel", matchIfMissing = true)
