...
public class CatchUpThrottle {
   ...
   *
   * <p><b>线程上下文(R-arch-audit-2026-05-23 P1)</b>:本方法包含 {@link Thread#sleep}, 必须从可阻塞线程 (e.g. {@code
   * triggerFireExecutor} 异步线程池) 调用,**严禁**在 Netty {@code HashedWheelTimer} 的 worker 线程内直接调用 —
   * 否则会阻塞整个 wheel tick 处理,导致其他等待 fire 的 task 全部延迟。
   *
   * <p>当前 fire 路径 (HashedWheelTriggerScheduler.fire → handleMisfire → acquire) 通过 P0 5a 修复将 DB
   * 操作异步化到独立 executor 后,sleep 不再阻塞 wheel worker。若新增调用方,务必 复核该调用方线程上下文。
   */
