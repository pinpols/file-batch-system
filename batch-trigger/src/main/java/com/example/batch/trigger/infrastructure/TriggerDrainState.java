 ...
 *
 * <p>R-arch-audit-2026-05-23 P1: 历史上 {@link TriggerGracefulShutdown} 同时承担状态保管 + Quartz
 * {@code Scheduler.standby/start/shutdown} 调度,在 wheel 模式下 Quartz 不再驱动 fire,但 {@code
 * isDraining()} 语义仍由 Quartz standby 状态计算,与 wheel 真实停机路径
 * ({@link com.example.batch.trigger.wheel.HashedWheelTriggerScheduler#shutdown()} 走 {@code @PreDestroy})
 * 完全脱节。
 *
 * <p>本类抽离 draining 标志位与时间戳,作为 scheduler-agnostic 的真值源:
 *
 * <ul>
 *   <li>{@link TriggerGracefulShutdown} 在 startDraining/stopDraining 时更新本状态,
 *       然后再分别调用 scheduler-specific 的停机动作 (Quartz: standby; Wheel: 由 @PreDestroy 接管)。
...
public class TriggerDrainState {
  ...
  /**
   * CAS 启动排水。返回 {@code true} 表示本调用是首次置位 (调用方应做 scheduler-specific 停机动作), 返回 {@code false} 表示已在排水中,幂等无副作用。
   */
