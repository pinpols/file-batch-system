 ...
 *
 * <p>过去的行为：secret 错配要等到第一次真实 trigger 打到 Orchestrator 才被 401 拦截，
 * 发生时刻往往远离启动日志，排障成本高。现在在 {@link ApplicationReadyEvent} 触发时调一次
 * {@code GET /internal/orchestrator/drain/status}（轻量 + 已鉴权）探测：
 *
 * <ul>
 *   <li>2xx：记 INFO，启动继续
 *   <li>401 / 403：<b>log.error</b> 明示"secret 不一致"；启动继续（不 fail-fast 以避免
 *       trigger 启动失败造成连锁延迟调度），但运维应在启动后 1 分钟内发现告警
