...
/**
 * Unit tests for DefaultDispatchStageExecutor — verifies three stage exit paths:
 * - success: step returns DispatchStageResult.success
...
class DefaultDispatchStageExecutorTest {
    ...
    void execute_returnsBusinessError_whenStepThrowsBizException() {
        when(prepareStep.execute(any()))
                .thenThrow(new BizException(ResultCode.INVALID_ARGUMENT, "invalid dispatch config"));
        DispatchJobContext context = buildContext();
        ...
        assertThat(results.get(0).message()).isEqualTo("invalid dispatch config");
    ...
    private DispatchJobContext buildContext(String implCode) {
        ...
        PipelineStepDefinition step = new PipelineStepDefinition(
                1L, 1L, "DISPATCH_PREPARE", "Dispatch Prepare", DispatchStage.PREPARE.name(),
