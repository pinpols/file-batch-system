...
/**
 * DISPATCH pipeline 阶段执行契约：只负责"按上下文跑一次"。 默认 step 模板的供给（启动期 pipeline_definition 自动登记）独立到 {@link
 * com.example.batch.worker.core.support.PipelineStepTemplateProvider}，避免初始化职责污染执行接口。
 */
