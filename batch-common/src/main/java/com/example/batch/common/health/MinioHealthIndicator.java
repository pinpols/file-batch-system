 ...
 *
 * <p>readiness group 会把本探针纳入判断，MinIO 挂时 Pod 从 Service 端点摘除，
 * 避免"接了请求再发现 IO 失败"。liveness 不含本探针——MinIO 抖动不值得重启 JVM。
 *
 * <p>使用 MinioAutoConfiguration 里配置过超时（connect 5s / read 30s），
 * 健康检查不会因 MinIO 全挂而阻塞健康端点响应。
 */
