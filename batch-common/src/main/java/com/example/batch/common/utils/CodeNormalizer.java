 ...
 *
 * <ul>
 *   <li>{@link #normalizeGroupCode} — 分组码（worker_group / tenant_id 等）：去空白 → 大写 → 格式检查 {@code
 *       ^[A-Z][A-Z0-9_]*$}。入口统一为大写是行业惯例（如 Kubernetes Labels / Kafka Topic 的 PascalCase 变体），匹配规则明确。
 *   <li>{@link #normalizeConfigCode} — 配置码（window_code / calendar_code / queue_code / channel_code
 *       / template_code 等）：去空白 → 小写 → 连字符替为下划线 → 格式检查 {@code
 *       ^[a-z0-9][a-z0-9_]*$}。配置码是运营用户手输的可读字符串，小写+下划线对 URL / 日志 / SQL identifier 都友好。
 * </ul>
 *
