...
public final class MinioBucketSupport {
    ...
    private static final ConcurrentHashMap<String, AtomicLong> LAST_FAILURE_LOG_AT = new ConcurrentHashMap<>();

    private MinioBucketSupport() {
    }

    public static boolean ensureBucket(MinioClient minioClient, String bucket, Logger log, String componentName) {
        ...
        try {
            ...
            AtomicLong lastLoggedAt = LAST_FAILURE_LOG_AT.computeIfAbsent(key, ignored -> new AtomicLong(0L));
            long previous = lastLoggedAt.get();
            if (now - previous >= FAILURE_LOG_COOLDOWN_MILLIS && lastLoggedAt.compareAndSet(previous, now)) {
                log.warn("{} minio bucket ensure failed: bucket={}, cause={}",
                        componentName, bucket, ex.getMessage());
            }
            return false;
        }
    }
