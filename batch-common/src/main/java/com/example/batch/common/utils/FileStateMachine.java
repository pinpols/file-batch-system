...
public final class FileStateMachine {
    ...
    private static final EnumSet<FileStatus> INITIAL_STATES = EnumSet.of(
            FileStatus.RECEIVED,
            FileStatus.GENERATED
    );

    private static final Map<FileStatus, EnumSet<FileStatus>> TRANSITIONS = Map.ofEntries(
            Map.entry(FileStatus.RECEIVED, EnumSet.of(FileStatus.PARSING, FileStatus.FAILED, FileStatus.ARCHIVED)),
            ...
            Map.entry(FileStatus.GENERATED, EnumSet.of(FileStatus.DISPATCHING, FileStatus.ARCHIVED, FileStatus.FAILED)),
            Map.entry(FileStatus.DISPATCHING, EnumSet.of(FileStatus.DISPATCHED, FileStatus.FAILED)),
            Map.entry(FileStatus.DISPATCHED, EnumSet.of(FileStatus.DISPATCHING, FileStatus.ARCHIVED, FileStatus.FAILED)),
            Map.entry(FileStatus.ARCHIVED, EnumSet.of(FileStatus.DELETED)),
            Map.entry(FileStatus.FAILED, EnumSet.of(FileStatus.PARSING, FileStatus.GENERATED, FileStatus.DISPATCHING, FileStatus.ARCHIVED)),
    ...
    public static void assertInitialStatus(String statusCode) {
        ...
        if (!INITIAL_STATES.contains(status)) {
            throw new BizException(ResultCode.STATE_CONFLICT, "unsupported initial file status: " + statusCode);
        }
    }
    ...
    public static void assertTransition(String currentStatusCode, String nextStatusCode) {
        ...
        EnumSet<FileStatus> allowed = TRANSITIONS.getOrDefault(current, EnumSet.noneOf(FileStatus.class));
    ...
    private static FileStatus parse(String statusCode) {
        ...
        if (status == null) {
            throw new BizException(ResultCode.INVALID_ARGUMENT, "file status is required");
        }
        return status;
    }
}
