...
public final class CodingConventionsArchRules {
   ...
   *
   * <p>"覆盖"= 配置列表里存在某个 entry 等于或为 prefix 的祖先包(如配置 {@code "io.github.pinpols.batch"} 覆盖 {@code
  ...
  public static ArchRule componentScanCoversRule(String... requiredPrefixes) {
    return ArchRuleDefinition.classes()
        ...
        .allowEmptyShould(true)
        .because(
            "新增 io.github.pinpols.batch.common.<subpkg> 后,所有 SB Application 的 ComponentScan 必须能扫到,"
                + "否则 bean 启动 NoSuchBean。defaults: 加 scanBasePackages=\"io.github.pinpols.batch\""
                + " 或在列表里显式补 "
