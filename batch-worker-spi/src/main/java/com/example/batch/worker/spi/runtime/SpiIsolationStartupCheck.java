...
import com.example.batch.worker.spi.http.HttpExecutorProperties;
import com.example.batch.worker.spi.shell.ShellExecutorProperties;
 ...
 *
 * <p><b>背景</b>:batch-worker-spi 同一个镜像可按 {@code batch.worker.executors.*} 开关启用 shell / sql /
 ...
 * {@code @ConfigurationProperties}</b>:{@code batch.worker.spi} 前缀已被 {@code SpiWorkerConfiguration}
 * (record)与 {@code BatchWorkerSpiProperties} 绑定。再加一个绑同前缀的 properties 类会与现有 record 产生构造绑定冲突 (record
 * 强类型构造无法容忍未知子键的部分绑定语义),因此本类直接用 {@code Environment.getProperty} 读两个 boolean flag,零绑定侵入。
 *
...
@Slf4j
@Component
public class SpiIsolationStartupCheck {

  static final String PROP_REQUIRE_ISOLATION = "batch.worker.spi.require-isolation";
  static final String PROP_ISOLATION_ACKNOWLEDGED = "batch.worker.spi.isolation-acknowledged";
  ...
  public SpiIsolationStartupCheck(
