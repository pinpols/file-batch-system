...
/**
 * {@link SpiIsolationStartupCheck} 单测 —— mock {@link Environment} + executor {@link
 * ObjectProvider}。
 *
...
class SpiIsolationStartupCheckTest {
  ...
  private SpiIsolationStartupCheck newCheck() {
    return new SpiIsolationStartupCheck(
    ...
    lenient()
        .when(
            environment.getProperty(
                SpiIsolationStartupCheck.PROP_REQUIRE_ISOLATION, Boolean.class, false))
        .thenReturn(requireIsolation);
    lenient()
        .when(
            environment.getProperty(
                SpiIsolationStartupCheck.PROP_ISOLATION_ACKNOWLEDGED, Boolean.class, false))
