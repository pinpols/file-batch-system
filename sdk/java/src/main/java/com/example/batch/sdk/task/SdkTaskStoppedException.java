 ...
 * commit} 在<b>已提交的安全点</b>抛出本异常 —— 取消总是停在两个批次之间的边界,不会留半个批次的脏数据。模板顶层统一捕获并落 cancelled 终态。
 *
 * <p><b>约定:业务代码不得吞掉本异常</b>(吞了就停不下来)。它继承 {@link RuntimeException} 以便穿透业务调用栈,但语义是「正常停止」而非「失败」, 模板据此落
 * cancelled 而非 failed。{@link #breakPosition()} 携带停止时的断点,便于运维 / 续跑定位。
 */
