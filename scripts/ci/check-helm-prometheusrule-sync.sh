...
# deploy/docker/observability/prometheus-batch-rules.yml 是同一份规则的两个落地点。
...
# 同步办法:cp deploy/docker/observability/prometheus-batch-rules.yml \
#            helm/batch-platform/files/prometheus-batch-rules.yml
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
canonical="$repo_root/deploy/docker/observability/prometheus-batch-rules.yml"
...
echo >&2
echo "修复:cp deploy/docker/observability/prometheus-batch-rules.yml helm/batch-platform/files/prometheus-batch-rules.yml" >&2
exit 1
