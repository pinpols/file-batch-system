...
source "$ROOT/scripts/lib/runtime-defaults.sh"

export PGHOST="${BATCH_PLATFORM_DB_HOST:-localhost}"
export PGPORT="${POSTGRES_PORT:-$BATCH_DEFAULT_POSTGRES_PORT}"
export PGUSER="${BATCH_PLATFORM_DB_USERNAME:-$BATCH_DEFAULT_POSTGRES_USERNAME}"
export PGDATABASE="${BATCH_PLATFORM_DB_NAME:-$BATCH_DEFAULT_POSTGRES_DATABASE}"
export BATCH_PLATFORM_DB_PASSWORD="${BATCH_PLATFORM_DB_PASSWORD:?BATCH_PLATFORM_DB_PASSWORD required}"
export POSTGRES_CONTAINER="${POSTGRES_CONTAINER:-$BATCH_DEFAULT_POSTGRES_CONTAINER}"
export ORCH_URL="http://localhost:${ORCH_PORT:-$BATCH_DEFAULT_ORCHESTRATOR_PORT}"
export TRIGGER_URL="http://localhost:${TRIGGER_PORT:-$BATCH_DEFAULT_TRIGGER_PORT}"
export KAFKA_HOST_PORT="${KAFKA_HOST_PORT:-$BATCH_DEFAULT_KAFKA_HOST_PORT}"
export KAFKA_CONTAINER="${KAFKA_CONTAINER:-$BATCH_DEFAULT_KAFKA_CONTAINER}"
