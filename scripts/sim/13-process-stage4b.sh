...
export BATCH_NO="${BATCH_NO:-sim-process-stage4b-$(date +%Y%m%d%H%M%S)}"
export BATCH_KEY="${BATCH_KEY:-$BATCH_NO-jsonb-idempotent}"
export RUN_ID="${RUN_ID:-process-stage4b-$(date +%Y%m%d%H%M%S)}"
...
target_out = psql("batch_business", target_assert_sql, tuples=True)
staging_out = psql("batch_business", staging_assert_sql, tuples=True)
