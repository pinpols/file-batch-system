...
#
# 用法:
#   bash scripts/data/load-console-demo.sh          # 导入
#   bash scripts/data/load-console-demo.sh --reset   # 清空+导入（等同默认行为）
#   bash scripts/data/load-console-demo.sh --counts   # 只看各表行数
# ============================================================
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
SEED_SQL="${ROOT_DIR}/scripts/data/console-demo-seed.sql"
