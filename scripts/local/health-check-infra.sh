...
PG_PRIMARY_PORT="${PG_PRIMARY_PORT:-${POSTGRES_PORT:-$BATCH_DEFAULT_POSTGRES_PORT}}"
PG_REPLICA_HOST="${PG_REPLICA_HOST:-localhost}"
PG_REPLICA_PORT="${PG_REPLICA_PORT:-${POSTGRES_REPLICA_PORT:-15433}}"
PG_USER="${PGUSER:-${POSTGRES_USER:-$BATCH_DEFAULT_POSTGRES_USERNAME}}"
PG_PASSWORD="${PGPASSWORD:-${POSTGRES_PASSWORD:-$BATCH_DEFAULT_POSTGRES_PASSWORD}}"
PG_DB="${POSTGRES_DB:-$BATCH_DEFAULT_POSTGRES_DATABASE}"
PG_REPLICA_LAG_THRESHOLD_SEC="${PG_REPLICA_LAG_THRESHOLD_SEC:-300}"  # 5min 默认

REDIS_HOST="${REDIS_HOST:-$BATCH_DEFAULT_REDIS_HOST}"
REDIS_PORT="${REDIS_PORT:-$BATCH_DEFAULT_REDIS_PORT}"

KAFKA_BOOTSTRAP="${KAFKA_BOOTSTRAP:-$(batch_format_host_port "${KAFKA_HOST:-localhost}" "${KAFKA_HOST_PORT:-$BATCH_DEFAULT_KAFKA_HOST_PORT}")}"

MINIO_URL="${MINIO_URL:-http://localhost:${MINIO_API_PORT:-$BATCH_DEFAULT_MINIO_API_PORT}}"
MINIO_BUCKET="${MINIO_BUCKET:-$BATCH_DEFAULT_MINIO_BUCKET}"
