...
-- ADR-029:专用 Task SPI worker(batch-worker-spi)示例原子任务 job。job_type='TASK' → 派发到
-- batch.task.dispatch.task,worker_group='task'。default_params 携带 SPI executor 协议(taskType + 各自参数)。
...
INSERT INTO batch.job_definition (
    ...
    (2004, 'default-tenant', 'spi_shell_demo', 'SPI Shell Demo', 'TASK', 'GENERAL', 'MANUAL', NULL, 'Asia/Shanghai', 5, 'task_queue', 'task', 'default-calendar', 'always_open', 'MANUAL', FALSE, 'STATIC', 'EXPONENTIAL', 1, 300, NULL, jsonb_build_object('type', 'object'), jsonb_build_object('taskType', 'shell', 'command', '/bin/echo', 'args', jsonb_build_array('hello-from-spi')), 1, TRUE, 'SPI 示例:shell 原子任务', 'system', 'system', TIMESTAMPTZ '2026-05-30 08:00:00+08', TIMESTAMPTZ '2026-05-30 08:00:00+08'),
    (2005, 'default-tenant', 'spi_sql_demo', 'SPI SQL Demo', 'TASK', 'GENERAL', 'MANUAL', NULL, 'Asia/Shanghai', 5, 'task_queue', 'task', 'default-calendar', 'always_open', 'MANUAL', FALSE, 'STATIC', 'EXPONENTIAL', 1, 300, NULL, jsonb_build_object('type', 'object'), jsonb_build_object('taskType', 'sql', 'sql', 'SELECT 1'), 1, TRUE, 'SPI 示例:sql 原子任务(只读 SELECT)', 'system', 'system', TIMESTAMPTZ '2026-05-30 08:00:00+08', TIMESTAMPTZ '2026-05-30 08:00:00+08'),
    (2006, 'default-tenant', 'spi_stored_proc_demo', 'SPI Stored Proc Demo', 'TASK', 'GENERAL', 'MANUAL', NULL, 'Asia/Shanghai', 5, 'task_queue', 'task', 'default-calendar', 'always_open', 'MANUAL', FALSE, 'STATIC', 'EXPONENTIAL', 1, 300, NULL, jsonb_build_object('type', 'object'), jsonb_build_object('taskType', 'stored_proc', 'procedureName', 'batch.refresh_metrics'), 1, TRUE, 'SPI 示例:stored-proc 原子任务', 'system', 'system', TIMESTAMPTZ '2026-05-30 08:00:00+08', TIMESTAMPTZ '2026-05-30 08:00:00+08'),
    (2007, 'default-tenant', 'spi_http_demo', 'SPI HTTP Demo', 'TASK', 'GENERAL', 'MANUAL', NULL, 'Asia/Shanghai', 5, 'task_queue', 'task', 'default-calendar', 'always_open', 'MANUAL', FALSE, 'STATIC', 'EXPONENTIAL', 1, 300, NULL, jsonb_build_object('type', 'object'), jsonb_build_object('taskType', 'http', 'url', 'https://example.internal/health', 'method', 'GET'), 1, TRUE, 'SPI 示例:http 原子任务', 'system', 'system', TIMESTAMPTZ '2026-05-30 08:00:00+08', TIMESTAMPTZ '2026-05-30 08:00:00+08')
ON CONFLICT (id) DO NOTHING;
