...
TRUNCATE TABLE
    ...
    batch.job_task,
    batch.job_instance_dedup_key,
    batch.job_instance,
    batch.outbox_event_dedup_key,
    batch.outbox_event,
...
INSERT INTO batch.business_calendar (
    ...
    (1301, 'default-tenant', 'default_calendar', 'Default Calendar', 'Asia/Shanghai', 'NEXT_WORKDAY', 'AUTO', 3, TRUE, TIMESTAMPTZ '2026-03-22 08:00:00+08', TIMESTAMPTZ '2026-03-22 08:00:00+08'),
...
INSERT INTO batch.worker_registry (
    ...
    -- capability_tags 必须为 JSON 字符串数组（与 WorkerHeartbeatDto / WorkerCapabilityTagsAuditScheduler 约定一致）
    (1501, 'default-tenant', 'import-node-1', 'import', 'import-host-1', '127.0.0.1', '10001', jsonb_build_array('ingest', 'JSON', 'DELIMITED', 'EXCEL', 'XML', 'FIXED_WIDTH'), 'ingest', 'ONLINE', current_timestamp, current_timestamp, 'v1', 2, NULL, NULL, current_timestamp, current_timestamp),
    (1502, 'default-tenant', 'export-node-1', 'export', 'export-host-1', '127.0.0.1', '10002', jsonb_build_array('report', 'JSON', 'DELIMITED', 'EXCEL', 'XML', 'FIXED_WIDTH'), 'report', 'ONLINE', current_timestamp, current_timestamp, 'v1', 1, NULL, NULL, current_timestamp, current_timestamp),
    (1503, 'default-tenant', 'dispatch-node-1', 'dispatch', 'dispatch-host-1', '127.0.0.1', '10003', jsonb_build_array('delivery', 'API', 'API_PUSH', 'LOCAL', 'NAS', 'OSS', 'SFTP', 'EMAIL'), 'delivery', 'ONLINE', current_timestamp, current_timestamp, 'v1', 3, NULL, NULL, current_timestamp, current_timestamp),
    (1504, 'default-tenant', 'dispatch-node-drain', 'dispatch', 'dispatch-host-2', '127.0.0.2', '10004', jsonb_build_array('delivery', 'NAS', 'OSS'), 'delivery', 'DRAINING', current_timestamp, current_timestamp, 'v1', 0, current_timestamp - interval '5 minutes', current_timestamp + interval '55 minutes', current_timestamp, current_timestamp),
    -- tenant-finance EXPORT 队列 finance_export_queue(resource_tag=finance) 需要组内至少一台 ONLINE；
    -- OFFLINE 会导致 DefaultWorkerSelector candidates=0 / no_online_workers_in_group。
    (1505, 'tenant-finance', 'finance-export-node', 'export', 'finance-export-host', '127.0.0.3', '10005', jsonb_build_array('finance', 'DELIMITED', 'EXCEL'), 'finance', 'ONLINE', current_timestamp, current_timestamp, 'v1', 0, NULL, NULL, current_timestamp, current_timestamp);
...
INSERT INTO batch.job_definition (
    ...
    (2001, 'default-tenant', 'import_customer_job', 'Customer Import Job', 'IMPORT', 'CUSTOMER', 'MANUAL', NULL, 'Asia/Shanghai', 3, 'import_queue', 'import', 'default_calendar', 'always_open', 'MIXED', FALSE, 'DYNAMIC', 'EXPONENTIAL', 3, 3600, 'com.example.ImportCustomerJobHandler', jsonb_build_object('type', 'object'), jsonb_build_object('streamingEnabled', true, 'templateCode', 'import_customer_v1'), 1, TRUE, 'System test import job', 'system', 'system', TIMESTAMPTZ '2026-03-22 08:00:00+08', TIMESTAMPTZ '2026-03-22 08:00:00+08'),
    (2002, 'default-tenant', 'export_settlement_job', 'Settlement Export Job', 'EXPORT', 'SETTLEMENT', 'MANUAL', NULL, 'Asia/Shanghai', 4, 'export_queue', 'export', 'default_calendar', 'always_open', 'MANUAL', TRUE, 'STATIC', 'FIXED', 2, 7200, 'com.example.ExportSettlementJobHandler', jsonb_build_object('type', 'object'), jsonb_build_object('pageSize', 1000, 'templateCode', 'export_settlement_v1'), 1, TRUE, 'System test export job', 'system', 'system', TIMESTAMPTZ '2026-03-22 08:00:00+08', TIMESTAMPTZ '2026-03-22 08:00:00+08'),
...
-- ADR-029:专用原子任务 worker(batch-worker-atomic)示例原子任务 job。job_type='ATOMIC' → 派发到
-- batch.task.dispatch.atomic,worker_group='atomic'。default_params 携带执行器协议(taskType + 各自参数)。
-- demo/sim 用样例;executor 默认全关,真跑需在 atomic worker 上 enable + 配白名单/schema(见 ADR-029)。
INSERT INTO batch.job_definition (
    id, tenant_id, job_code, job_name, job_type, biz_type, schedule_type, schedule_expr, timezone, priority,
    queue_code, worker_group, calendar_code, window_code, trigger_mode, dag_enabled, shard_strategy, retry_policy,
    retry_max_count, timeout_seconds, execution_handler, param_schema, default_params, version, enabled, description,
    ...
    (2004, 'default-tenant', 'atomic_shell_demo', 'Atomic Shell Demo', 'ATOMIC', 'GENERAL', 'MANUAL', NULL, 'Asia/Shanghai', 5, 'atomic_queue', 'atomic', 'default_calendar', 'always_open', 'MANUAL', FALSE, 'STATIC', 'EXPONENTIAL', 1, 300, NULL, jsonb_build_object('type', 'object'), jsonb_build_object('taskType', 'shell', 'command', '/bin/echo', 'args', jsonb_build_array('hello-from-atomic')), 1, TRUE, '原子任务示例:shell', 'system', 'system', TIMESTAMPTZ '2026-05-30 08:00:00+08', TIMESTAMPTZ '2026-05-30 08:00:00+08'),
    (2005, 'default-tenant', 'atomic_sql_demo', 'Atomic SQL Demo', 'ATOMIC', 'GENERAL', 'MANUAL', NULL, 'Asia/Shanghai', 5, 'atomic_queue', 'atomic', 'default_calendar', 'always_open', 'MANUAL', FALSE, 'STATIC', 'EXPONENTIAL', 1, 300, NULL, jsonb_build_object('type', 'object'), jsonb_build_object('taskType', 'sql', 'sql', 'SELECT 1'), 1, TRUE, '原子任务示例:sql(只读 SELECT)', 'system', 'system', TIMESTAMPTZ '2026-05-30 08:00:00+08', TIMESTAMPTZ '2026-05-30 08:00:00+08'),
    (2006, 'default-tenant', 'atomic_stored_proc_demo', 'Atomic Stored Proc Demo', 'ATOMIC', 'GENERAL', 'MANUAL', NULL, 'Asia/Shanghai', 5, 'atomic_queue', 'atomic', 'default_calendar', 'always_open', 'MANUAL', FALSE, 'STATIC', 'EXPONENTIAL', 1, 300, NULL, jsonb_build_object('type', 'object'), jsonb_build_object('taskType', 'stored_proc', 'procedureName', 'batch.refresh_metrics'), 1, TRUE, '原子任务示例:stored-proc', 'system', 'system', TIMESTAMPTZ '2026-05-30 08:00:00+08', TIMESTAMPTZ '2026-05-30 08:00:00+08'),
    (2007, 'default-tenant', 'atomic_http_demo', 'Atomic HTTP Demo', 'ATOMIC', 'GENERAL', 'MANUAL', NULL, 'Asia/Shanghai', 5, 'atomic_queue', 'atomic', 'default_calendar', 'always_open', 'MANUAL', FALSE, 'STATIC', 'EXPONENTIAL', 1, 300, NULL, jsonb_build_object('type', 'object'), jsonb_build_object('taskType', 'http', 'url', 'https://example.internal/health', 'method', 'GET'), 1, TRUE, '原子任务示例:http', 'system', 'system', TIMESTAMPTZ '2026-05-30 08:00:00+08', TIMESTAMPTZ '2026-05-30 08:00:00+08')
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO batch.file_template_config (
    ...
    (5001, 'default-tenant', 'import_customer_v1', 'Customer Import Template', 'IMPORT', 'CUSTOMER', 'DELIMITED', 'UTF-8', 'UTF-8', FALSE, E'\n', ',', '"', '\\', 0, 1, 0, jsonb_build_object('title', 'customer-import'), NULL, 'SHA-256', 'NONE', 'NONE', 'customer-${bizDate}-${version}', jsonb_build_object('customer_no', 'customerNo', 'customer_name', 'customerName'), jsonb_build_object('required', jsonb_build_array('customerNo','customerName')), 'customer_query', 'select * from biz.customer_account where tenant_id = :tenantId', jsonb_build_object(
        'tenantId', 'string',
    ...
    (5002, 'default-tenant', 'import_customer_json_v1', 'Customer Import JSON Template', 'IMPORT', 'CUSTOMER', 'JSON', 'UTF-8', 'UTF-8', FALSE, E'\n', NULL, NULL, NULL, 0, 0, 0, jsonb_build_object('title', 'customer-import-json'), NULL, 'SHA-256', 'NONE', 'NONE', 'customer-json-${bizDate}-${version}', jsonb_build_object('customer_no', 'customerNo', 'customer_name', 'customerName'), jsonb_build_object('required', jsonb_build_array('customerNo','customerName')), 'customer_query_json', 'select * from biz.customer_account where tenant_id = :tenantId', jsonb_build_object(
        'tenantId', 'string',
        'jdbcMappedImport', jsonb_build_object(
            'schema', 'biz',
            'table', 'customer_account',
    ...
    (5003, 'default-tenant', 'export_settlement_v1', 'Settlement Export Template', 'EXPORT', 'SETTLEMENT', 'DELIMITED', 'UTF-8', 'UTF-8', TRUE, E'\n', ',', '"', '\\', 0, 1, 0, jsonb_build_object('title', 'settlement-export'), jsonb_build_object('title', 'settlement-export-trailer'), 'SHA-256', 'NONE', 'NONE', 'settlement-${bizDate}-${version}', jsonb_build_object('settlement_no', 'settlementNo', 'net_amount', 'netAmount'), jsonb_build_object('required', jsonb_build_array('settlementNo')), 'settlement_query', 'select d.id, d.tenant_id, b.batch_no, d.settlement_no, d.customer_no, d.biz_date, d.gross_amount, d.fee_amount, d.net_amount from biz.settlement_detail d join biz.settlement_batch b on b.id = d.batch_id and b.tenant_id = d.tenant_id where d.tenant_id = :tenantId and b.batch_no = :batchNo order by d.id asc', jsonb_build_object(
