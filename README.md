...
File Batch System 是一个多模块 Spring Boot 应用，实现了从文件接收、解析、校验、加载到分发的完整批处理链路，核心架构遵循 `DB → Outbox → Kafka → CLAIM → EXECUTE → REPORT` 主链。此外有专用 SPI worker(batch-worker-spi)承载 shell/sql/stored-proc/http 原子任务,与文件 pipeline worker 隔离(ADR-029)。
...
| `batch-worker-spi` | 18087 | 专用原子任务 worker(ADR-029):shell / sql / stored-proc / http 执行器,不带文件 pipeline;dual-use(RCE 级)能力隔离到最小权限进程 |
