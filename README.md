...
| `batch-worker-sdk-python` | — | Python SDK(ADR-035 跨语言对等实现)。Python 3.12+ async-only,pydantic v2 / httpx / aiokafka。**独立工具链**(pip),不进 Maven reactor;跨 SDK contract drift 由 Lane P guard。详见 [`batch-worker-sdk-python/README.md`](batch-worker-sdk-python/README.md) |
...
- JDK 25（**强制**：主工程 `maven.compiler.release=25`，**不支持 JDK 17/21 直接构建主工程**——低版本 JDK 会编译失败或产出与 CI/生产不一致的字节码。CI/Docker base image 已统一 temurin 25，本地务必对齐）
- Docker（用于本地基础设施）
- Maven 3.9+
